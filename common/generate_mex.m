...
if( GenerateAll || GenerateResampling )
    disp("Mexing Resampling : " );
    rep_c_simplex = 'gen_mex/resampling';
    ctx = InitFeedoptPlan( FeedoptDefaultConfig );
    codegen('-config', cfg,'-d', rep_c_simplex , ...
        'resampling', '-args', {ctx},...
        '-o', rep_c_simplex + 'resampling_mex');
end

if( GenerateAll || GenerateSimplex )
    disp( "Mexing simplex solver : " );
