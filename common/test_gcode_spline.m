...
f_r = @(u) [-Rpiece*cos(Ntour*2*pi/u1*u).*sin(pi/2*(1 - u/u1)); -Rpiece*sin(Ntour*2*pi/u1*u).*sin(pi/2*(1 - u/u1)); offset_secu_z+(Rpiece-offset_secu_z)*cos(pi/2*(1 - u/u1)); 0*u; unit_rot/4*(1 - u/u1); Ntour*unit_rot/u1*u];
% code G pour un dôme bizarre sans RTCP
f_a = @(u) [(-d1+offset_secu_z)*sin(pi/2*(1 - u/u1)); 0*u; Rpiece+d1-d1*cos(pi/2*(1 - u/u1)); 0*u; unit_rot/4*(1 - u/u1); Ntour*unit_rot/u1*u];
...
figure;
plot(u, r_a(1,:), u, r_a(2,:), u, r_a(3,:));%, u, r_a(5,:), u, r_a(6,:));
hold on
plot(u, fr_a(1,:), u, fr_a(2,:), u, fr_a(3,:));%, u, fr_a(5,:), u, fr_a(6,:));
legend('r_aX', 'r_aY', 'r_aZ', 'fr_aX', 'fr_aY', 'fr_aZ');%, 'r_aB', 'r_aC');
grid;

figure;
plot(u, fr_r(1,:), u, fr_r(2,:), u, fr_r(3,:), u, fr_r(5,:), u, fr_r(6,:));
legend('X', 'Y', 'Z', 'B', 'C');
grid;
