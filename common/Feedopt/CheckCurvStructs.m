...
for k = 2 : N                       % 从第二条曲线开始遍历到最后一条
    ...
    % ── 尖点检测触发门控 ────────────────────────────────────────────────
    % 条件一：~isAZeroEnd(curv1)
    %   curv1 末端尚未标记零速（NZ/ZZ）。若已是零速则机床本来就会停下，
    %   无需重复插入，直接跳过。
    %
    % 条件二：iscusp(...)
    %   仅取笛卡尔轴（indCart = XYZ）的切向量做夹角检测，忽略旋转轴。
    %   cos(angle) = dot(t1,t2)/(‖t1‖·‖t2‖) < CuspThreshold 则判为尖点。
    %   两个条件同时成立才触发：在边界插入 NZ/ZN 零速停顿，
    %   防止尖点处产生过大加速度冲击。
    if ( ~isAZeroEnd( curv1 ) ) && ...
