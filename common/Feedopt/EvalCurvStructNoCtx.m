...
a = curv.a_param;   % 参数窗口宽度：定义局部参数区间 [0,1] 映射到全局参数区间的长度
                    % 取值范围通常为 (0, 1]：
                    %   - a = 1 时表示当前曲线段使用完整参数区间
                    %   - a < 1 时表示当前曲线段是原曲线的子段（截取段）
                    %   - a = 0 不合法，因为会导致参数映射退化
b = curv.b_param;   % 参数窗口起点：全局参数区间的起点偏移量
                    % 取值范围通常为 [0, +∞)，但在有效曲线段内应该使 [b, b+a]
                    % 落在该曲线对应的全局参数定义域内
