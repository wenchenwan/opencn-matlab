...
% derivative. 
function [p5_3D, status, alpha0, alpha1] = G2_Hermite_Interpolation(r0D0, r0D1, r0D2, r1D0, r1D1, r1D2)
...
[t0, n0, kappa0] = CalcFrenet(r0D1, r0D2);
[t1, n1, kappa1] = CalcFrenet(r1D1, r1D2);
...
CoefPS = CoefPolySys(r0D0, t0, n0, kappa0, r1D0, t1, n1, kappa1);
%
a1     = CoefPS(1);
a0     = CoefPS(2);
b1     = CoefPS(3);
...
elseif kappa0 == 0
    ...
    if isempty(alpha1_t)
        % alpha1_t<0
        alpha1_t = real(alpha1_v(abs(imag(alpha1_v)) < TolZero));
    end
...
elseif kappa1 == 0
    ...
    if isempty(alpha0_t)
        % alpha0_t<0
        alpha0_t = real(alpha0_v(abs(imag(alpha0_v)) < TolZero));
    end
...
p5_3D = r0D0 * h00 + alpha0*t0 * h10 + (beta0*t0 + kappa0*alpha0^2*n0) * h20 + ...
        r1D0 * h01 + alpha1*t1 * h11 + (beta1*t1 + kappa1*alpha1^2*n1) * h21;
%% last cross check ...
