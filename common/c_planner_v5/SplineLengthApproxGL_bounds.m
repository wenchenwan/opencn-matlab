...
%
if u2 < Knots(k)
    L = splineLengthApprox_Interval(ctx.cfg, spline, u1, u2);       % u1 and u2 in the same break interval
else
    L = splineLengthApprox_Interval(ctx.cfg, spline, u1, Knots(k)); % remaining length to next break point
    while u2 > Knots(k)
        L = L + Lk(k);  % sum up precalculated length between knots
        k = k + 1;
    end
    L = L + splineLengthApprox_Interval(ctx.cfg, spline, Knots(k), u2); % remaining length to u2
end
