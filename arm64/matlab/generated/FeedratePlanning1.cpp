                      ...
                      ::coder::array<double, 2U> &Coeff, int *NCoeff, bool *success, int *status)
{
    ...
    for (int i10{0}; i10 < h_loop_ub; i10++) {
        ...
        for (int i11{0}; i11 < i_loop_ub; i11++) {
            r[i11 + r.size(0) * i10] = -b[i11 + b.size(0) * i10];
        }
    }
    A.vertcat(r, &b_A);
    ...
    b_A.vertcat(&A_jerk, &r1);
    c_simplex(f, &r1, b_b, Aeq, beq, Coeff2, &c_success, &c_status);
    d_success = c_success;
    ...
    if (!c_success) {
        ...
        if (ctx->cfg.opt.USE_SLACK_ON_JERK) {
            ...
            r.set_size(b_A.m, A_jerk.n);
            n_loop_ub = A_jerk.n;
            for (int i23{0}; i23 < n_loop_ub; i23++) {
                int o_loop_ub;
                o_loop_ub = b_A.m;
                for (int i24{0}; i24 < o_loop_ub; i24++) {
                    r[i24 + r.size(0) * i23] = 0.0;
                }
            }
            A_jerk.vertcat(r, &Atot);
            ...
            coder::b_eml_find(&b_Atot, ii, jj, Avs);
            ...
            while ((!d_success) && (max_increase > 0)) {
                ...
                b_A.vertcat(&A_jerk, &r2);
                c_simplex(f, &r2, b_b, Aeq, beq, Coeff2, &d_success, &c_status);
    ...
    if (!d_success) {
        ...
        b_EvalCurvStruct(&ctx->q_splines, CurvStructsArray[0].Type, CurvStructsArray[0].P0,
                         ...
                         CurvStructsArray[0].b_param, 1.0, a__1, r1D, r2D, r3D);
        ...
        c_EvalCurvStruct(&ctx->q_splines, CurvStructsArray[0].Type, CurvStructsArray[0].P0,
                         ...
                         CurvStructsArray[0].pitch, CurvStructsArray[0].CoeffP5,
                         CurvStructsArray[0].sp_index, CurvStructsArray[0].a_param,
                         CurvStructsArray[0].b_param, a__2, r1D);
        ...
        ctx->at_0 = ((r2D[0] * q_val + 0.5 * b_idx_0) * (r1D[0] / n_tmp) +
...
//
// File trailer for FeedratePlanning1.cpp
//
// [EOF]
//
