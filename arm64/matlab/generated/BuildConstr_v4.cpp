                    ...
                    ::coder::array<double, 1U> &beq)
{
    ...
    for (int b_k{0}; b_k <= i60 - 2; b_k++) {
            ...
            for (int i89{0}; i89 < pc_loop_ub; i89++) {
                ...
                for (int i93{0}; i93 < qc_loop_ub; i93++) {
                    R3[i93 + R3.size(0) * i89] =
                        r1[i93 + r1.size(0) * i89] + 0.5 * r2[i93 + r2.size(0) * i89];
                }
            }
        } else {
            c_binary_expand_op(R3, r1, r2);
        ...
        if (BasisVal.size(1) == BasisValD.size(1)) {
            ...
            coder::internal::blas::mtimes(t_1, g_a, r5);
            fd_loop_ub = r5.size(1);
            for (int i131{0}; i131 < fd_loop_ub; i131++) {
                Aeq[(static_cast<int>(u) + Aeq.size(0) * (i129 + i131)) + 1] = r5[i131];
            }
        } else {
        ...
        hd_loop_ub = r3.size(1);
        for (int i134{0}; i134 < hd_loop_ub; i134++) {
            Aeq[(static_cast<int>(u) + Aeq.size(0) * ((i132 + i134) - 1)) + 1] = -r3[i134];
        }
        //
    ...
    for (int i66{0}; i66 < vb_loop_ub; i66++) {
        Aeq[unnamed_idx_1 + Aeq.size(0) * (i64 + i66)] = r3[i66];
    }
    int b_beq;
    //
    ...
    coder::b_linspace(ctx_cfg_opt_VEL_RAMP_OVER_WINDOWS, static_cast<double>(M), r3);
    vel_ramp.set_size(r3.size(1));
    ...
    // M)',1,6);
    coder::b_linspace(ctx_cfg_opt_ACC_RAMP_OVER_WINDOWS, static_cast<double>(M), r3);
    r4.set_size(r3.size(1));
    dc_loop_ub = r3.size(1);
    for (int i73{0}; i73 < dc_loop_ub; i73++) {
        r4[i73] = r3[i73];
    }
    acc_ramp.set_size(r4.size(0), 6);
    if (r4.size(0) != 0) {
        int na;
        na = r4.size(0);
        for (int t{0}; t < 6; t++) {
            for (int d_k{0}; d_k < na; d_k++) {
                acc_ramp[d_k + acc_ramp.size(0) * t] = r4[d_k];
            }
        }
    }
