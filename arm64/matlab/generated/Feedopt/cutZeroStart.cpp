                  ...
                  CurvStruct *curv2)
{
    ...
    // 'cutZeroStart:12' [ ret, curv1, curv2 ] = cutCurvStruct( ctx, curv, 0, Lcut, false );
    cutCurvStruct(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size, ctx_cfg_maskCart_data,
                  ctx_cfg_maskCart_size, ctx_cfg_maskRot_data, ctx_cfg_maskRot_size,
                  ctx_cfg_indCart, ctx_cfg_indRot, ctx_cfg_NumberAxis, ctx_cfg_NCart, ctx_cfg_NRot,
                  ctx_cfg_GaussLegendreX, ctx_cfg_GaussLegendreW, curv, Lcut, &b_ret, curv1, curv2);
    ...
    if (b_ret < 0.0) {
        ...
        *curv1 = *curv;
        ...
        // 'cutZeroStart:17' [ ~, ~, ~, jps ]        = zeroSpeedCurv( ctx, curv1, false );
        b_ctx_kin = *ctx_kin;
        b_curv1 = *curv1;
        zeroSpeedCurv(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
        ...
        // 'cutZeroStart:20' ocn_assert( check_curv_length( ctx, curv1, Lcut ), "Curve Length not
        ...
        x = LengthCurv(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
                       ...
                       curv1) -
            Lcut;
        s_ocn_assert(std::abs(x) <= 0.1);
        // 'cutZeroStart:21' curv1.UseConstJerk      = true;
        curv1->UseConstJerk = true;
        // 'cutZeroStart:22' [ ~, ~, ~, jps ]        = zeroSpeedCurv( ctx, curv1, false );
        c_ctx_kin = *ctx_kin;
        c_curv1 = *curv1;
        zeroSpeedCurv(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
                      ctx_cfg_maskCart_data, ctx_cfg_maskCart_size, ctx_cfg_maskRot_data,
                      ctx_cfg_maskRot_size, ctx_cfg_indCart, ctx_cfg_indRot, ctx_cfg_NumberAxis,
                      ctx_cfg_NCart, ctx_cfg_NRot, ctx_cfg_vmax, ctx_cfg_amax, ctx_cfg_jmax,
                      ctx_cfg_dt, &c_ctx_kin, &c_curv1, a__4, a__5, a__6, &curv1->ConstJerk);
        ...
        // 'cutZeroStart:60' r           = repmat( curv.R0( ctx.cfg.maskTot ), 1, 2, 3 );
        ...
        // 'cutZeroStart:61' rd          = r;
        // 'cutZeroStart:62' rdd         = r;
        ...
        for (int j{0}; j < 3; j++) {
            ...
            int c_loop_ub;
            int loop_ub;
            // 'cutZeroStart:65' [ r( :, :, j ) , rd( :, :, j ), rdd( :, :, j ) ] = ...
            // 'cutZeroStart:66'         EvalCurvStruct( ctx, curv_vec( j ),  u_vec );
            ...
            // 'cutZeroStart:67' rd( :, :, j )    = rd( :, :, j )  / curv_vec( j ).a_param;
            ...
            double c_tmp;
            // 'cutZeroStart:68' if( 1 )
            // 'cutZeroStart:69' rdd( :, :, j )   = rdd( :, :, j ) / curv_vec( j ).a_param^2;
            ...
            for (int i10{0}; i10 < 2; i10++) {
                ...
                for (int i16{0}; i16 <= rdd; i16++) {
                    b_rdd_data[i16 + trueCount * i10] =
                        rdd_data[(i16 + trueCount * i10) + trueCount * 2 * j] / c;
                }
            }
            for (int i12{0}; i12 < 2; i12++) {
                for (int i15{0}; i15 < trueCount; i15++) {
                    rdd_data[(i15 + trueCount * i12) + trueCount * 2 * j] =
                        b_rdd_data[i15 + trueCount * i12];
                }
            }
        }
        // 'cutZeroStart:75' ind1 = [ 1, 1; 1, 2; 2, 2 ];
        // 'cutZeroStart:76' ind2 = [ 2, 1; 3, 2; 3, 1 ];
        // 'cutZeroStart:78' for j = 1 : 3
        for (int b_j{0}; b_j < 3; b_j++) {
            ...
            bool exitg1;
            bool varargout_1;
            // 'cutZeroStart:79' r1   = r( : , ind1( j, 2 ), ind1( j, 1 ) );
            // 'cutZeroStart:80' r1d  = rd( : , ind1( j, 2 ), ind1( j, 1 ) );
            // 'cutZeroStart:81' r1dd = rdd( : , ind1( j, 2 ), ind1( j, 1 ) );
            // 'cutZeroStart:82' r2   = r( : , ind2( j, 2 ), ind2( j, 1 ) );
            // 'cutZeroStart:83' r2d  = rd( : , ind2( j, 2 ), ind2( j, 1 ) );
            // 'cutZeroStart:84' r2dd = rdd( : , ind2( j, 2 ), ind2( j, 1 ) );
            // 'cutZeroStart:85' assert_continuity( r1, r1d, r1dd, r2, r2d, r2dd );
            // 'cutZeroStart:92' diff_r      = abs( r1    -r2 );
            b_x.set_size(trueCount);
            for (int i9{0}; i9 < trueCount; i9++) {
                b_x[i9] = r_data[(i9 + trueCount * (ind1[1][b_j] - 1)) +
                                 trueCount * 2 * (ind1[0][b_j] - 1)] -
                          r_data[(i9 + trueCount * (ind2[1][b_j] - 1)) +
            ...
            // 'cutZeroStart:93' diff_rd     = abs( r1d   -r2d );
            b_x.set_size(trueCount);
            for (int i14{0}; i14 < trueCount; i14++) {
                b_x[i14] = rd_data[(i14 + trueCount * (ind1[1][b_j] - 1)) +
                                   trueCount * 2 * (ind1[0][b_j] - 1)] -
                           rd_data[(i14 + trueCount * (ind2[1][b_j] - 1)) +
                                   trueCount * 2 * (ind2[0][b_j] - 1)];
            }
            ...
            // 'cutZeroStart:94' diff_rdd    = abs( r1dd  -r2dd );
            b_x.set_size(trueCount);
            for (int i18{0}; i18 < trueCount; i18++) {
                b_x[i18] = rdd_data[(i18 + trueCount * (ind1[1][b_j] - 1)) +
                                    trueCount * 2 * (ind1[0][b_j] - 1)] -
                           rdd_data[(i18 + trueCount * (ind2[1][b_j] - 1)) +
                                    trueCount * 2 * (ind2[0][b_j] - 1)];
            }
            ...
            // 'cutZeroStart:96' tol = 1E-6;
            // 'cutZeroStart:99' ocn_assert( all( diff_r    < tol, 'all' ), "Continuity C0 failed",
            // mfilename );
            c_x.set_size(z1.size(0));
            d_loop_ub = z1.size(0);
            for (int i20{0}; i20 < d_loop_ub; i20++) {
                c_x[i20] = (z1[i20] < 1.0E-6);
            }
            varargout_1 = true;
            e_k = 0;
            exitg1 = false;
            ...
            t_ocn_assert(varargout_1);
            // 'cutZeroStart:100' ocn_assert( all( diff_rd   < tol, 'all' ), "Continuity C1 failed",
            // mfilename );
            c_x.set_size(b_z1.size(0));
            e_loop_ub = b_z1.size(0);
            for (int i21{0}; i21 < e_loop_ub; i21++) {
                c_x[i21] = (b_z1[i21] < 1.0E-6);
            }
            b_varargout_1 = true;
            f_k = 0;
            exitg1 = false;
            ...
            u_ocn_assert(b_varargout_1);
            // 'cutZeroStart:101' ocn_assert( all( diff_rdd   < tol, 'all' ),"Continuity C2 failed",
            // mfilename );
            c_x.set_size(c_z1.size(0));
            f_loop_ub = c_z1.size(0);
            for (int i22{0}; i22 < f_loop_ub; i22++) {
                c_x[i22] = (c_z1[i22] < 1.0E-6);
            }
            c_varargout_1 = true;
            g_k = 0;
            exitg1 = false;
