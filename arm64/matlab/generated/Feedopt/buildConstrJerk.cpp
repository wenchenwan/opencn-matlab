    ...
    for (int b_k{0}; b_k < i6; b_k++) {
        ...
        for (int j{0}; j < ctx_cfg_NumberAxis; j++) {
            ...
            for (int f_k{0}; f_k < i21; f_k++) {
                h_y[f_k] = std::sqrt(h_y[f_k]);
            }
            r2.set_size(d_y.size(1));
            for (int i23{0}; i23 < j_loop_ub; i23++) {
                r2[i23] = static_cast<int>(d_y[i23]) + f_y;
            }
            ...
            if ((BasisVal.size(0) == r3D.size(1)) && (r.size(0) == BasisValD.size(0)) &&
                ...
                binary_expand_op(Jerk, r2, r3D, j, r3D.size(1), BasisVal, r, BasisValD, r1,
                                 BasisValDD, h_y);
            }
        }
        ...
        for (int i22{0}; i22 < p_loop_ub; i22++) {
            r3[i22] = static_cast<int>(static_cast<double>(d_y[i22]) + e_k) - 1;
        }
        ...
        for (int i30{0}; i30 < result; i30++) {
            for (int i32{0}; i32 < t_loop_ub; i32++) {
                Aj[r2[i32] + Aj.size(0) * r3[i30]] = Jerk[i32 + t_loop_ub * i30];
            }
        }
        for (int i31{0}; i31 < result; i31++) {
            for (int i34{0}; i34 < u_loop_ub; i34++) {
                Aj[r2[i34 + t_loop_ub] + Aj.size(0) * r3[i31]] = varargin_2[i34 + u_loop_ub * i31];
            }
        }
