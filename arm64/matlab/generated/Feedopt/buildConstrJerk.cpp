                     ...
                     ::coder::array<double, 1U> &b)
{
    ...
    for (int b_k{0}; b_k < i6; b_k++) {
        ...
        for (int j{0}; j < ctx_cfg_NumberAxis; j++) {
            ...
            for (int d_k{0}; d_k < inner; d_k++) {
                for (int c_i{0}; c_i <= mc; c_i++) {
                    r2[c_i] = r2[c_i] + BasisVal[c_i + BasisVal.size(0) * d_k] *
            ...
            if (r1.size(0) == 1) {
                i40 = BasisValDD.size(0);
            } else {
                i40 = r1.size(0);
            }
            if (BasisVal.size(1) == 1) {
                i42 = BasisValD.size(1);
            } else {
                i42 = BasisVal.size(1);
            }
            if (r3D.size(1) == 1) {
            ...
            if ((BasisVal.size(0) == r3D.size(1)) && (r.size(0) == BasisValD.size(0)) &&
                (i32 == i34) && (BasisVal.size(1) == BasisValD.size(1)) &&
                (r1.size(0) == BasisValDD.size(0)) && (i36 == i40) && (i42 == BasisValDD.size(1)) &&
                (i48 == r2.size(0))) {
                for (int i49{0}; i49 < o_loop_ub; i49++) {
                    int t_loop_ub;
                    t_loop_ub = r3D.size(1);
                    for (int i50{0}; i50 < t_loop_ub; i50++) {
                        Jerk[r3[i50] + Jerk.size(0) * i49] =
        ...
        for (int i30{0}; i30 < n_loop_ub; i30++) {
            ...
            for (int i33{0}; i33 < q_loop_ub; i33++) {
                c_varargin_2[i33 + c_varargin_2.size(0) * i30] = -Jerk[i33 + Jerk.size(0) * i30];
            }
        }
        ...
        for (int i37{0}; i37 < c_result; i37++) {
            for (int i39{0}; i39 < r_loop_ub; i39++) {
                Aj[r3[i39] + Aj.size(0) * r4[i37]] = Jerk[i39 + r_loop_ub * i37];
            }
        }
        for (int i38{0}; i38 < c_result; i38++) {
            for (int i41{0}; i41 < s_loop_ub; i41++) {
                Aj[r3[i41 + r_loop_ub] + Aj.size(0) * r4[i38]] =
                    c_varargin_2[i41 + s_loop_ub * i38];
            }
        }
        // 'buildConstrJerk:46' bj( indAL )          = [ b_jmax( : ); b_jmax( : ) ];
        unnamed_idx_1 = b_b.size(0) * b_b.size(1);
        b_unnamed_idx_1 = b_b.size(0) * b_b.size(1);
        for (int i43{0}; i43 < unnamed_idx_1; i43++) {
            bj[indAL[i43] - 1] = b_b[i43];
        }
        for (int i44{0}; i44 < b_unnamed_idx_1; i44++) {
            bj[indAL[i44 + unnamed_idx_1] - 1] = b_b[i44];
        }
    }
    //  Slack variables
    ...
    A.set_size(i22 + 1, sizes_idx_1);
    for (int i25{0}; i25 < sizes_idx_1; i25++) {
        for (int i27{0}; i27 < input_sizes_idx_0; i27++) {
            A[i27 + A.size(0) * i25] = b_result[i27 + input_sizes_idx_0 * i25];
        }
    }
    for (int i26{0}; i26 < sizes_idx_1; i26++) {
        A[input_sizes_idx_0 + A.size(0) * i26] = b_varargin_2[i26];
    }
