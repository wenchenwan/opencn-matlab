                           ...
                           const ::coder::array<double, 2U> &spline_sp_Lk, double L, double u0)
{
    ...
    if (d < u0) {
        ...
        for (int d_k{0}; d_k < 5; d_k++) {
            ...
            r1Dnorm[d_k] = d2;
            c_r1Dnorm += d2 * cfg_GaussLegendreW[d_k];
        }
    ...
    if (LkEndVec_size[1] == 0) {
        ...
        while ((count < 1000) && (std::abs(fk) > 1.0E-7)) {
            ...
            for (int i17{0}; i17 < 5; i17++) {
                double d3;
                d3 = cfg_GaussLegendreX[i17];
                dv[i17] = ((1.0 - uMiddle) * (1.0 - d3) + (1.0 - uStart_tmp) * (d3 + 1.0)) / 2.0;
            }
            EvalBSpline(spline_sp_Bl_handle, spline_sp_coeff, dv, a__1, r1D);
            // 'splineLengthApprox_Interval:36' r1Dnorm     = MyNorm( r1D );
            //  MyNorm : My implementation of the norm computation.
            ...
            for (int i18{0}; i18 < 5; i18++) {
                for (int i19{0}; i19 < i_loop_ub; i19++) {
                    double b_varargin_1;
                    b_varargin_1 = r1D[i19 + r1D.size(0) * i18];
                    r[i19 + r.size(0) * i18] = std::pow(b_varargin_1, 2.0);
                }
            }
            coder::sum(r, r1Dnorm);
            //  mysqrt : Custom implementation of the sqrt method.
            ...
            for (int e_k{0}; e_k < 5; e_k++) {
                ...
                r1Dnorm[e_k] = d4;
                d_r1Dnorm += d4 * cfg_GaussLegendreW[e_k];
            }
                         ...
                         const ::coder::array<double, 2U> &spline_sp_Lk, double L, double u0)
{
    ...
    if (c_spline_sp_knots < u0) {
        ...
        for (int i11{0}; i11 < 5; i11++) {
            ...
            d_spline_sp_knots[i11] = (c_spline_sp_knots * (1.0 - d) + u0 * (d + 1.0)) / 2.0;
        }
        ...
        for (int i12{0}; i12 < 5; i12++) {
            for (int i13{0}; i13 < f_loop_ub; i13++) {
                ...
                r[i13 + r.size(0) * i12] = std::pow(varargin_1, 2.0);
            }
        }
        coder::sum(r, r1Dnorm);
        //  mysqrt : Custom implementation of the sqrt method.
    ...
    if (LkEndVec_size[1] == 0) {
        ...
        while ((count < 1000) && (std::abs(fk) > 1.0E-7)) {
            ...
            for (int i15{0}; i15 < 5; i15++) {
                double d2;
                d2 = cfg_GaussLegendreX[i15];
                uStart[i15] = (uStart_tmp * (1.0 - d2) + uMiddle * (d2 + 1.0)) / 2.0;
            }
            ...
            for (int c_k{0}; c_k < 5; c_k++) {
                ...
                r1Dnorm[c_k] = d3;
                d_r1Dnorm += d3 * cfg_GaussLegendreW[c_k];
            }
