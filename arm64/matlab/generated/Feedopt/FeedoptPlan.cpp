...
void FeedoptPlan(FeedoptContext *ctx, bool *optimized, CurvStruct *opt_struct)
{
    ...
    // 'FeedoptPlan:4' ocn_assert( ctx.errcode == FeedoptPlanError.Success, ...
    // 'FeedoptPlan:5'     "FeedoptPlan: error code was not handled...", mfilename );
    // 'FeedoptPlan:7' optimized = false;
    b_optimized = false;
    // 'FeedoptPlan:9' opt_struct = constrCurvStructType;
    ...
    *opt_struct = opt_struct_tmp;
    // 'FeedoptPlan:11' switch ctx.op
    switch (ctx->op) {
    case Fopt_Init:
        // 'FeedoptPlan:12' case Fopt.Init
        // 'FeedoptPlan:13' ctx.op = Fopt.GCode;
        ctx->op = Fopt_GCode;
        //
        break;
    case Fopt_GCode: {
        int status;
        // 'FeedoptPlan:15' case Fopt.GCode
        // 'FeedoptPlan:16' ctx.k0  = int32(1);
        ctx->k0 = 1;
        ...
        std::copy(&ctx->cfg.source[0], &ctx->cfg.source[1024], &cv[0]);
        cv[1024] = '\x00';
        status = cpp_interp_init(&cv[0]);
        ...
        while (status != 0) {
            ...
            int b_status;
            int is_loaded;
            // 'FeedoptPlan:21' ctx.k0 = ctx.k0 + 1;
            ctx->k0++;
            // 'FeedoptPlan:22' [ status, CurvStruct ] = ReadGCode( ReadGCodeCmd.Read, ...
            // 'FeedoptPlan:23'                 ctx.cfg.source );
            //  coder.extrinsic('ReadGCode_mex');
            //  Wrapper for pulling the next gcode line from the interpreter
            // 'ReadGCode:6' status      = int32(0);
            // 'ReadGCode:7' CurvStruct  = constrCurvStructType;
            ...
            status = b_status;
            // 'FeedoptPlan:24' if( ctx.q_gcode.isempty )
            if (ctx->q_gcode.isempty()) {
                // 'FeedoptPlan:25' prev_tool = constrToolStructType;
                prev_tool_offset_x = 0.0;
                prev_tool_offset_y = 0.0;
                prev_tool_offset_z = 0.0;
            } else {
                // 'FeedoptPlan:26' else
                // 'FeedoptPlan:27' prev_tool = ctx.q_gcode.rget(1).tool;
                ctx->q_gcode.rget(&r1);
                prev_tool_offset_x = r1.tool.offset.x;
                prev_tool_offset_y = r1.tool.offset.y;
                prev_tool_offset_z = r1.tool.offset.z;
                // 'FeedoptPlan:29' if( ~toolIsEqual(prev_tool, CurvStruct.tool ) )
                if (!toolIsEqual(r1.tool.toolno, r1.tool.pocketno, r1.tool.offset.x,
                    ...
                    bool b_zeroFlag;
                    bool c_zeroFlag;
                    // 'FeedoptPlan:30' curv1 = ctx.q_gcode.rget(1);
                    ctx->q_gcode.rget(&curv1);
                    // 'FeedoptPlan:31' if( isAZeroStart(curv1) )
                    ...
                    if (b_zeroFlag) {
                        // 'FeedoptPlan:32' curv1.Info.zspdmode = ZSpdMode.ZZ;
                        curv1.Info.zspdmode = ZSpdMode_ZZ;
                    } else {
                        // 'FeedoptPlan:33' else
                        // 'FeedoptPlan:34' curv1.Info.zspdmode = ZSpdMode.NZ;
                        curv1.Info.zspdmode = ZSpdMode_NZ;
                    }
                    // 'FeedoptPlan:37' ctx.q_gcode.set(ctx.q_gcode.size, curv1);
                    ctx->q_gcode.set(ctx->q_gcode.size(), &curv1);
                    // 'FeedoptPlan:39' if( isAZeroEnd(CurvStruct) )
                    ...
                    if (c_zeroFlag) {
                        // 'FeedoptPlan:40' CurvStruct.Info.zspdmode = ZSpdMode.ZZ;
                        b_CurvStruct.Info.zspdmode = ZSpdMode_ZZ;
                    } else {
                        // 'FeedoptPlan:41' else
                        // 'FeedoptPlan:42' CurvStruct.Info.zspdmode = ZSpdMode.ZN;
                        b_CurvStruct.Info.zspdmode = ZSpdMode_ZN;
                    }
                }
            }
            // 'FeedoptPlan:47' [CurvStruct] = add_tool_offset( CurvStruct, ctx.cfg.indCart,
            // prev_tool );
            ...
            if (!b_CurvStruct.Info.TRAFO) {
                ...
                if (ctx->cfg.indCart.size[0] == 3) {
                    ...
                    for (int i130{0}; i130 < jf_loop_ub; i130++) {
                    ...
                    for (int i137{0}; i137 < CurvStruct_size; i137++) {
                        b_CurvStruct.R0[ctx->cfg.indCart.data[i137] - 1] = CurvStruct_data[i137];
                    }
                } else {
                ...
                if (ctx->cfg.indCart.size[0] == 3) {
                    ...
                    for (int i133{0}; i133 < mf_loop_ub; i133++) {
                        CurvStruct_data[i133] =
                            b_CurvStruct.R1[ctx->cfg.indCart.data[i133] - 1] + off[i133];
                    }
                    for (int i139{0}; i139 < CurvStruct_size; i139++) {
                        b_CurvStruct.R1[ctx->cfg.indCart.data[i139] - 1] = CurvStruct_data[i139];
                    }
                } else {
            ...
            // 'FeedoptPlan:49' CurvStruct.R0( 4 : end ) = deg2rad( CurvStruct.R0( 4 : end ) );
            // 'FeedoptPlan:50' CurvStruct.R1( 4 : end ) = deg2rad( CurvStruct.R1( 4 : end ) );
            ...
            // 'FeedoptPlan:52' for j = 1 : StructTypeName.NumberAxisMax
            // 'FeedoptPlan:61' if( CurvStruct.Info.FeedRate == 0.0 )
            if (b_CurvStruct.Info.FeedRate == 0.0) {
                // 'FeedoptPlan:62' CurvStruct.Info.FeedRate = ctx.cfg.fmax;
                b_CurvStruct.Info.FeedRate = ctx->cfg.fmax;
            }
            // 'FeedoptPlan:64' if( status == 1 && CurvStruct.Info.Type ~= CurveType.None )
            if ((b_status == 1) && (b_CurvStruct.Info.Type != CurveType_None)) {
                // 'FeedoptPlan:65' if ( CurvStruct.Info.FeedRate == 0.0 )
                if (b_CurvStruct.Info.FeedRate == 0.0) {
                    //  check for undefined feedrate
                    // 'FeedoptPlan:67' CurvStruct.Info.FeedRate = ctx.cfg.fmax;
                    b_CurvStruct.Info.FeedRate = ctx->cfg.fmax;
                }
                // 'FeedoptPlan:69' ctx.q_gcode.push( CurvStruct );
                ctx->q_gcode.push(&b_CurvStruct);
            }
        }
        // 'FeedoptPlan:72' if ctx.q_gcode.isempty()
        if (ctx->q_gcode.isempty()) {
            // 'FeedoptPlan:73' ctx.op = Fopt.Finished;
            ctx->op = Fopt_Finished;
            // 'FeedoptPlan:74' DebugLog(DebugCfg.Warning, ...
            // 'FeedoptPlan:75'                 'ERROR: Optimization failed, Gcode queue is
            // empty\n');
            //  1 -> stdout
            //  2 -> stderr
            // 'DebugLog:5' if IsEnabledDebugLog(cfg)
            ...
            bool zeroFlag;
            // 'FeedoptPlan:78' last = ctx.q_gcode.rget(1);
            ctx->q_gcode.rget(&last);
            // 'FeedoptPlan:79' if( isAZeroStart(last) )
            //  isAZeroStart : Return true if the curv starts with zero speed
            //  Input :
            //  curv / Info / ZSpdMode : A structure containning the information of the
            //  curv zero speed.
            ...
            if (zeroFlag) {
                // 'FeedoptPlan:80' last.Info.zspdmode = ZSpdMode.ZZ;
                last.Info.zspdmode = ZSpdMode_ZZ;
            } else {
                // 'FeedoptPlan:81' else
                // 'FeedoptPlan:82' last.Info.zspdmode = ZSpdMode.NZ;
                last.Info.zspdmode = ZSpdMode_NZ;
            }
            // 'FeedoptPlan:84' ctx.q_gcode.set( ctx.q_gcode.size, last );
            ctx->q_gcode.set(ctx->q_gcode.size(), &last);
            // 'FeedoptPlan:86' assert_queue( ctx.op, ctx.q_gcode );
            // 'FeedoptPlan:164' msg = string( op );
            // 'FeedoptPlan:165' ocn_assert( checkGeometry( queue ), ...
            // 'FeedoptPlan:166'     msg + " - Check geometry failed...", mfilename );
            checkGeometry(&ctx->q_gcode);
            // 'FeedoptPlan:167' ocn_assert( checkZSpdmode( queue ), ...
            // 'FeedoptPlan:168'     msg + " - Check zspdmode failed...", mfilename );
            checkZSpdmode(&ctx->q_gcode);
            // 'FeedoptPlan:169' ocn_assert( checkParametrisation( queue ), ...
            // 'FeedoptPlan:170'     msg + " - Check parametrisation failed...", mfilename );
            checkParametrisation(&ctx->q_gcode);
            // 'FeedoptPlan:88' ctx.op = Fopt.Check;
            ctx->op = Fopt_Check;
        }
    } break;
    case Fopt_Check: {
        // 'FeedoptPlan:90' case Fopt.Check
        //          ctx.op = Fopt.Finished; return;
        //          [ ctx.q_gcode ] = checkTrafo( ctx, ctx.q_gcode );
        //          histogramLength( ctx, ctx.q_gcode, "Gcode");
        // 'FeedoptPlan:94' if ~ctx.cfg.Cusp.Skip
        if (!ctx->cfg.Cusp.Skip) {
            ...
            int yc_loop_ub;
            int yd_loop_ub;
            // 'FeedoptPlan:95' ctx     = CheckCurvStructs( ctx );
            ...
            for (int i15{0}; i15 < o_loop_ub; i15++) {
                ...
                for (int i22{0}; i22 < v_loop_ub; i22++) {
                    r.BasisValD[i22 + r.BasisValD.size(0) * i15] =
            ...
            for (int i25{0}; i25 < y_loop_ub; i25++) {
                ...
                for (int i32{0}; i32 < gb_loop_ub; i32++) {
                    r.BasisValDD[i32 + r.BasisValDD.size(0) * i25] =
            ...
            for (int i35{0}; i35 < jb_loop_ub; i35++) {
                ...
                for (int i42{0}; i42 < qb_loop_ub; i42++) {
                    r.BasisValDDD[i42 + r.BasisValDDD.size(0) * i35] =
            ...
            for (int i65{0}; i65 < ed_loop_ub; i65++) {
            ...
            for (int i85{0}; i85 < od_loop_ub; i85++) {
                int td_loop_ub;
                td_loop_ub = ctx->Coeff.size(0);
                for (int i90{0}; i90 < td_loop_ub; i90++) {
            ...
            for (int i158{0}; i158 < ah_loop_ub; i158++) {
            ...
            for (int i178{0}; i178 < kh_loop_ub; i178++) {
                int ph_loop_ub;
                ph_loop_ub = r.Coeff.size(0);
                for (int i183{0}; i183 < ph_loop_ub; i183++) {
        ...
        // 'FeedoptPlan:98' assert_queue( ctx.op, ctx.q_gcode );
        // 'FeedoptPlan:164' msg = string( op );
        // 'FeedoptPlan:165' ocn_assert( checkGeometry( queue ), ...
        // 'FeedoptPlan:166'     msg + " - Check geometry failed...", mfilename );
        checkGeometry(&ctx->q_gcode);
        ...
        // 'FeedoptPlan:170'     msg + " - Check parametrisation failed...", mfilename );
        checkParametrisation(&ctx->q_gcode);
        // 'FeedoptPlan:100' ctx.op  = Fopt.Compress;
        ctx->op = Fopt_Compress;
    } break;
    case Fopt_Compress: {
        // 'FeedoptPlan:102' case Fopt.Compress
        //          ctx.op = Fopt.Finished; return;
        // 'FeedoptPlan:105' if ctx.cfg.Compressing.Skip
        if (ctx->cfg.Compressing.Skip) {
            int i;
            unsigned int u;
            // 'FeedoptPlan:106' for j = 1 : ctx.q_gcode.size
            u = ctx->q_gcode.size();
            i = static_cast<int>(u);
            for (int j{0}; j < i; j++) {
                //  Copy queue GCode in queue Compress
                // 'FeedoptPlan:107' ctx.q_compress.push( ctx.q_gcode.get( j ) );
            ...
            int yf_loop_ub;
            int yg_loop_ub;
            // 'FeedoptPlan:109' else
            // 'FeedoptPlan:110' ctx = compressCurvStructs(ctx);
            r.BasisVal.set_size(ctx->BasisVal.size(0), ctx->BasisVal.size(1));
            ...
            for (int i3{0}; i3 < c_loop_ub; i3++) {
                ...
                for (int i10{0}; i10 < j_loop_ub; i10++) {
                    r.BasisVal[i10 + r.BasisVal.size(0) * i3] =
                        ctx->BasisVal[i10 + ctx->BasisVal.size(0) * i3];
                }
            }
            r.BasisValD.set_size(ctx->BasisValD.size(0), ctx->BasisValD.size(1));
            ...
            for (int i33{0}; i33 < hb_loop_ub; i33++) {
                ...
                for (int i40{0}; i40 < ob_loop_ub; i40++) {
                    r.BasisValDDD[i40 + r.BasisValDDD.size(0) * i33] =
                        ctx->BasisValDDD[i40 + ctx->BasisValDDD.size(0) * i33];
                }
            }
            r.BasisIntegr.set_size(ctx->BasisIntegr.size(0));
            rb_loop_ub = ctx->BasisIntegr.size(0);
            for (int i43{0}; i43 < rb_loop_ub; i43++) {
                r.BasisIntegr[i43] = ctx->BasisIntegr[i43];
            }
            r.Bl = ctx->Bl;
            r.u_vec.set_size(1, ctx->u_vec.size(1));
            ...
            for (int i63{0}; i63 < cd_loop_ub; i63++) {
            ...
            for (int i83{0}; i83 < md_loop_ub; i83++) {
                int rd_loop_ub;
                rd_loop_ub = ctx->Coeff.size(0);
                for (int i88{0}; i88 < rd_loop_ub; i88++) {
                    r.Coeff[i88 + r.Coeff.size(0) * i83] =
                        ctx->Coeff[i88 + ctx->Coeff.size(0) * i83];
                }
            }
            r.Skipped = ctx->Skipped;
            r.kin = ctx->kin;
            ...
            for (int i93{0}; i93 < wd_loop_ub; i93++) {
                int ee_loop_ub;
                ee_loop_ub = r.BasisVal.size(0);
                for (int i100{0}; i100 < ee_loop_ub; i100++) {
                    ctx->BasisVal[i100 + ctx->BasisVal.size(0) * i93] =
                        r.BasisVal[i100 + r.BasisVal.size(0) * i93];
                }
            }
            ctx->BasisValD.set_size(r.BasisValD.size(0), r.BasisValD.size(1));
            he_loop_ub = r.BasisValD.size(1);
            for (int i103{0}; i103 < he_loop_ub; i103++) {
                int oe_loop_ub;
                oe_loop_ub = r.BasisValD.size(0);
                for (int i110{0}; i110 < oe_loop_ub; i110++) {
                    ctx->BasisValD[i110 + ctx->BasisValD.size(0) * i103] =
                        r.BasisValD[i110 + r.BasisValD.size(0) * i103];
                }
            }
            ctx->BasisValDD.set_size(r.BasisValDD.size(0), r.BasisValDD.size(1));
            ...
            for (int i113{0}; i113 < re_loop_ub; i113++) {
                ...
                for (int i120{0}; i120 < ye_loop_ub; i120++) {
                    ctx->BasisValDD[i120 + ctx->BasisValDD.size(0) * i113] =
                        r.BasisValDD[i120 + r.BasisValDD.size(0) * i113];
                }
            }
            ctx->BasisValDDD.set_size(r.BasisValDDD.size(0), r.BasisValDDD.size(1));
            cf_loop_ub = r.BasisValDDD.size(1);
            for (int i123{0}; i123 < cf_loop_ub; i123++) {
                int kf_loop_ub;
                kf_loop_ub = r.BasisValDDD.size(0);
                for (int i131{0}; i131 < kf_loop_ub; i131++) {
                    ctx->BasisValDDD[i131 + ctx->BasisValDDD.size(0) * i123] =
                        r.BasisValDDD[i131 + r.BasisValDDD.size(0) * i123];
                }
            }
            ctx->BasisIntegr.set_size(r.BasisIntegr.size(0));
            of_loop_ub = r.BasisIntegr.size(0);
            for (int i135{0}; i135 < of_loop_ub; i135++) {
                ctx->BasisIntegr[i135] = r.BasisIntegr[i135];
            }
            ctx->Bl = r.Bl;
            ctx->u_vec.set_size(1, r.u_vec.size(1));
            tf_loop_ub = r.u_vec.size(1);
            for (int i142{0}; i142 < tf_loop_ub; i142++) {
            ...
            for (int i157{0}; i157 < yg_loop_ub; i157++) {
            ...
            for (int i177{0}; i177 < jh_loop_ub; i177++) {
                int oh_loop_ub;
                oh_loop_ub = r.Coeff.size(0);
                for (int i182{0}; i182 < oh_loop_ub; i182++) {
        ...
        // 'FeedoptPlan:113' assert_queue( ctx.op, ctx.q_compress );
        // 'FeedoptPlan:164' msg = string( op );
        // 'FeedoptPlan:165' ocn_assert( checkGeometry( queue ), ...
        // 'FeedoptPlan:166'     msg + " - Check geometry failed...", mfilename );
        ...
        // 'FeedoptPlan:115' ctx.op = Fopt.Smooth;
        ctx->op = Fopt_Smooth;
        // 'FeedoptPlan:116' if( coder.target( 'MATLAB') )
    } break;
    case Fopt_Smooth: {
        int ac_loop_ub;
        int ad_loop_ub;
        ...
        int wf_loop_ub;
        int wg_loop_ub;
        // 'FeedoptPlan:118' case Fopt.Smooth
        //          ctx.op = Fopt.Finished; return;
        //          histogramLength( ctx, ctx.q_compress, "Compressing" );
        // 'FeedoptPlan:122' ctx = smoothCurvStructs(ctx);
        r.BasisVal.set_size(ctx->BasisVal.size(0), ctx->BasisVal.size(1));
        ...
        for (int i8{0}; i8 < h_loop_ub; i8++) {
            ...
            for (int i16{0}; i16 < p_loop_ub; i16++) {
                r.BasisValD[i16 + r.BasisValD.size(0) * i8] =
                    ctx->BasisValD[i16 + ctx->BasisValD.size(0) * i8];
            }
        }
        r.BasisValDD.set_size(ctx->BasisValDD.size(0), ctx->BasisValDD.size(1));
        ...
        for (int i18{0}; i18 < r_loop_ub; i18++) {
            ...
            for (int i26{0}; i26 < ab_loop_ub; i26++) {
                r.BasisValDD[i26 + r.BasisValDD.size(0) * i18] =
                    ctx->BasisValDD[i26 + ctx->BasisValDD.size(0) * i18];
            }
        }
        r.BasisValDDD.set_size(ctx->BasisValDDD.size(0), ctx->BasisValDDD.size(1));
        ...
        for (int i28{0}; i28 < cb_loop_ub; i28++) {
            ...
            for (int i36{0}; i36 < kb_loop_ub; i36++) {
                r.BasisValDDD[i36 + r.BasisValDDD.size(0) * i28] =
                    ctx->BasisValDDD[i36 + ctx->BasisValDDD.size(0) * i28];
            }
        }
        r.BasisIntegr.set_size(ctx->BasisIntegr.size(0));
        ...
        for (int i51{0}; i51 < pc_loop_ub; i51++) {
            r.cfg.indCart[i51] = ctx->cfg.indCart.data[i51];
        }
        r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        uc_loop_ub = ctx->cfg.indRot.size[0];
        for (int i56{0}; i56 < uc_loop_ub; i56++) {
            r.cfg.indRot[i56] = ctx->cfg.indRot.data[i56];
        }
        r.cfg.NumberAxis = ctx->cfg.NumberAxis;
        r.cfg.NCart = ctx->cfg.NCart;
        r.cfg.NRot = ctx->cfg.NRot;
        ...
        for (int i61{0}; i61 < ad_loop_ub; i61++) {
            r.cfg.D[i61] = ctx->cfg.D.data[i61];
        }
        r.cfg.coeffD = ctx->cfg.coeffD;
        r.cfg.kin_params.size[0] = ctx->cfg.kin_params.size[0];
        ...
        for (int i66{0}; i66 < 5; i66++) {
            r.cfg.kin_type[i66] = ctx->cfg.kin_type[i66];
        }
        r.cfg.NDiscr = ctx->cfg.NDiscr;
        r.cfg.NBreak = ctx->cfg.NBreak;
        r.cfg.UseDynamicBreakpoints = ctx->cfg.UseDynamicBreakpoints;
        ...
        for (int i71{0}; i71 < 6; i71++) {
            r.cfg.vmax[i71] = ctx->cfg.vmax[i71];
            r.cfg.amax[i71] = ctx->cfg.amax[i71];
            r.cfg.jmax[i71] = ctx->cfg.jmax[i71];
        }
        r.cfg.LeeSplineDegree = ctx->cfg.LeeSplineDegree;
        r.cfg.SplineDegree = ctx->cfg.SplineDegree;
        r.cfg.CutOff = ctx->cfg.CutOff;
        ...
        for (int i81{0}; i81 < kd_loop_ub; i81++) {
            int pd_loop_ub;
            pd_loop_ub = ctx->Coeff.size(0);
            for (int i86{0}; i86 < pd_loop_ub; i86++) {
                r.Coeff[i86 + r.Coeff.size(0) * i81] = ctx->Coeff[i86 + ctx->Coeff.size(0) * i81];
            }
        }
        r.Skipped = ctx->Skipped;
        r.kin = ctx->kin;
        ...
        for (int i91{0}; i91 < ud_loop_ub; i91++) {
            int ae_loop_ub;
            ae_loop_ub = r.BasisVal.size(0);
            for (int i96{0}; i96 < ae_loop_ub; i96++) {
                ctx->BasisVal[i96 + ctx->BasisVal.size(0) * i91] =
                    r.BasisVal[i96 + r.BasisVal.size(0) * i91];
            }
        }
        ctx->BasisValD.set_size(r.BasisValD.size(0), r.BasisValD.size(1));
        ce_loop_ub = r.BasisValD.size(1);
        for (int i98{0}; i98 < ce_loop_ub; i98++) {
            int ke_loop_ub;
            ke_loop_ub = r.BasisValD.size(0);
            for (int i106{0}; i106 < ke_loop_ub; i106++) {
                ctx->BasisValD[i106 + ctx->BasisValD.size(0) * i98] =
                    r.BasisValD[i106 + r.BasisValD.size(0) * i98];
            }
        }
        ctx->BasisValDD.set_size(r.BasisValDD.size(0), r.BasisValDD.size(1));
        me_loop_ub = r.BasisValDD.size(1);
        for (int i108{0}; i108 < me_loop_ub; i108++) {
            int ue_loop_ub;
            ue_loop_ub = r.BasisValDD.size(0);
            for (int i116{0}; i116 < ue_loop_ub; i116++) {
                ctx->BasisValDD[i116 + ctx->BasisValDD.size(0) * i108] =
                    r.BasisValDD[i116 + r.BasisValDD.size(0) * i108];
            }
        }
        ctx->BasisValDDD.set_size(r.BasisValDDD.size(0), r.BasisValDDD.size(1));
        we_loop_ub = r.BasisValDDD.size(1);
        for (int i118{0}; i118 < we_loop_ub; i118++) {
            int ff_loop_ub;
            ff_loop_ub = r.BasisValDDD.size(0);
            for (int i126{0}; i126 < ff_loop_ub; i126++) {
                ctx->BasisValDDD[i126 + ctx->BasisValDDD.size(0) * i118] =
                    r.BasisValDDD[i126 + r.BasisValDDD.size(0) * i118];
            }
        }
        ctx->BasisIntegr.set_size(r.BasisIntegr.size(0));
        hf_loop_ub = r.BasisIntegr.size(0);
        for (int i128{0}; i128 < hf_loop_ub; i128++) {
            ctx->BasisIntegr[i128] = r.BasisIntegr[i128];
        }
        ctx->Bl = r.Bl;
        ctx->u_vec.set_size(1, r.u_vec.size(1));
        rf_loop_ub = r.u_vec.size(1);
        for (int i140{0}; i140 < rf_loop_ub; i140++) {
            ctx->u_vec[i140] = r.u_vec[i140];
        }
        ctx->q_spline = r.q_spline;
        ctx->q_gcode = r.q_gcode;
        ctx->q_compress = r.q_compress;
        ...
        for (int i145{0}; i145 < mg_loop_ub; i145++) {
            ctx->cfg.indCart.data[i145] = r.cfg.indCart[i145];
        }
        ctx->cfg.indRot.size[0] = r.cfg.indRot.size(0);
        rg_loop_ub = r.cfg.indRot.size(0);
        for (int i150{0}; i150 < rg_loop_ub; i150++) {
            ctx->cfg.indRot.data[i150] = r.cfg.indRot[i150];
        }
        ctx->cfg.NumberAxis = r.cfg.NumberAxis;
        ctx->cfg.NCart = r.cfg.NCart;
        ctx->cfg.NRot = r.cfg.NRot;
        ...
        for (int i155{0}; i155 < wg_loop_ub; i155++) {
            ctx->cfg.D.data[i155] = r.cfg.D[i155];
        }
        ctx->cfg.coeffD = r.cfg.coeffD;
        ctx->cfg.kin_params.size[0] = r.cfg.kin_params.size[0];
        ...
        for (int i160{0}; i160 < 5; i160++) {
            ctx->cfg.kin_type[i160] = r.cfg.kin_type[i160];
        }
        ctx->cfg.NDiscr = r.cfg.NDiscr;
        ctx->cfg.NBreak = r.cfg.NBreak;
        ctx->cfg.UseDynamicBreakpoints = r.cfg.UseDynamicBreakpoints;
        ...
        ctx->cfg.fmax = r.cfg.fmax;
        ctx->cfg.smax = r.cfg.smax;
        for (int i165{0}; i165 < 6; i165++) {
            ctx->cfg.vmax[i165] = r.cfg.vmax[i165];
            ctx->cfg.amax[i165] = r.cfg.amax[i165];
            ctx->cfg.jmax[i165] = r.cfg.jmax[i165];
        }
        ctx->cfg.LeeSplineDegree = r.cfg.LeeSplineDegree;
        ctx->cfg.SplineDegree = r.cfg.SplineDegree;
        ctx->cfg.CutOff = r.cfg.CutOff;
        ...
        ctx->cfg.opt = r.cfg.opt;
        for (int i170{0}; i170 < 9; i170++) {
            ctx->cfg.LogFileName[i170] = r.cfg.LogFileName[i170];
        }
        ctx->errcode = r.errcode;
        ctx->jmax_increase_count = r.jmax_increase_count;
        ctx->zero_start = r.zero_start;
        ...
        for (int i175{0}; i175 < hh_loop_ub; i175++) {
            int lh_loop_ub;
            lh_loop_ub = r.Coeff.size(0);
            for (int i179{0}; i179 < lh_loop_ub; i179++) {
                ctx->Coeff[i179 + ctx->Coeff.size(0) * i175] =
                    r.Coeff[i179 + r.Coeff.size(0) * i175];
            }
        }
        ctx->Skipped = r.Skipped;
        ctx->kin = r.kin;
        // 'FeedoptPlan:123' ctx.op = Fopt.Split;
        ctx->op = Fopt_Split;
        // 'FeedoptPlan:125' assert_queue( ctx.op, ctx.q_smooth );
        // 'FeedoptPlan:164' msg = string( op );
        // 'FeedoptPlan:165' ocn_assert( checkGeometry( queue ), ...
        // 'FeedoptPlan:166'     msg + " - Check geometry failed...", mfilename );
        ...
        // 'FeedoptPlan:127' if( coder.target( 'MATLAB') )
    } break;
    case Fopt_Split: {
        int b_loop_ub;
        int bc_loop_ub;
        ...
        int xf_loop_ub;
        int xg_loop_ub;
        // 'FeedoptPlan:129' case Fopt.Split
        //          ctx.op = Fopt.Finished; return;
        //          histogramLength( ctx, ctx.q_smooth, "Smoothing" );
        // 'FeedoptPlan:133' ctx     = splitQueue( ctx );
        r.BasisVal.set_size(ctx->BasisVal.size(0), ctx->BasisVal.size(1));
        ...
        for (int i62{0}; i62 < bd_loop_ub; i62++) {
        ...
        for (int i82{0}; i82 < ld_loop_ub; i82++) {
            int qd_loop_ub;
            qd_loop_ub = ctx->Coeff.size(0);
            for (int i87{0}; i87 < qd_loop_ub; i87++) {
        ...
        for (int i92{0}; i92 < vd_loop_ub; i92++) {
            int be_loop_ub;
            be_loop_ub = r.BasisVal.size(0);
            for (int i97{0}; i97 < be_loop_ub; i97++) {
        ...
        for (int i99{0}; i99 < de_loop_ub; i99++) {
            int le_loop_ub;
            le_loop_ub = r.BasisValD.size(0);
            for (int i107{0}; i107 < le_loop_ub; i107++) {
        ...
        for (int i109{0}; i109 < ne_loop_ub; i109++) {
            int ve_loop_ub;
            ve_loop_ub = r.BasisValDD.size(0);
            for (int i117{0}; i117 < ve_loop_ub; i117++) {
        ...
        for (int i119{0}; i119 < xe_loop_ub; i119++) {
            int gf_loop_ub;
            gf_loop_ub = r.BasisValDDD.size(0);
            for (int i127{0}; i127 < gf_loop_ub; i127++) {
        ...
        for (int i129{0}; i129 < if_loop_ub; i129++) {
        ...
        for (int i156{0}; i156 < xg_loop_ub; i156++) {
        ...
        for (int i176{0}; i176 < ih_loop_ub; i176++) {
            int mh_loop_ub;
            mh_loop_ub = r.Coeff.size(0);
            for (int i180{0}; i180 < mh_loop_ub; i180++) {
        ...
        ctx->Skipped = r.Skipped;
        ctx->kin = r.kin;
        // 'FeedoptPlan:134' ctx.op  = Fopt.Opt;
        ctx->op = Fopt_Opt;
        // 'FeedoptPlan:136' assert_queue( ctx.op, ctx.q_split );
        // 'FeedoptPlan:164' msg = string( op );
        // 'FeedoptPlan:165' ocn_assert( checkGeometry( queue ), ...
        // 'FeedoptPlan:166'     msg + " - Check geometry failed...", mfilename );
        ...
        // 'FeedoptPlan:139' if( coder.target( 'MATLAB' ) )
    } break;
    case Fopt_Opt: {
        int bg_loop_ub;
        int bh_loop_ub;
        ...
        int xc_loop_ub;
        int xd_loop_ub;
        // 'FeedoptPlan:141' case Fopt.Opt
        // 'FeedoptPlan:142' if( ctx.q_opt.size() == 0 )
        if (ctx->q_opt.size() == 0U) {
            // 'FeedoptPlan:142' ctx.k0 = int32( 1 );
            ctx->k0 = 1;
        }
        //          ctx.op = Fopt.Finished; return;
        // 'FeedoptPlan:144' [ ctx, optimized, opt_struct, quit ] = feedratePlanning( ctx );
        ...
        for (int i4{0}; i4 < d_loop_ub; i4++) {
            ...
            for (int i11{0}; i11 < k_loop_ub; i11++) {
                b_ctx.BasisVal[i11 + b_ctx.BasisVal.size(0) * i4] =
        ...
        for (int i14{0}; i14 < n_loop_ub; i14++) {
            ...
            for (int i21{0}; i21 < u_loop_ub; i21++) {
                b_ctx.BasisValD[i21 + b_ctx.BasisValD.size(0) * i14] =
        ...
        for (int i24{0}; i24 < x_loop_ub; i24++) {
            ...
            for (int i31{0}; i31 < fb_loop_ub; i31++) {
                b_ctx.BasisValDD[i31 + b_ctx.BasisValDD.size(0) * i24] =
        ...
        for (int i34{0}; i34 < ib_loop_ub; i34++) {
            ...
            for (int i41{0}; i41 < pb_loop_ub; i41++) {
                b_ctx.BasisValDDD[i41 + b_ctx.BasisValDDD.size(0) * i34] =
        ...
        for (int i54{0}; i54 < sc_loop_ub; i54++) {
            b_ctx.cfg.indCart[i54] = ctx->cfg.indCart.data[i54];
        }
        b_ctx.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        xc_loop_ub = ctx->cfg.indRot.size[0];
        for (int i59{0}; i59 < xc_loop_ub; i59++) {
            b_ctx.cfg.indRot[i59] = ctx->cfg.indRot.data[i59];
        }
        ...
        for (int i64{0}; i64 < dd_loop_ub; i64++) {
            b_ctx.cfg.D[i64] = ctx->cfg.D.data[i64];
        }
        ...
        for (int i79{0}; i79 < 9; i79++) {
            b_ctx.cfg.LogFileName[i79] = ctx->cfg.LogFileName[i79];
        }
        ...
        for (int i84{0}; i84 < nd_loop_ub; i84++) {
            int sd_loop_ub;
            sd_loop_ub = ctx->Coeff.size(0);
            for (int i89{0}; i89 < sd_loop_ub; i89++) {
                b_ctx.Coeff[i89 + b_ctx.Coeff.size(0) * i84] =
        ...
        for (int i149{0}; i149 < qg_loop_ub; i149++) {
            ctx->cfg.indCart.data[i149] = b_ctx.cfg.indCart[i149];
        }
        ctx->cfg.indRot.size[0] = b_ctx.cfg.indRot.size(0);
        vg_loop_ub = b_ctx.cfg.indRot.size(0);
        for (int i154{0}; i154 < vg_loop_ub; i154++) {
            ctx->cfg.indRot.data[i154] = b_ctx.cfg.indRot[i154];
        }
        ...
        for (int i159{0}; i159 < bh_loop_ub; i159++) {
            ctx->cfg.D.data[i159] = b_ctx.cfg.D[i159];
        }
        ...
        for (int i164{0}; i164 < 5; i164++) {
            ctx->cfg.kin_type[i164] = b_ctx.cfg.kin_type[i164];
        }
        ...
        for (int i169{0}; i169 < 6; i169++) {
            ctx->cfg.vmax[i169] = b_ctx.cfg.vmax[i169];
            ctx->cfg.amax[i169] = b_ctx.cfg.amax[i169];
            ctx->cfg.jmax[i169] = b_ctx.cfg.jmax[i169];
        }
        ...
        for (int i174{0}; i174 < 9; i174++) {
            ctx->cfg.LogFileName[i174] = b_ctx.cfg.LogFileName[i174];
        }
        ...
        for (int i181{0}; i181 < nh_loop_ub; i181++) {
            int qh_loop_ub;
            qh_loop_ub = b_ctx.Coeff.size(0);
            for (int i184{0}; i184 < qh_loop_ub; i184++) {
                ctx->Coeff[i184 + ctx->Coeff.size(0) * i181] =
                    b_ctx.Coeff[i184 + b_ctx.Coeff.size(0) * i181];
            }
        }
        ctx->Skipped = b_ctx.Skipped;
        ctx->kin = b_ctx.kin;
        b_optimized = c_optimized;
        // 'FeedoptPlan:145' if optimized
        if (c_optimized) {
            // 'FeedoptPlan:146' ctx.go_next = true;
            ctx->go_next = true;
            // 'FeedoptPlan:147' ctx.q_opt.push( opt_struct );
            b_ctx.q_opt.push(opt_struct);
        }
        // 'FeedoptPlan:150' if( quit )
    } break;
    case Fopt_Finished:
        // 'FeedoptPlan:152' case Fopt.Finished
        // 'FeedoptPlan:153' ctx.op = Fopt.Finished;
        ctx->op = Fopt_Finished;
        break;
    default:
        // 'FeedoptPlan:155' otherwise
        // 'FeedoptPlan:156' DebugLog(DebugCfg.Global, 'FEEDOPT: WRONG STATE\n')
        //  1 -> stdout
        //  2 -> stderr
        // 'DebugLog:5' if IsEnabledDebugLog(cfg)
        // 'IsEnabledDebugLog:4' value = false;
        ...
        // 'FeedoptPlan:157' ctx.op = Fopt.Finished;
        ctx->op = Fopt_Finished;
        break;
    }
