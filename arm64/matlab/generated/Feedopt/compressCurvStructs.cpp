              ...
              CurvStruct *curv, CurvStruct *spline)
{
    ...
    *spline = *curv;
    ...
    for (int c_j{0}; c_j < i68; c_j++) {
        ...
        for (int i74{0}; i74 < qb_loop_ub; i74++) {
            r6[i74] = 0.0;
        }
        ...
        for (int i77{0}; i77 < sb_loop_ub; i77++) {
            r7[i77] = 0.0;
        }
    ...
    for (int i71{0}; i71 < nb_loop_ub; i71++) {
        ...
        for (int i72{0}; i72 < ob_loop_ub; i72++) {
            double c_varargin_1;
            c_varargin_1 = r1D[i72 + r1D.size(0) * i71];
            r[i72 + r.size(0) * i71] = std::pow(c_varargin_1, 2.0);
        }
    }
    coder::sum(r, r1Dnorm);
    ...
    for (int i75{0}; i75 < rb_loop_ub; i75++) {
        b_r1Dnorm[i75] = (r1Dnorm[i75] >= 0.0);
    }
    r_ocn_assert(b_r1Dnorm);
    // 'mysqrt:11' y = sqrt(x);
    i76 = r1Dnorm.size(1);
    for (int f_k{0}; f_k < i76; f_k++) {
    ...
    if ((jb_loop_ub == ib_loop_ub) && (r2.size(1) == i81)) {
        int vb_loop_ub;
        spline->sp.Lk.set_size(1, r2.size(1));
        vb_loop_ub = r2.size(1);
        for (int i84{0}; i84 < vb_loop_ub; i84++) {
            spline->sp.Lk[i84] =
                r2[i84] *
                (spline->sp.knots[((i55 + i58) + i84) + 1] - spline->sp.knots[i55 + i84]) / 2.0;
        }
    } else {
