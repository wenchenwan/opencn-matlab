                                   ...
                                   double u1, double u2)
{
    ...
    if (u2 < expl_temp.sp.knots[i5]) {
        ...
        for (int i6{0}; i6 < 5; i6++) {
            double d;
            d = ctx_cfg_GaussLegendreX[i6];
            b_u1[i6] = (u1 * (1.0 - d) + u2 * (d + 1.0)) / 2.0;
        }
        ...
        for (int i7{0}; i7 < 5; i7++) {
            double d1;
            d1 = ctx_cfg_GaussLegendreX[i7];
            b_u1[i7] = (u1 * (1.0 - d1) + expl_temp_tmp * (d1 + 1.0)) / 2.0;
        }
        ...
        for (int e_k{0}; e_k < 5; e_k++) {
            double d3;
            d3 = std::sqrt(r1Dnorm[e_k]);
            r1Dnorm[e_k] = d3;
            c_r1Dnorm += d3 * ctx_cfg_GaussLegendreW[e_k];
        }
        ...
        for (int i8{0}; i8 < loop_ub; i8++) {
            b_expl_temp.sp.CoeffX[i8] = d_expl_temp.sp.CoeffX[i8];
        }
        ...
        for (int i9{0}; i9 < b_loop_ub; i9++) {
            b_expl_temp.sp.CoeffY[i9] = d_expl_temp.sp.CoeffY[i9];
        }
        ...
        for (int i10{0}; i10 < c_loop_ub; i10++) {
            b_expl_temp.sp.CoeffZ[i10] = d_expl_temp.sp.CoeffZ[i10];
        }
        //  get Gauss-Legendre knots and weights
        // 'SplineLengthApprox_Interval1:9' GL_X   = ctx.cfg.GaussLegendreX;
        // 'SplineLengthApprox_Interval1:10' GL_W   = ctx.cfg.GaussLegendreW;
        ...
        for (int i11{0}; i11 < 5; i11++) {
            double d4;
            d4 = ctx_cfg_GaussLegendreX[i11];
            e_expl_temp[i11] = (c_expl_temp * (1.0 - d4) + u2 * (d4 + 1.0)) / 2.0;
        }
        EvalBSplineNoCtx(b_expl_temp.sp.CoeffX, b_expl_temp.sp.CoeffY, b_expl_temp.sp.CoeffZ,
        ...
        for (int g_k{0}; g_k < 5; g_k++) {
            double d5;
            d5 = std::sqrt(r1Dnorm[g_k]);
            r1Dnorm[g_k] = d5;
            d_r1Dnorm += d5 * ctx_cfg_GaussLegendreW[g_k];
        }
