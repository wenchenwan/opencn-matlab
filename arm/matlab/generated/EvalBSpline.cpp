                 ...
                 ::coder::array<double, 2U> &r3D)
{
    ...
    for (int k{0}; k < i4; k++) {
        ...
        r0Dx[k] = uvec[k];
        bspline_eval(expl_temp.sp.Bl.handle, expl_temp.sp.CoeffX, &r0Dx[k], &d, &d1, &d2);
        r3Dx[k] = d2;
        r2Dx[k] = d1;
        r1Dx[k] = d;
    ...
    for (int b_k{0}; b_k < i9; b_k++) {
        // 'bspline_eval_vec:9' [xk, xdk, xddk, xdddk] = bspline_eval(Bl, coeffs, u(k));
        r0Dy[b_k] = uvec[b_k];
        bspline_eval(expl_temp.sp.Bl.handle, expl_temp.sp.CoeffY, &r0Dy[b_k], &d3, &d4, &d5);
        r3Dy[b_k] = d5;
        r2Dy[b_k] = d4;
        r1Dy[b_k] = d3;
        // 'bspline_eval_vec:10' x(k) = xk;
    ...
    for (int c_k{0}; c_k < i14; c_k++) {
        // 'bspline_eval_vec:9' [xk, xdk, xddk, xdddk] = bspline_eval(Bl, coeffs, u(k));
        r0Dz[c_k] = uvec[c_k];
        bspline_eval(expl_temp.sp.Bl.handle, expl_temp.sp.CoeffZ, &r0Dz[c_k], &d6, &d7, &d8);
        r3Dz[c_k] = d8;
        r2Dz[c_k] = d7;
        r1Dz[c_k] = d6;
        // 'bspline_eval_vec:10' x(k) = xk;
