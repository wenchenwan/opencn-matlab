                      ...
                      const ::coder::array<double, 2U> &b_b)
{
    ...
    for (int i2{0}; i2 < loop_ub; i2++) {
        ...
        for (int i4{0}; i4 < c_loop_ub; i4++) {
            c_b[i4 + c_b.size(0) * i2] = b[i4 * stride_0_0 + b.size(0) * aux_0_1] *
                                             r_0D[i4 * stride_1_0 + r_0D.size(0) * aux_1_1] +
    ...
    r_0D.set_size(c_b.size(0), c_b.size(1));
    b_loop_ub = c_b.size(1);
    for (int i3{0}; i3 < b_loop_ub; i3++) {
        int d_loop_ub;
        d_loop_ub = c_b.size(0);
        for (int i5{0}; i5 < d_loop_ub; i5++) {
            r_0D[i5 + r_0D.size(0) * i3] = c_b[i5 + c_b.size(0) * i3];
        }
    }
}

//
// Arguments    : ::coder::array<double, 1U> &r_0D
//                double u_vec
//                const int outsize[2]
                      ...
                      const ::coder::array<double, 1U> &r)
{
    ...
    for (int i1{0}; i1 < loop_ub; i1++) {
        b_u_vec[i1] = u_vec * r_0D[i1 * stride_1_0] + r[i1 * stride_2_0];
    }
    r_0D.set_size(b_u_vec.size(0));
