                ...
                if (!ctx->zero_end) {
                    ...
                    if ((window[0].Info.zspdmode == ZSpdMode_ZN) ||
                        ...
                        *opt_struct = window[0];
                        ...
                        // 'mysqrt:3' y = sqrt(x);
                        r4.set_size(r1D.size(0));
                        c_loop_ub = r1D.size(0);
                        for (int i3{0}; i3 < c_loop_ub; i3++) {
                            double varargin_1;
                            varargin_1 = r1D[i3] * ud;
                            r4[i3] = std::pow(varargin_1, 2.0);
                        }
                        ctx->v_0 = std::sqrt(coder::combineVectorElements(r4));
                        ...
                        for (int i4{0}; i4 < d_loop_ub; i4++) {
                            double b_varargin_1;
                            b_varargin_1 = r1D[i4];
                            r4[i4] = std::pow(b_varargin_1, 2.0);
                        }
                        n = std::sqrt(coder::combineVectorElements(r4));
                        // 'mysqrt:4' sqrt_calls = sqrt_calls + 1;
                        sqrt_calls++;
                        ...
                        if ((r2D.size(0) == r1D.size(0)) && (i6 == r1D.size(0))) {
                            int f_loop_ub;
                            r4.set_size(r2D.size(0));
                            f_loop_ub = r2D.size(0);
                            for (int i7{0}; i7 < f_loop_ub; i7++) {
                                double d_varargin_1;
                                d_varargin_1 = (r2D[i7] * b_y + r1D[i7] * udd) * (r1D[i7] / n);
                                r4[i7] = std::pow(d_varargin_1, 2.0);
                            }
                        } else {
                            binary_expand_op(r4, r2D, b_y, r1D, udd, n);
                        }
                        ctx->at_0 = std::sqrt(coder::combineVectorElements(r4));
                        // 'mysqrt:4' sqrt_calls = sqrt_calls + 1;
                        sqrt_calls++;
                        ...
                        if ((window[static_cast<int>(ind) - 1].Info.zspdmode == ZSpdMode_NZ) ||
                            ...
                            for (int i5{0}; i5 < e_loop_ub; i5++) {
                                double c_varargin_1;
                                c_varargin_1 = r1D[i5];
                                r4[i5] = std::pow(c_varargin_1, 2.0);
                            }
                            b_n = std::sqrt(coder::combineVectorElements(r4));
                            // 'mysqrt:4' sqrt_calls = sqrt_calls + 1;
                            sqrt_calls++;
                            // 'calcNormVNormAT:14' atNorm  = MyNorm( A.*t );
                            // 'MyNorm:2' coder.inline('always');
                            ...
                            for (int i8{0}; i8 < g_loop_ub; i8++) {
                                ...
                                r4[i8] = std::pow(e_varargin_1, 2.0);
                            }
                            ctx->v_1 = -std::sqrt(coder::combineVectorElements(r4));
                            ...
                            if ((r2D.size(0) == r1D.size(0)) && (i9 == r1D.size(0))) {
                                ...
                                for (int i10{0}; i10 < h_loop_ub; i10++) {
                                    ...
                                    r4[i10] = std::pow(f_varargin_1, 2.0);
                                }
                            } else {
                                binary_expand_op(r4, r2D, c_y, r1D, b_udd, b_n);
                            }
                            ctx->at_1 = -std::sqrt(coder::combineVectorElements(r4));
