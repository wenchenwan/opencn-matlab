                          ...
                          CurvStruct *curv, CurvStruct *spline)
{
    ...
    if (i5 != 0) {
        ...
        for (int k{0}; k <= i8; k++) {
            ...
            for (int b_k{0}; b_k < 5; b_k++) {
                c[b_k + 5 * k] = spline->sp.knots[(i2 + varargin_2) + 1] * b[b_k];
            }
        }
    }
    ...
    if (csz_idx_1_tmp != 0) {
        ...
        for (int c_k{0}; c_k <= i9; c_k++) {
            ...
            for (int d_k{0}; d_k < 5; d_k++) {
                b_c[d_k + 5 * c_k] = spline->sp.knots[(i2 + i11) + 1] * b[d_k];
            }
        }
    }
    ...
    for (int i13{0}; i13 < c_loop_ub; i13++) {
        int d_loop_ub;
    ...
    for (int i19{0}; i19 < i_loop_ub; i19++) {
        ...
        for (int i20{0}; i20 < k_loop_ub; i20++) {
            double varargin_1;
            varargin_1 = r1D[i20 + r1D.size(0) * i19];
            r[i20 + r.size(0) * i19] = std::pow(varargin_1, 2.0);
        }
    }
    coder::sum(r, r1Dnorm);
    // 'mysqrt:3' y = sqrt(x);
    ...
    if ((csz_idx_1_tmp == i5) && (y.size(1) == i28)) {
        int m_loop_ub;
        ...
        for (int i30{0}; i30 < m_loop_ub; i30++) {
            spline->sp.Lk[i30] =
                y[i30] *
                (spline->sp.knots[((i2 + i6) + i30) + 2] - spline->sp.knots[(i2 + i30) + 1]) / 2.0;
        }
    } else {
    ...
    for (int i32{0}; i32 < 6; i32++) {
        dv1[i32] = (*(double(*)[6]) & batch_pvec[6 * (c_batch_pvec - 1)])[i32];
    }
            ...
            if (curv.Info.Type != CurveType_Line) {
                ...
                if ((coder::b_norm(V0) < 2.2204460492503131E-16) ||
                    ...
                    for (int i7{0}; i7 < b_loop_ub; i7++) {
                        double b_varargin_1;
                        b_varargin_1 = V1[i7];
                        r5[i7] = std::pow(b_varargin_1, 2.0);
                    }
