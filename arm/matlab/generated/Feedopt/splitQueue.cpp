        ...
        for (int k{0}; k < i; k++) {
            ...
            if (curv.Info.Type == CurveType_TransP5) {
                ...
                if ((curv.Info.zspdmode == ZSpdMode_NZ) || (curv.Info.zspdmode == ZSpdMode_ZZ)) {
                    ...
                    r7.set_size(b_r1D.size(0));
                    if (b_r1D.size(0) != 0) {
                        int i7;
                        i7 = b_r1D.size(0);
                        for (int c_k{0}; c_k < i7; c_k++) {
                            r7[c_k] = std::abs(b_r1D[c_k]);
                        }
                    }
                    jps =
                        coder::internal::minimum(ctx_data, ctx_size) / coder::internal::maximum(r7);
                    //  searchU   = true;
                    // while searchU
                    ...
                    while (searchJps) {
                        ...
                        r8.set_size(1, y.size(1));
                        f_loop_ub = y.size(1);
                        for (int i11{0}; i11 < f_loop_ub; i11++) {
                            double varargin_1;
                            varargin_1 = y[i11];
                            r8[i11] = std::pow(varargin_1, 2.0);
                        }
                        ud.set_size(1, r8.size(1));
                        g_loop_ub = r8.size(1);
                        for (int i12{0}; i12 < g_loop_ub; i12++) {
                            ud[i12] = jps * r8[i12] / 2.0;
                        }
                        ...
                        for (int i13{0}; i13 < h_loop_ub; i13++) {
                            double b_varargin_1;
                            b_varargin_1 = y[i13];
                            r8[i13] = std::pow(b_varargin_1, 3.0);
                        }
                        b_u.set_size(1, r8.size(1));
                        i_loop_ub = r8.size(1);
                        for (int i14{0}; i14 < i_loop_ub; i14++) {
                            b_u[i14] = jps * r8[i14] / 6.0;
                        }
                        ...
                        r8.set_size(1, ud.size(1));
                        s_loop_ub = ud.size(1);
                        for (int i24{0}; i24 < s_loop_ub; i24++) {
                            double c_varargin_1;
                            c_varargin_1 = ud[i24];
                            r8[i24] = std::pow(c_varargin_1, 2.0);
                        }
                        if (r2D.size(1) == 1) {
                            i26 = r8.size(1);
                        } else {
                        ...
                        if ((r2D.size(1) == r8.size(1)) && (c_r1D.size(1) == udd_vec.size(1)) &&
                            ...
                            for (int i28{0}; i28 < u_loop_ub; i28++) {
                                ...
                                for (int i30{0}; i30 < w_loop_ub; i30++) {
                                    A[i30 + A.size(0) * i28] =
                                        r2D[i30 + r2D.size(0) * i28] * r8[i28] +
                                        c_r1D[i30 + c_r1D.size(0) * i28] * udd_vec[i28];
                                }
                            }
                        } else {
                            binary_expand_op(A, r2D, r8, c_r1D, udd_vec);
                        }
                        ...
                        for (int i29{0}; i29 < v_loop_ub; i29++) {
                            double d_varargin_1;
                            d_varargin_1 = ud[i29];
                            r8[i29] = std::pow(d_varargin_1, 3.0);
                        }
                        if (r2D.size(1) == 1) {
                            i31 = ud.size(1);
                        } else {
                        ...
                        if ((r3D.size(1) == r8.size(1)) && (r2D.size(1) == ud.size(1)) &&
                            ...
                            binary_expand_op(c_J, r3D, r8, r2D, ud, udd_vec, c_r1D, jps, uv);
                        }
                        ...
                        coder::vecnorm(f_r1D, r8);
                        if (r8.size(1) == ud.size(1)) {
                            int eb_loop_ub;
                            feed.set_size(1, r8.size(1));
                            eb_loop_ub = r8.size(1);
                            for (int i49{0}; i49 < eb_loop_ub; i49++) {
                                feed[i49] = r8[i49] * ud[i49];
                            }
                        } else {
                            times(feed, r8, ud);
                        }
                        ...
                        if (b_y) {
                            ...
                            h_k = 0;
                            exitg1 = false;
                            while ((!exitg1) && (h_k <= r10.size(0) * r10.size(1) - 1)) {
                                if (r10[h_k]) {
                                    c_y = true;
                                    exitg1 = true;
                                } else {
                            ...
                            if (c_y) {
                                ...
                                if (r9.size(0) == b_trueCount) {
                                    int jb_loop_ub;
                                    int lb_loop_ub;
                                    r11.set_size(r9.size(0), r9.size(1));
                                    jb_loop_ub = r9.size(1);
                                    for (int i58{0}; i58 < jb_loop_ub; i58++) {
                                        int kb_loop_ub;
                                        kb_loop_ub = r9.size(0);
                                        for (int i59{0}; i59 < kb_loop_ub; i59++) {
                                            r11[i59 + r11.size(0) * i58] =
                                                r9[i59 + r9.size(0) * i58] - vmax_data[i59];
                                        }
                                    }
                                    r9.set_size(r11.size(0), r11.size(1));
                                    lb_loop_ub = r11.size(1);
                                    for (int i60{0}; i60 < lb_loop_ub; i60++) {
                                        int nb_loop_ub;
                                        nb_loop_ub = r11.size(0);
                                        for (int i62{0}; i62 < nb_loop_ub; i62++) {
                                            r9[i62 + r9.size(0) * i60] =
                                                r11[i62 + r11.size(0) * i60];
                                        }
                                    }
                                    coder::internal::b_maximum(r9, &v_delta, &b_iindx);
                                } else {
                                    binary_expand_op(r9, vmax_data, &b_trueCount, &v_delta,
                                                     &b_iindx);
                                }
                                ...
                                if ((A.size(0) != 0) && (A.size(1) != 0)) {
                                    ...
                                    for (int i_k{0}; i_k < i56; i_k++) {
                                        ...
                                        for (int j_k{0}; j_k < i57; j_k++) {
                                            r9[j_k + r9.size(0) * i_k] =
                                                std::abs(A[j_k + A.size(0) * i_k]);
                                        }
                                    }
                                }
                                if (c_trueCount == 1) {
                                    f_trueCount = r9.size(0);
                                } else {
                                    f_trueCount = c_trueCount;
                                }
                                r10.set_size(f_trueCount, r9.size(1));
                                b_stride_0_0 = (r9.size(0) != 1);
                                b_stride_1_0 = (c_trueCount != 1);
                                mb_loop_ub = r9.size(1);
                                for (int i61{0}; i61 < mb_loop_ub; i61++) {
                                    int ob_loop_ub;
                                    if (c_trueCount == 1) {
                                        ob_loop_ub = r9.size(0);
                                    } else {
                                        ob_loop_ub = c_trueCount;
                                    }
                                    for (int i63{0}; i63 < ob_loop_ub; i63++) {
                                        r10[i63 + r10.size(0) * i61] =
                                            (r9[i63 * b_stride_0_0 + r9.size(0) * i61] >
                                ...
                                k_k = 0;
                                exitg1 = false;
                                while ((!exitg1) && (k_k <= r10.size(0) * r10.size(1) - 1)) {
                                    if (r10[k_k]) {
                                        d_y = true;
                                        exitg1 = true;
                                    } else {
                                ...
                                if (d_y) {
                                    ...
                                    if (r9.size(0) == c_trueCount) {
                                        int pb_loop_ub;
                                        int rb_loop_ub;
                                        r12.set_size(r9.size(0), r9.size(1));
                                        pb_loop_ub = r9.size(1);
                                        for (int i66{0}; i66 < pb_loop_ub; i66++) {
                                            int qb_loop_ub;
                                            qb_loop_ub = r9.size(0);
                                            for (int i67{0}; i67 < qb_loop_ub; i67++) {
                                                r12[i67 + r12.size(0) * i66] =
                                                    r9[i67 + r9.size(0) * i66] - amax_data[i67];
                                            }
                                        }
                                        r9.set_size(r12.size(0), r12.size(1));
                                        rb_loop_ub = r12.size(1);
                                        for (int i68{0}; i68 < rb_loop_ub; i68++) {
                                            int tb_loop_ub;
                                            tb_loop_ub = r12.size(0);
                                            for (int i70{0}; i70 < tb_loop_ub; i70++) {
                                                r9[i70 + r9.size(0) * i68] =
                                                    r12[i70 + r12.size(0) * i68];
                                            }
                                        }
                                        coder::internal::b_maximum(r9, &a_delta, &c_iindx);
                                    } else {
                                        binary_expand_op(r9, amax_data, &c_trueCount, &a_delta,
                                                         &c_iindx);
                                    }
                                    ...
                                    int sb_loop_ub;
                                    bool e_y;
                                    r9.set_size(c_J.size(0), c_J.size(1));
                                    ...
                                    r10.set_size(g_trueCount, r9.size(1));
                                    c_stride_0_0 = (r9.size(0) != 1);
                                    c_stride_1_0 = (d_trueCount != 1);
                                    sb_loop_ub = r9.size(1);
                                    for (int i69{0}; i69 < sb_loop_ub; i69++) {
                                        int ub_loop_ub;
                                        if (d_trueCount == 1) {
                                            ub_loop_ub = r9.size(0);
                                        } else {
                                            ub_loop_ub = d_trueCount;
                                        }
                                        for (int i71{0}; i71 < ub_loop_ub; i71++) {
                                            r10[i71 + r10.size(0) * i69] =
                                                (r9[i71 * c_stride_0_0 + r9.size(0) * i69] >
                                    ...
                                    n_k = 0;
                                    exitg1 = false;
                                    while ((!exitg1) && (n_k <= r10.size(0) * r10.size(1) - 1)) {
                                        if (r10[n_k]) {
                                            e_y = true;
                                            exitg1 = true;
                                        } else {
                                    ...
                                    if (e_y) {
                                        ...
                                        // jmax, [], 'all' );
                                        if (r9.size(0) == d_trueCount) {
                                            int vb_loop_ub;
                                            int xb_loop_ub;
                                            r13.set_size(r9.size(0), r9.size(1));
                                            vb_loop_ub = r9.size(1);
                                            for (int i72{0}; i72 < vb_loop_ub; i72++) {
                                                int wb_loop_ub;
                                                wb_loop_ub = r9.size(0);
                                                for (int i73{0}; i73 < wb_loop_ub; i73++) {
                                                    r13[i73 + r13.size(0) * i72] =
                                                        r9[i73 + r9.size(0) * i72] - jmax_data[i73];
                                                }
                                            }
                                            r9.set_size(r13.size(0), r13.size(1));
                                            xb_loop_ub = r13.size(1);
                                            for (int i74{0}; i74 < xb_loop_ub; i74++) {
                                                int yb_loop_ub;
                                                yb_loop_ub = r13.size(0);
                                                for (int i75{0}; i75 < yb_loop_ub; i75++) {
                                                    r9[i75 + r9.size(0) * i74] =
                                                        r13[i75 + r13.size(0) * i74];
                                                }
                                            }
                                            coder::internal::b_maximum(r9, &j_delta, &d_iindx);
                                        } else {
                                            binary_expand_op(r9, jmax_data, &d_trueCount, &j_delta,
                                                             &d_iindx);
                                        }
