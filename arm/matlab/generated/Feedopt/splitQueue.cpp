            ...
            if (curv.Info.Type == CurveType_TransP5) {
                ...
                if ((curv.Info.zspdmode == ZSpdMode_ZN) || (curv.Info.zspdmode == ZSpdMode_ZZ)) {
                    ...
                    while (searchJps) {
                        ...
                        r7.set_size(1, y.size(1));
                        c_loop_ub = y.size(1);
                        for (int i6{0}; i6 < c_loop_ub; i6++) {
                            double varargin_1;
                            varargin_1 = y[i6];
                            r7[i6] = std::pow(varargin_1, 3.0);
                        }
                        b_u.set_size(1, r7.size(1));
                        d_loop_ub = r7.size(1);
                        for (int i7{0}; i7 < d_loop_ub; i7++) {
                            b_u[i7] = jps * r7[i7] / 6.0;
                        }
                        ...
                        ud_vec.set_size(1, r7.size(1));
                        f_loop_ub = r7.size(1);
                        for (int i9{0}; i9 < f_loop_ub; i9++) {
                            ud_vec[i9] = jps * r7[i9] / 2.0;
                        }
                        ...
                        if ((r2D.size(1) == r7.size(1)) && (b_r1D.size(1) == udd_vec.size(1)) &&
                            ...
                            for (int i16{0}; i16 < k_loop_ub; i16++) {
                                ...
                                for (int i18{0}; i18 < m_loop_ub; i18++) {
                                    A[i18 + A.size(0) * i16] =
                                        r2D[i18 + r2D.size(0) * i16] * r7[i16] +
                                        b_r1D[i18 + b_r1D.size(0) * i16] * udd_vec[i16];
                                }
                            }
                        } else {
                            binary_expand_op(A, r2D, r7, b_r1D, udd_vec);
                        }
                        ...
                        for (int i17{0}; i17 < l_loop_ub; i17++) {
                            ...
                            r7[i17] = std::pow(d_varargin_1, 3.0);
                        }
                        if (r2D.size(1) == 1) {
                            i19 = ud_vec.size(1);
                        } else {
                        ...
                        if ((r3D.size(1) == r7.size(1)) && (r2D.size(1) == ud_vec.size(1)) &&
                            ...
                            binary_expand_op(J, r3D, r7, r2D, ud_vec, udd_vec, b_r1D, jps, uv);
                        }
                        ...
                        coder::vecnorm(V, r7);
                        x.set_size(1, r7.size(1));
                        r_loop_ub = r7.size(1);
                        for (int i32{0}; i32 < r_loop_ub; i32++) {
                            x[i32] = (r7[i32] > curvS.Info.FeedRate);
                        }
                        b_y = false;
                        g_k = 0;
                        exitg1 = false;
                        ...
                        if (b_y) {
                            ...
                            for (int i34{0}; i34 < s_loop_ub; i34++) {
                                r7[i34] = r7[i34] - curvS.Info.FeedRate;
                            }
                            coder::internal::maximum(r7, &v_delta, &iindx);
                            ...
                            if ((A.size(0) != 0) && (A.size(1) != 0)) {
                                ...
                                for (int h_k{0}; h_k < i33; h_k++) {
                                    ...
                                    for (int i_k{0}; i_k < i35; i_k++) {
                                        r8[i_k + r8.size(0) * h_k] =
                                            std::abs(A[i_k + A.size(0) * h_k]);
                                    }
                                }
                            }
                            if (r8.size(0) == ctx->cfg.indTot.size[0]) {
                                int t_loop_ub;
                                ...
                                for (int i36{0}; i36 < t_loop_ub; i36++) {
                                    ...
                                    for (int i37{0}; i37 < v_loop_ub; i37++) {
                                        b_x[i37 + b_x.size(0) * i36] =
                                            (r8[i37 + r8.size(0) * i36] > amax_data[i37]);
                                    }
                                }
                            } else {
                                e_binary_expand_op(b_x, r8, amax_data, ctx);
                            }
                            c_y = false;
                            j_k = 0;
                            exitg1 = false;
                            ...
                            if (c_y) {
                                ...
                                if (r8.size(0) == ctx->cfg.indTot.size[0]) {
                                    int bb_loop_ub;
                                    int w_loop_ub;
                                    r9.set_size(r8.size(0), r8.size(1));
                                    w_loop_ub = r8.size(1);
                                    for (int i41{0}; i41 < w_loop_ub; i41++) {
                                        int y_loop_ub;
                                        y_loop_ub = r8.size(0);
                                        for (int i43{0}; i43 < y_loop_ub; i43++) {
                                            r9[i43 + r9.size(0) * i41] =
                                                r8[i43 + r8.size(0) * i41] - amax_data[i43];
                                        }
                                    }
                                    r8.set_size(r9.size(0), r9.size(1));
                                    bb_loop_ub = r9.size(1);
                                    for (int i45{0}; i45 < bb_loop_ub; i45++) {
                                        int cb_loop_ub;
                                        cb_loop_ub = r9.size(0);
                                        for (int i46{0}; i46 < cb_loop_ub; i46++) {
                                            r8[i46 + r8.size(0) * i45] = r9[i46 + r9.size(0) * i45];
                                        }
                                    }
                                    coder::internal::b_maximum(r8, &a_delta, &b_iindx);
                                } else {
                                    binary_expand_op(r8, amax_data, ctx, &a_delta, &b_iindx);
                                }
                                    ...
                                    for (int i42{0}; i42 < x_loop_ub; i42++) {
                                        ...
                                        for (int i44{0}; i44 < ab_loop_ub; i44++) {
                                            b_x[i44 + b_x.size(0) * i42] =
                                                (r8[i44 + r8.size(0) * i42] > jmax_data[i44]);
                                        }
                                    }
                                } else {
                                    e_binary_expand_op(b_x, r8, jmax_data, ctx);
                                }
                                d_y = false;
                                m_k = 0;
                                exitg1 = false;
                                    ...
                                    if (r8.size(0) == ctx->cfg.indTot.size[0]) {
                                        ...
                                        for (int i47{0}; i47 < db_loop_ub; i47++) {
                                            ...
                                            for (int i48{0}; i48 < eb_loop_ub; i48++) {
                                                r10[i48 + r10.size(0) * i47] =
                                                    r8[i48 + r8.size(0) * i47] - jmax_data[i48];
                                            }
                                        }
                                        r8.set_size(r10.size(0), r10.size(1));
                                        fb_loop_ub = r10.size(1);
                                        for (int i49{0}; i49 < fb_loop_ub; i49++) {
                                            int gb_loop_ub;
                                            gb_loop_ub = r10.size(0);
                                            for (int i50{0}; i50 < gb_loop_ub; i50++) {
                                                r8[i50 + r8.size(0) * i49] =
                                                    r10[i50 + r10.size(0) * i49];
                                            }
                                        }
                                        coder::internal::b_maximum(r8, &j_delta, &c_iindx);
                                    } else {
                                        binary_expand_op(r8, jmax_data, ctx, &j_delta, &c_iindx);
                                    }
