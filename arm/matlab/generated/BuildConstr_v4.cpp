                    ...
                    ::coder::array<double, 1U> &beq)
{
    ...
    for (int i8{0}; i8 < c_loop_ub; i8++) {
        ...
        r4[3 * i8] = std::pow(b_varargin_1, 2.0);
        c_varargin_1 = r1D[3 * i8 + 1];
        r4[3 * i8 + 1] = std::pow(c_varargin_1, 2.0);
        d_varargin_1 = r1D[3 * i8 + 2];
        r4[3 * i8 + 2] = std::pow(d_varargin_1, 2.0);
    }
    ...
    coder::bsxfun(b_r2D, BasisVal, r5);
    e_loop_ub = r1D.size(1);
    b_r1D.set_size(r1D.size(1));
    for (int i10{0}; i10 < e_loop_ub; i10++) {
        b_r1D[i10] = r1D[3 * i10];
    }
    coder::bsxfun(b_r1D, BasisValD, r6);
    if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
        int f_loop_ub;
        ...
        for (int i11{0}; i11 < f_loop_ub; i11++) {
            ...
            for (int i13{0}; i13 < h_loop_ub; i13++) {
                R1[i13 + R1.size(0) * i11] =
                    r5[i13 + r5.size(0) * i11] + 0.5 * r6[i13 + r6.size(0) * i11];
            }
        }
    } else {
        d_binary_expand_op(R1, r5, r6);
    }
        ...
        for (int i15{0}; i15 < j_loop_ub; i15++) {
            ...
            for (int i17{0}; i17 < l_loop_ub; i17++) {
                R2[i17 + R2.size(0) * i15] =
                    r5[i17 + r5.size(0) * i15] + 0.5 * r6[i17 + r6.size(0) * i15];
            }
        }
    } else {
        d_binary_expand_op(R2, r5, r6);
    }
        ...
        for (int i19{0}; i19 < n_loop_ub; i19++) {
            ...
            for (int i20{0}; i20 < o_loop_ub; i20++) {
                R3[i20 + R3.size(0) * i19] =
                    r5[i20 + r5.size(0) * i19] + 0.5 * r6[i20 + r6.size(0) * i19];
            }
        }
    } else {
        d_binary_expand_op(R3, r5, r6);
    }
    //
    ...
    for (int i21{0}; i21 < p_loop_ub; i21++) {
        ...
        for (int i22{0}; i22 < q_loop_ub; i22++) {
            varargin_3[i22 + varargin_3.size(0) * i21] = -R1[i22 + R1.size(0) * i21];
        }
    }
    ...
    for (int i23{0}; i23 < r_loop_ub; i23++) {
        ...
        for (int i24{0}; i24 < s_loop_ub; i24++) {
            varargin_5[i24 + varargin_5.size(0) * i23] = -R2[i24 + R2.size(0) * i23];
        }
    }
    ...
    for (int i25{0}; i25 < t_loop_ub; i25++) {
        ...
        for (int i26{0}; i26 < u_loop_ub; i26++) {
            varargin_7[i26 + varargin_7.size(0) * i25] = -R3[i26 + R3.size(0) * i25];
        }
    }
    ...
    if (BasisVal.size(1) == BasisValD.size(1)) {
        ...
        coder::internal::blas::mtimes(c_r1D, g_a, r7);
    } else {
        b_binary_expand_op(r7, r1D, a, BasisVal, b_a, BasisValD, c_a, d_a, e_a, f_a);
    }
    ib_loop_ub = BasisVal.size(1);
    for (int i55{0}; i55 < ib_loop_ub; i55++) {
        Aeq[Aeq.size(0) * i55] = BasisVal[BasisVal.size(0) * i55];
    }
    kb_loop_ub = r7.size(1);
    for (int i56{0}; i56 < kb_loop_ub; i56++) {
        Aeq[Aeq.size(0) * i56 + 1] = r7[i56];
    }
    ...
    for (int b_k{0}; b_k <= i60 - 2; b_k++) {
        ...
        for (int i67{0}; i67 < xb_loop_ub; i67++) {
            ...
            r4[3 * i67] = std::pow(e_varargin_1, 2.0);
            f_varargin_1 = r1Dn[3 * i67 + 1];
            r4[3 * i67 + 1] = std::pow(f_varargin_1, 2.0);
            g_varargin_1 = r1Dn[3 * i67 + 2];
            r4[3 * i67 + 2] = std::pow(g_varargin_1, 2.0);
        }
        double d3;
        double x_tmp;
        int e_r1D;
        ...
        if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
            int jc_loop_ub;
            R2.set_size(r5.size(0), r5.size(1));
            jc_loop_ub = r5.size(1);
            for (int i80{0}; i80 < jc_loop_ub; i80++) {
                int lc_loop_ub;
                lc_loop_ub = r5.size(0);
                for (int i82{0}; i82 < lc_loop_ub; i82++) {
                    R2[i82 + R2.size(0) * i80] =
                        r5[i82 + r5.size(0) * i80] + 0.5 * r6[i82 + r6.size(0) * i80];
                }
            }
        } else {
            d_binary_expand_op(R2, r5, r6);
        ...
        b_r1Dn.set_size(r1Dn.size(1));
        for (int i84{0}; i84 < mc_loop_ub; i84++) {
            b_r1Dn[i84] = r1Dn[3 * i84 + 2];
        }
        coder::bsxfun(b_r1Dn, BasisValD, r6);
        if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
            int pc_loop_ub;
            R3.set_size(r5.size(0), r5.size(1));
            pc_loop_ub = r5.size(1);
            for (int i89{0}; i89 < pc_loop_ub; i89++) {
                int qc_loop_ub;
                qc_loop_ub = r5.size(0);
                for (int i93{0}; i93 < qc_loop_ub; i93++) {
                    R3[i93 + R3.size(0) * i89] =
                        r5[i93 + r5.size(0) * i89] + 0.5 * r6[i93 + r6.size(0) * i89];
                }
            }
        } else {
            d_binary_expand_op(R3, r5, r6);
        ...
        for (int i95{0}; i95 < sc_loop_ub; i95++) {
            ...
            for (int i96{0}; i96 < tc_loop_ub; i96++) {
                varargin_3[i96 + varargin_3.size(0) * i95] = -R1[i96 + R1.size(0) * i95];
            }
        }
        varargin_5.set_size(R2.size(0), R2.size(1));
        uc_loop_ub = R2.size(1);
        for (int i97{0}; i97 < uc_loop_ub; i97++) {
            int vc_loop_ub;
            vc_loop_ub = R2.size(0);
            for (int i98{0}; i98 < vc_loop_ub; i98++) {
                varargin_5[i98 + varargin_5.size(0) * i97] = -R2[i98 + R2.size(0) * i97];
            }
        }
        varargin_7.set_size(R3.size(0), R3.size(1));
        wc_loop_ub = R3.size(1);
        for (int i99{0}; i99 < wc_loop_ub; i99++) {
            int xc_loop_ub;
            xc_loop_ub = R3.size(0);
            for (int i100{0}; i100 < xc_loop_ub; i100++) {
                varargin_7[i100 + varargin_7.size(0) * i99] = -R3[i100 + R3.size(0) * i99];
            }
        }
        if ((BasisVal.size(0) != 0) && (BasisVal.size(1) != 0)) {
        ...
        if (BasisVal.size(1) == BasisValD.size(1)) {
            ...
            coder::internal::blas::mtimes(t_1, g_a, r9);
            fd_loop_ub = r9.size(1);
            for (int i131{0}; i131 < fd_loop_ub; i131++) {
                Aeq[(static_cast<int>(u) + Aeq.size(0) * (i129 + i131)) + 1] = r9[i131];
            }
        } else {
        ...
        if (BasisVal.size(1) == BasisValD.size(1)) {
            ...
            coder::internal::blas::mtimes(t_1, g_a, r7);
        } else {
            binary_expand_op(r7, t_1, w_a, BasisVal, x_a, BasisValD, y_a, ab_a, bb_a, cb_a);
        }
        hd_loop_ub = r7.size(1);
        for (int i134{0}; i134 < hd_loop_ub; i134++) {
            Aeq[(static_cast<int>(u) + Aeq.size(0) * ((i132 + i134) - 1)) + 1] = -r7[i134];
        }
        //
    ...
    if (BasisVal.size(1) == BasisValD.size(1)) {
        ...
        binary_expand_op(r7, r1D, h_a, BasisVal, i_a, BasisValD, j_a, k_a, l_a, m_a);
    }
    ...
    vb_loop_ub = r7.size(1);
    for (int i66{0}; i66 < vb_loop_ub; i66++) {
        Aeq[unnamed_idx_1 + Aeq.size(0) * (i64 + i66)] = r7[i66];
    }
    int b_beq;
    //
    ...
    coder::b_linspace(ctx_cfg_opt_VEL_RAMP_OVER_WINDOWS, static_cast<double>(M), r7);
    vel_ramp.set_size(r7.size(1));
    bc_loop_ub = r7.size(1);
    for (int i71{0}; i71 < bc_loop_ub; i71++) {
        vel_ramp[i71] = r7[i71];
    }
    // 'BuildConstr_v4:162' acc_ramp = repmat(linspace(1, ctx.cfg.opt.ACC_RAMP_OVER_WINDOWS,
    // M)',1,6);
    coder::b_linspace(ctx_cfg_opt_ACC_RAMP_OVER_WINDOWS, static_cast<double>(M), r7);
    r8.set_size(r7.size(1));
    dc_loop_ub = r7.size(1);
    for (int i73{0}; i73 < dc_loop_ub; i73++) {
        r8[i73] = r7[i73];
    }
    acc_ramp.set_size(r8.size(0), 6);
    if (r8.size(0) != 0) {
        int na;
        na = r8.size(0);
        for (int t{0}; t < 6; t++) {
            for (int d_k{0}; d_k < na; d_k++) {
                acc_ramp[d_k + acc_ramp.size(0) * t] = r8[d_k];
            }
        }
    }
    ...
    if (CurvStructs.size(1) > 1) {
        ...
        for (int i88{0}; i88 < oc_loop_ub; i88++) {
            for (int i91{0}; i91 < b_result; i91++) {
                ramp[i91 + ramp.size(0) * ((i88 + input_sizes_idx_1) + b_input_sizes_idx_1)] =
                    acc_ramp[i91 + acc_ramp.size(0) * i88];
            }
        }
        for (int i90{0}; i90 < d_input_sizes_idx_1; i90++) {
            for (int i92{0}; i92 < b_result; i92++) {
                ramp[i92 + ramp.size(0) * (((i90 + input_sizes_idx_1) + b_input_sizes_idx_1) +
                                           c_input_sizes_idx_1)] = varargin_4[i92 + b_result * i90];
            }
        }
        ...
        if (b.size(0) == ramp.size(0) * ramp.size(1)) {
            int rc_loop_ub;
            rc_loop_ub = b.size(0);
            for (int i94{0}; i94 < rc_loop_ub; i94++) {
