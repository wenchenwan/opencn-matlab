                    ...
                    ::coder::array<double, 1U> &beq)
{
    ...
    for (int i6{0}; i6 < b_loop_ub; i6++) {
        ...
        r4[3 * i6] = std::pow(b_varargin_1, 2.0);
        c_varargin_1 = r1D[3 * i6 + 1];
        r4[3 * i6 + 1] = std::pow(c_varargin_1, 2.0);
        d_varargin_1 = r1D[3 * i6 + 2];
        r4[3 * i6 + 2] = std::pow(d_varargin_1, 2.0);
    }
    ...
    if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
        ...
        for (int i9{0}; i9 < e_loop_ub; i9++) {
            ...
            for (int i11{0}; i11 < g_loop_ub; i11++) {
                R1[i11 + R1.size(0) * i9] =
                    r5[i11 + r5.size(0) * i9] + 0.5 * r6[i11 + r6.size(0) * i9];
            }
        }
    } else {
        ...
        for (int i13{0}; i13 < i_loop_ub; i13++) {
            ...
            for (int i15{0}; i15 < k_loop_ub; i15++) {
                R2[i15 + R2.size(0) * i13] =
                    r5[i15 + r5.size(0) * i13] + 0.5 * r6[i15 + r6.size(0) * i13];
            }
        }
    } else {
        ...
        for (int i17{0}; i17 < m_loop_ub; i17++) {
            ...
            for (int i18{0}; i18 < n_loop_ub; i18++) {
                R3[i18 + R3.size(0) * i17] =
                    r5[i18 + r5.size(0) * i17] + 0.5 * r6[i18 + r6.size(0) * i17];
            }
        }
    } else {
    ...
    jb_loop_ub = r7.size(1);
    for (int i54{0}; i54 < jb_loop_ub; i54++) {
        Aeq[Aeq.size(0) * i54 + 1] = r7[i54];
    }
    ...
    for (int b_k{0}; b_k <= i58 - 2; b_k++) {
        ...
        for (int i65{0}; i65 < ub_loop_ub; i65++) {
            ...
            r4[3 * i65] = std::pow(e_varargin_1, 2.0);
            f_varargin_1 = r1Dn[3 * i65 + 1];
            r4[3 * i65 + 1] = std::pow(f_varargin_1, 2.0);
            g_varargin_1 = r1Dn[3 * i65 + 2];
            r4[3 * i65 + 2] = std::pow(g_varargin_1, 2.0);
        }
        double d3;
        double x_tmp;
        int e_r1D;
        ...
        if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
            ...
            for (int i71{0}; i71 < cc_loop_ub; i71++) {
                int ec_loop_ub;
                ec_loop_ub = r5.size(0);
                for (int i73{0}; i73 < ec_loop_ub; i73++) {
                    R1[i73 + R1.size(0) * i71] =
                        r5[i73 + r5.size(0) * i71] + 0.5 * r6[i73 + r6.size(0) * i71];
                }
            }
        } else {
            d_binary_expand_op(R1, r5, r6);
        ...
        if ((r5.size(0) == r6.size(0)) && (r5.size(1) == r6.size(1))) {
            int gc_loop_ub;
            R2.set_size(r5.size(0), r5.size(1));
            gc_loop_ub = r5.size(1);
            for (int i75{0}; i75 < gc_loop_ub; i75++) {
                int ic_loop_ub;
                ic_loop_ub = r5.size(0);
                for (int i77{0}; i77 < ic_loop_ub; i77++) {
                    R2[i77 + R2.size(0) * i75] =
                        r5[i77 + r5.size(0) * i75] + 0.5 * r6[i77 + r6.size(0) * i75];
                }
            }
        } else {
            d_binary_expand_op(R2, r5, r6);
        ...
        if (BasisVal.size(1) == BasisValD.size(1)) {
            ...
            coder::internal::blas::mtimes(t_1, g_a, r8);
            yc_loop_ub = r8.size(1);
            for (int i117{0}; i117 < yc_loop_ub; i117++) {
                Aeq[(static_cast<int>(u) + Aeq.size(0) * (i115 + i117)) + 1] = r8[i117];
            }
        } else {
        ...
        bd_loop_ub = r7.size(1);
        for (int i120{0}; i120 < bd_loop_ub; i120++) {
            Aeq[(static_cast<int>(u) + Aeq.size(0) * ((i118 + i120) - 1)) + 1] = -r7[i120];
        }
        //
    ...
    for (int i64{0}; i64 < vb_loop_ub; i64++) {
        Aeq[unnamed_idx_1 + Aeq.size(0) * (i62 + i64)] = r7[i64];
    }
    int b_beq;
    //
