...
static bool sMultiWordLe(const unsigned int u1[], const unsigned int u2[]);
                  ...
                  CurvStruct *CurvStruct1, CurvStruct *CurvStruct2)
{
    ...
    sLong2MultiWord(static_cast<int>(std::round(k0 + static_cast<double>(ctx_cfg_NHorz))) - 1,
                    (unsigned int *)&x.chunks[0U]);
    uLong2MultiWord(ctx_q_gcode->size(), (unsigned int *)&y.chunks[0U]);
    if (sMultiWordGt((unsigned int *)&x.chunks[0U], (unsigned int *)&y.chunks[0U])) {
        r1 = y;
    } else {
        r1 = x;
    }
    ...
    exitg1 = false;
    while ((!exitg1) &&
           sMultiWordLe((unsigned int *)&k.chunks[0U], (unsigned int *)&r1.chunks[0U])) {
        double b_y;
        ...
        if (expl_temp.zspdmode == ZSpdMode_NZ) {
            ...
            MultiWordAdd((unsigned int *)&k.chunks[0U], (unsigned int *)&r3.chunks[0U],
    ...
    if (d1uk.size(1) == d2uk.size(1)) {
        ...
        coder::bsxfun(r2D, b_d1uk, r5);
    } else {
        b_binary_expand_op(r5, r2D, d1uk, d2uk);
    }
    if (r2.size(1) == 1) {
        i10 = r5.size(1);
    } else {
    ...
    if ((r2.size(1) == r5.size(1)) && (i10 == c.size(1))) {
        int j_loop_ub;
        ...
        for (int i11{0}; i11 < 3; i11++) {
            for (int i12{0}; i12 < j_loop_ub; i12++) {
                a[i12 + a.size(0) * i11] =
                    (r2[i11 + 3 * i12] + 3.0 * r5[i11 + 3 * i12]) + c[i11 + 3 * i12];
            }
        }
    } else {
        binary_expand_op(a, r2, r5, c);
    }
    ...
    coder::bsxfun(r2D, r, r2);
    coder::bsxfun(b_r1D, d2uk, r5);
    if (r2.size(1) == r5.size(1)) {
        ...
        for (int i20{0}; i20 < 3; i20++) {
            for (int i21{0}; i21 < n_loop_ub; i21++) {
                a[i21 + a.size(0) * i20] = r2[i20 + 3 * i21] + r5[i20 + 3 * i21];
            }
        }
    } else {
        b_binary_expand_op(a, r2, r5);
    }
    b_x.set_size(a.size(0), 3);
    if (a.size(0) != 0) {
        int c_acoef;
    ...
    coder::bsxfun(b_r1D, d1uk, r2);
    r2.set_size(3, r2.size(1));
    o_loop_ub = r2.size(1);
    for (int i24{0}; i24 < o_loop_ub; i24++) {
        double e_varargin_1;
        double f_varargin_1;
        double g_varargin_1;
        e_varargin_1 = r2[3 * i24];
        r2[3 * i24] = std::pow(e_varargin_1, 2.0);
        f_varargin_1 = r2[3 * i24 + 1];
        r2[3 * i24 + 1] = std::pow(f_varargin_1, 2.0);
        g_varargin_1 = r2[3 * i24 + 2];
        r2[3 * i24 + 2] = std::pow(g_varargin_1, 2.0);
    }
    coder::sum(r2, c_x);
    i25 = c_x.size(1);
    ...
    coder::eml_find(b_max_jt, (int *)&cut_index_vel_data, cut_index_vel_size);
    ...
    coder::eml_find(b_max_at, (int *)&cut_index_vel_data, cut_index_vel_size);
    ...
    coder::eml_find(b_norm_vt, (int *)&cut_index_vel_data, cut_index_vel_size);
    // 'orelse:2' if isempty(value)
    if (cut_index_vel_size[1] == 0) {
        // 'orelse:3' v = elsevalue;
                    ...
                    CurvStruct *CurvStruct1, CurvStruct *CurvStruct2)
{
    ...
    b_vmax = b_CurvStruct->FeedRate;
    // 'CutZeroStart:23' N = min(int64(k0 + ctx.cfg.NHorz - 1), int64(ctx.q_gcode.size()));
    sLong2MultiWord(ctx_cfg_NHorz, (unsigned int *)&x.chunks[0U]);
    uLong2MultiWord(ctx_q_gcode->size(), (unsigned int *)&y.chunks[0U]);
    if (sMultiWordGt((unsigned int *)&x.chunks[0U], (unsigned int *)&y.chunks[0U])) {
        r1 = y;
    } else {
    ...
    while ((!exitg1) &&
        ...
        if (expl_temp.zspdmode == ZSpdMode_NZ) {
            ...
            MultiWordAdd((unsigned int *)&k.chunks[0U], (unsigned int *)&r2.chunks[0U],
                         (unsigned int *)&r4.chunks[0U]);
            k = r4;
        }
    }
    ...
    if ((r3.size(1) == r5.size(1)) && (i10 == c.size(1))) {
        int j_loop_ub;
        ...
        for (int i11{0}; i11 < 3; i11++) {
            for (int i12{0}; i12 < j_loop_ub; i12++) {
                a[i12 + a.size(0) * i11] =
                    (r3[i11 + 3 * i12] + 3.0 * r5[i11 + 3 * i12]) + c[i11 + 3 * i12];
            }
        }
    } else {
        binary_expand_op(a, r3, r5, c);
    }
    b_x.set_size(a.size(0), 3);
    if (a.size(0) != 0) {
        int b_acoef;
    ...
    coder::bsxfun(r2D, r, r3);
    coder::bsxfun(b_r1D, d2uk, r5);
    if (r3.size(1) == r5.size(1)) {
        int n_loop_ub;
        a.set_size(r3.size(1), 3);
        n_loop_ub = r3.size(1);
        for (int i20{0}; i20 < 3; i20++) {
            for (int i21{0}; i21 < n_loop_ub; i21++) {
                a[i21 + a.size(0) * i20] = r3[i20 + 3 * i21] + r5[i20 + 3 * i21];
            }
        }
    } else {
        b_binary_expand_op(a, r3, r5);
    }
    b_x.set_size(a.size(0), 3);
    if (a.size(0) != 0) {
        int c_acoef;
    ...
    sqrt_calls++;
    // 'CutZeroStart:38' norm_vt = norm_vt.';
    coder::bsxfun(b_r1D, d1uk, r3);
    r3.set_size(3, r3.size(1));
    o_loop_ub = r3.size(1);
    for (int i24{0}; i24 < o_loop_ub; i24++) {
        double e_varargin_1;
        double f_varargin_1;
        double g_varargin_1;
        e_varargin_1 = r3[3 * i24];
        r3[3 * i24] = std::pow(e_varargin_1, 2.0);
        f_varargin_1 = r3[3 * i24 + 1];
        r3[3 * i24 + 1] = std::pow(f_varargin_1, 2.0);
        g_varargin_1 = r3[3 * i24 + 2];
        r3[3 * i24 + 2] = std::pow(g_varargin_1, 2.0);
    }
    coder::sum(r3, c_x);
    i25 = c_x.size(1);
    for (int k_k{0}; k_k < i25; k_k++) {
