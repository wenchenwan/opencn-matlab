                     ...
                     int *sp3D_Bl_degree, double sp3D_knots[8])
{
    ...
    u[1] = d;
    r = _mm_loadu_pd(&u[0]);
    _mm_storeu_pd(&u[0], _mm_div_pd(r, _mm_set1_pd(d)));
                     ...
                     int *sp3D_Bl_degree, ::coder::array<double, 2U> &sp3D_knots)
{
    ...
    for (i6 = 0; i6 <= vectorUB; i6 += 2) {
        ...
        _mm_storeu_pd(&sp3D_Bl_breakpoints[i6],
                      _mm_div_pd(b_r, _mm_set1_pd(b_sp3D_Bl_breakpoints)));
    }
    ...
    for (int f_k{0}; f_k <= i13; f_k++) {
        ...
        //      sp             = spmak(knots, coef);
        //      BasisVal(:, k) = spval(sp, u);          % tridiagonal matrix
        bspline_eval_vec(Bl_handle, coef, sp3D_Bl_breakpoints, r1);
        n_loop_ub = BasisVal.size(0);
        for (int i17{0}; i17 < n_loop_ub; i17++) {
            BasisVal[i17 + BasisVal.size(0) * f_k] = r1[i17];
        }
        //      sp2D           = fnder(sp, 2);          % evaluate 2nd derivative of basis functions
        //      @u=0,1
        a__2 = 0.0;
