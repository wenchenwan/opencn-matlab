...
//
namespace ocn {
void ConstrHelixStruct(bool trafo, const double Poff[3], const double Aoff[3], const double Uoff[3],
                       ...
                       double pitch, double FeedRate, ZSpdMode mode, CurvStruct *b_CurvStruct)
{
    ...
    // Aoff, ... 'ConstrHelixStruct:8'                                 Uoff, Doff, P0, P1, A0, A1,
    // U0, U1, ... 'ConstrHelixStruct:9'                                 Cprim, delta, evec, theta,
    // ... 'ConstrHelixStruct:10'                                 pitch, CoeffP5, FeedRate);
    for (int i{0}; i < 6; i++) {
        dv[i][0] = 0.0;
        dv[i][1] = 0.0;
        dv[i][2] = 0.0;
    }
    ConstrCurvStruct(CurveType_Helix, mode, trafo, Poff, Aoff, Uoff, Doff, P0, P1, A0, A1, U0, U1,
                     Cprim, delta, evec, theta, pitch, dv, FeedRate, b_CurvStruct);
