...
void CompressCurvStructs(const FeedoptContext *ctx)
{
    ...
    if (!ctx->q_gcode.isempty()) {
        ...
        // 'CompressCurvStructs:18' spline_index = ctx.q_splines.size() + 1;
        spline_index = ctx->q_splines.size() + 1U;
        //  New index in q_spline
        // 'CompressCurvStructs:19' Ncrv = ctx.q_gcode.size;
        Ncrv = ctx->q_gcode.size();
        //  Number of curve in g-code queue
        // 'CompressCurvStructs:20' Length_Threshold = ctx.cfg.LThreshold;
        //  in [mm]
        // 'CompressCurvStructs:22' CumulatedLength = 0;
        CumulatedLength = 0.0;
        //  Accumulator for the length
        // 'CompressCurvStructs:23' spindle_speed = 75000;
        spindle_speed = 75000.0;
        //  in [rpm]
        // 'CompressCurvStructs:25' DebugLog(DebugCfg.Validate, 'Compressing...\n');
        //  1 -> stdout
        //  2 -> stderr
        ...
        //  Satisfy coder
        // 'CompressCurvStructs:28' if coder.target('rtw') || coder.target('mex')
        // 'CompressCurvStructs:29' pvec = zeros(3, 0);
        pvec.set_size(3, 0);
        // 'CompressCurvStructs:30' coder.varsize('pvec', [3, Inf], [0, 1]);
        // 'CompressCurvStructs:31' CurvStruct1 = ctx.q_gcode.get(1);
        ctx->q_gcode.get(static_cast<double>(1.0), &b_ctx);
        // 'CompressCurvStructs:34' k = 1;
        //  index for the Gcode queue
        // 'CompressCurvStructs:36' while k <= Ncrv
        for (double k{1.0}; k <= Ncrv; k++) {
            bool Collinear;
            // 'CompressCurvStructs:37' Curv = ctx.q_gcode.get(k);
            ...
            // 'CompressCurvStructs:40' Collinear = false;
            Collinear = false;
            //  Set down collinear flag
            // 'CompressCurvStructs:41' if k > 1
            if (k > 1.0) {
                double absxk;
                double scale;
                double t;
                ...
                // 'CompressCurvStructs:42' Collinear = CurvCollinear(ctx, ctx.q_gcode.get(k-1),
                // Curv, ... 'CompressCurvStructs:43' ctx.cfg.Compressing.ColTolCos);
                ctx->q_gcode.get(k - 1.0, &expl_temp);
                // 'CurvCollinear:3' [~, V0] = EvalCurvStruct(ctx, Curv1, 1);
                c_EvalCurvStruct(&ctx->q_splines, expl_temp.Type, expl_temp.P0, expl_temp.P1,
                ...
                b_EvalCurvStruct(&ctx->q_splines, Curv.Type, Curv.P0, Curv.P1,
                                 ...
                                 Curv.CoeffP5, Curv.sp_index, Curv.a_param, Curv.b_param, a__2, V1);
            ...
            // 'CompressCurvStructs:50' if ( LengthCurv(ctx, Curv, 0, 1) >= Length_Threshold ) ||
            // ... 'CompressCurvStructs:51'        ( Curv.zspdmode ~= ZSpdMode.NN ) ||...
            // 'CompressCurvStructs:52'        ( CumulatedLength == 0 && ~Collinear )
            if ((LengthCurv(&ctx->q_splines, ctx->cfg.GaussLegendreX, ctx->cfg.GaussLegendreW,
                ...
                // 'CompressCurvStructs:56' if CumulatedLength == 0
                if (CumulatedLength == 0.0) {
                    //  Depending of the speed at the boundary, the segment is split
                    //  and then send to q_compress.
                    // 'CompressCurvStructs:59' if Curv.zspdmode == ZSpdMode.ZN
                    if (Curv.zspdmode == ZSpdMode_ZN) {
                        // 'CompressCurvStructs:60' [CurvStruct1_C, CurvStruct2_C] =
                        // cutZeroStart(ctx, Curv);
                        cutZeroStart(&ctx->q_splines, ctx->cfg.amax, ctx->cfg.jmax, ctx->cfg.dt,
                                     &Curv, &CurvStruct1_C, &CurvStruct2_C);
                        // 'CompressCurvStructs:61' ctx.q_compress.push(CurvStruct1_C);
                        ctx->q_compress.push(&CurvStruct1_C);
                        // 'CompressCurvStructs:62' ctx.q_compress.push(CurvStruct2_C);
                        ctx->q_compress.push(&CurvStruct2_C);
                    } else if (Curv.zspdmode == ZSpdMode_NZ) {
                        // 'CompressCurvStructs:63' elseif Curv.zspdmode == ZSpdMode.NZ
                        // 'CompressCurvStructs:64' [CurvStruct1_C, CurvStruct2_C] = cutZeroEnd(ctx,
                        // Curv);
                        ...
                        // 'CompressCurvStructs:67' elseif Curv.zspdmode == ZSpdMode.ZZ
                        // 'CompressCurvStructs:68' [CurvStruct1_C, CurvStruct2_C] =
                        // cutZeroStart(ctx, Curv);
                        cutZeroStart(&ctx->q_splines, ctx->cfg.amax, ctx->cfg.jmax, ctx->cfg.dt,
                                     &Curv, &CurvStruct1_C, &CurvStruct2_C);
                        // 'CompressCurvStructs:69' [CurvStruct2_C, CurvStruct3_C] = cutZeroEnd(ctx,
                        // CurvStruct2_C);
                        cutZeroEnd(&ctx->q_splines, ctx->cfg.amax, ctx->cfg.jmax, ctx->cfg.dt,
                                   &CurvStruct2_C, &b_CurvStruct2_C, &CurvStruct3_C);
                        // 'CompressCurvStructs:70' ctx.q_compress.push(CurvStruct1_C);
                        ctx->q_compress.push(&CurvStruct1_C);
                        // 'CompressCurvStructs:71' ctx.q_compress.push(CurvStruct2_C);
                        ctx->q_compress.push(&b_CurvStruct2_C);
                        // 'CompressCurvStructs:72' ctx.q_compress.push(CurvStruct3_C);
                        ctx->q_compress.push(&CurvStruct3_C);
                    } else {
                        // 'CompressCurvStructs:73' else
                        // 'CompressCurvStructs:74' ctx.q_compress.push(Curv);
                        ctx->q_compress.push(&Curv);
                    }
                    //  If there was an on-going compression
                } else {
                    // 'CompressCurvStructs:78' else
                    //  We have more than 2 points, thus constructing a spline
                    //  is warranted
                    // 'CompressCurvStructs:81' if size(pvec, 2) > 2
                    if (pvec.size(1) > 2) {
                        int b_pvec;
                        // 'CompressCurvStructs:82' SplineCurve = ConstrCurvStructType;
                        ConstrCurvStructType(&SplineCurve);
                        // 'CompressCurvStructs:83' SplineCurve.sp = CalcBspline_Lee(ctx.cfg, pvec);
                        CalcBspline_Lee(ctx->cfg.LeeSplineDegree, pvec, SplineCurve.sp.CoeffX,
                                        SplineCurve.sp.CoeffY, SplineCurve.sp.CoeffZ, &t1_Bl_ncoeff,
                                        SplineCurve.sp.Bl.breakpoints, &t1_Bl_handle, &t1_Bl_degree,
                                        SplineCurve.sp.knots);
                        SplineCurve.sp.Bl.ncoeff = t1_Bl_ncoeff;
                        SplineCurve.sp.Bl.handle = t1_Bl_handle;
                        SplineCurve.sp.Bl.degree = t1_Bl_degree;
                        // 'CompressCurvStructs:84' SplineCurve.sp.Ltot = 0;
                        //  satisfy coder
                        // 'CompressCurvStructs:85' SplineCurve.sp.Lk = 0;
                        //  satisfy coder
                        // 'CompressCurvStructs:86' [Ltot, Lk]    = SplineLengthApproxGL_tot(ctx,
                        // SplineCurve);
                        SplineLengthApproxGL_tot(ctx->cfg.GaussLegendreN, ctx->cfg.GaussLegendreX,
                                                 ...
                                                 t1_Bl_handle, SplineCurve.sp.knots,
                                                 &SplineCurve.sp.Ltot, SplineCurve.sp.Lk);
                        // 'CompressCurvStructs:87' SplineCurve.sp.Ltot = Ltot;
                        // 'CompressCurvStructs:88' SplineCurve.sp.Lk   = Lk;
                        // 'CompressCurvStructs:89' ctx.q_splines.push(SplineCurve);
                        ctx->q_splines.push(&SplineCurve);
                        // 'CompressCurvStructs:90' spline = ConstrBSplineStruct(Curv.TRAFO,
                        // Curv.HSC,... 'CompressCurvStructs:91' Curv.Poff, ...
                        // 'CompressCurvStructs:92' Curv.Aoff, Curv.Uoff, ...
                        // 'CompressCurvStructs:93' Curv.Doff, pvec,... 'CompressCurvStructs:94'
                        // [Curv.A0,Curv.A1], ... 'CompressCurvStructs:95' [Curv.U0,Curv.U1], ...
                        // 'CompressCurvStructs:96' ZSpdMode.NN, Curv.FeedRate);
                        ...
                        // 'CompressCurvStructs:97' spline.gcode_source_line =
                        // Curv.gcode_source_line;
                        spline.gcode_source_line = Curv.gcode_source_line;
                        // 'CompressCurvStructs:98' spline.sp_index = int32(spline_index);
                        spline.sp_index = static_cast<int>(spline_index);
                        // 'CompressCurvStructs:99' spline_index = spline_index + 1;
                        spline_index++;
                        // 'CompressCurvStructs:100' spline.SpindleSpeed = spindle_speed;
                        spline.SpindleSpeed = spindle_speed;
                        // 'CompressCurvStructs:101' ctx.q_compress.push(spline);
                        ctx->q_compress.push(&spline);
                        // 'CompressCurvStructs:103' if Curv.zspdmode == ZSpdMode.NZ
                        if (Curv.zspdmode == ZSpdMode_NZ) {
                            // 'CompressCurvStructs:104' [CurvStruct1_C, CurvStruct2_C] =
                            // cutZeroEnd(ctx, Curv);
                            cutZeroEnd(&ctx->q_splines, ctx->cfg.amax, ctx->cfg.jmax, ctx->cfg.dt,
                                       &Curv, &CurvStruct1_C, &CurvStruct2_C);
                            // 'CompressCurvStructs:105' ctx.q_compress.push(CurvStruct1_C);
                            ctx->q_compress.push(&CurvStruct1_C);
                            // 'CompressCurvStructs:106' ctx.q_compress.push(CurvStruct2_C);
                            ctx->q_compress.push(&CurvStruct2_C);
                        } else {
                            // 'CompressCurvStructs:107' else
                            // 'CompressCurvStructs:108' ctx.q_compress.push(Curv);
                            ctx->q_compress.push(&Curv);
                        }
                        //  With only two points, construct a line
                    } else {
                        // 'CompressCurvStructs:111' else
                        // 'CompressCurvStructs:112' C = ctx.q_gcode.get(k-1);
                        ctx->q_gcode.get(k - 1.0, &C);
                        //  get the previous segment
                        // 'CompressCurvStructs:113' C.gcode_source_line=Curv.gcode_source_line;
                        C.gcode_source_line = Curv.gcode_source_line;
                        // 'CompressCurvStructs:114' ctx.q_compress.push(C);
                        ctx->q_compress.push(&C);
                        //  push segment to q_compress
                        // 'CompressCurvStructs:116' if Curv.zspdmode == ZSpdMode.NZ
                        if (Curv.zspdmode == ZSpdMode_NZ) {
                            //  split if zero end
                            // 'CompressCurvStructs:117' [CurvStruct1_C, CurvStruct2_C] =
                            // cutZeroEnd(ctx, Curv);
                            cutZeroEnd(&ctx->q_splines, ctx->cfg.amax, ctx->cfg.jmax, ctx->cfg.dt,
                                       &Curv, &CurvStruct1_C, &CurvStruct2_C);
                            // 'CompressCurvStructs:118' ctx.q_compress.push(CurvStruct1_C);
                            ctx->q_compress.push(&CurvStruct1_C);
                            // 'CompressCurvStructs:119' ctx.q_compress.push(CurvStruct2_C);
                            ctx->q_compress.push(&CurvStruct2_C);
                        } else {
                            // 'CompressCurvStructs:120' else
                            // 'CompressCurvStructs:121' ctx.q_compress.push(Curv);
                            ctx->q_compress.push(&Curv);
                            //  push segment to q_compress
                        }
                    }
                    // 'CompressCurvStructs:124' CumulatedLength = 0;
                    CumulatedLength = 0.0;
                }
                ...
                int c_pvec;
                // 'CompressCurvStructs:128' elseif (k==Ncrv) && (CumulatedLength ~= 0)
                // 'CompressCurvStructs:129' SplineCurve = ConstrCurvStructType;
                ConstrCurvStructType(&SplineCurve);
                // 'CompressCurvStructs:130' SplineCurve.sp=CalcBspline_Lee(ctx.cfg, pvec);
                CalcBspline_Lee(ctx->cfg.LeeSplineDegree, pvec, SplineCurve.sp.CoeffX,
                                SplineCurve.sp.CoeffY, SplineCurve.sp.CoeffZ, &t1_Bl_ncoeff,
                                SplineCurve.sp.Bl.breakpoints, &t1_Bl_handle, &t1_Bl_degree,
                                SplineCurve.sp.knots);
                ...
                SplineCurve.sp.Bl.handle = t1_Bl_handle;
                SplineCurve.sp.Bl.degree = t1_Bl_degree;
                // 'CompressCurvStructs:131' SplineCurve.sp.Ltot = 0;
                //  satisfy coder
                // 'CompressCurvStructs:132' SplineCurve.sp.Lk = 0;
                //  satisfy coder
                // 'CompressCurvStructs:133' [Ltot, Lk]     = SplineLengthApproxGL_tot(ctx,
                // SplineCurve);
                SplineLengthApproxGL_tot(
                    ...
                    t1_Bl_handle, SplineCurve.sp.knots, &SplineCurve.sp.Ltot, SplineCurve.sp.Lk);
                // 'CompressCurvStructs:134' SplineCurve.sp.Ltot = Ltot;
                // 'CompressCurvStructs:135' SplineCurve.sp.Lk   = Lk;
                // 'CompressCurvStructs:136' ctx.q_splines.push(SplineCurve);
                ctx->q_splines.push(&SplineCurve);
                // 'CompressCurvStructs:137' spline = ConstrBSplineStruct(Curv.TRAFO, Curv.HSC,...
                // 'CompressCurvStructs:138'                                     Curv.Poff,...
                // 'CompressCurvStructs:139'                                     Curv.Aoff,
                // Curv.Uoff, ... 'CompressCurvStructs:140' Curv.Doff, pvec,...
                // 'CompressCurvStructs:141'                                     [Curv.A0,Curv.A1],
                // ... 'CompressCurvStructs:142' [Curv.U0,Curv.U1], ... 'CompressCurvStructs:143'
                ...
                // 'CompressCurvStructs:144' spline.gcode_source_line = Curv.gcode_source_line;
                spline.gcode_source_line = Curv.gcode_source_line;
                // 'CompressCurvStructs:145' spline.sp_index = int32(spline_index);
                spline.sp_index = static_cast<int>(spline_index);
                // 'CompressCurvStructs:146' spline.SpindleSpeed = spindle_speed;
                spline.SpindleSpeed = spindle_speed;
                // 'CompressCurvStructs:147' ctx.q_compress.push(spline);
                ctx->q_compress.push(&spline);
                //  If this is the first (and elligible) WHAT
            } else if (k == 1.0) {
                // 'CompressCurvStructs:150' elseif k==1
                // 'CompressCurvStructs:151' ctx.q_compress.push(Curv);
                ctx->q_compress.push(&Curv);
                ...
                // 'CompressCurvStructs:156' if CumulatedLength == 0
                if (CumulatedLength == 0.0) {
                    // 'CompressCurvStructs:157' P0 = EvalCurvStruct(ctx, Curv, 0);
                    b_EvalCurvStruct(&ctx->q_splines, Curv.Type, Curv.P0, Curv.P1,
                                     Curv.CorrectedHelixCenter, Curv.evec, Curv.theta, Curv.pitch,
                                     Curv.CoeffP5, Curv.sp_index, Curv.b_param, P0);
                    // 'CompressCurvStructs:158' pvec = P0;
                    pvec.set_size(3, 1);
                    pvec[0] = P0[0];
                    pvec[1] = P0[1];
                    pvec[2] = P0[2];
                    // 'CompressCurvStructs:159' spindle_speed = Curv.SpindleSpeed;
                    spindle_speed = Curv.SpindleSpeed;
                }
                // 'CompressCurvStructs:162' CumulatedLength = CumulatedLength + LengthCurv(ctx,
                // Curv, 0, 1);
                ...
                // 'CompressCurvStructs:163' P1 = EvalCurvStruct(ctx, Curv, 1);
                b_EvalCurvStruct(&ctx->q_splines, Curv.Type, Curv.P0, Curv.P1,
                                 Curv.CorrectedHelixCenter, Curv.evec, Curv.theta, Curv.pitch,
                                 Curv.CoeffP5, Curv.sp_index, Curv.a_param, Curv.b_param, P1);
                // 'CompressCurvStructs:164' pvec = [pvec P1];
                i = pvec.size(1);
                pvec.set_size(3, pvec.size(1) + 1);
                pvec[3 * i] = P1[0];
                pvec[3 * i + 1] = P1[1];
                pvec[3 * i + 2] = P1[2];
                // 'CompressCurvStructs:165' spindle_speed = min(spindle_speed, Curv.SpindleSpeed);
                spindle_speed = std::fmin(spindle_speed, Curv.SpindleSpeed);
            }
            // 'CompressCurvStructs:167' k = k + 1;
        }
    }
}
