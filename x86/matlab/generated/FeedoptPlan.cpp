...
void FeedoptPlan(FeedoptContext *ctx, bool *optimized, CurvStruct *opt_struct)
{
    ...
    case Fopt_GCode: {
        ...
        while (status != 0) {
            ...
            if ((b_status == 1) && (static_cast<int>(b_CurvStruct.Type) != 0)) {
                // 'FeedoptPlan:38' if ( CurvStruct.FeedRate == 0 )
                if (b_CurvStruct.FeedRate == 0.0) {
                    // 'FeedoptPlan:39' CurvStruct.FeedRate = ctx.cfg.vmax;
        ...
        // 'FeedoptPlan:45' if ctx.q_gcode.isempty()
        if (ctx->q_gcode.isempty()) {
            // 'FeedoptPlan:46' ctx.op = Fopt.Finished;
            ctx->op = Fopt_Finished;
            // 'FeedoptPlan:47' DebugLog(DebugCfg.Warning, ...
            // 'FeedoptPlan:48'                 'ERROR: Optimization failed, Gcode queue is
            // empty\n');
            //  1 -> stdout
            //  2 -> stderr
            // 'DebugLog:5' if IsEnabledDebugLog(cfg)
            ...
            // 'FeedoptPlan:51' last = ctx.q_gcode.rget(1);
            ctx->q_gcode.rget(&last);
            // 'FeedoptPlan:52' if last.zspdmode == ZSpdMode.NN
            if (last.zspdmode == ZSpdMode_NN) {
                // 'FeedoptPlan:53' last.zspdmode = ZSpdMode.NZ;
                last.zspdmode = ZSpdMode_NZ;
            } else if (last.zspdmode == ZSpdMode_ZN) {
                // 'FeedoptPlan:54' elseif last.zspdmode == ZSpdMode.ZN
                // 'FeedoptPlan:55' last.zspdmode = ZSpdMode.ZZ;
                last.zspdmode = ZSpdMode_ZZ;
            }
            // 'FeedoptPlan:57' ctx.q_gcode.set(ctx.q_gcode.size, last);
    ...
    case Fopt_Compress:
        ...
        if (ctx->cfg.Compressing.Skip) {
            // 'FeedoptPlan:66' ctx = ExpandZeroStructs(ctx);
            ExpandZeroStructs(ctx);
        } else {
            // 'FeedoptPlan:67' else
            // 'FeedoptPlan:68' ctx = CompressCurvStructs(ctx);
            CompressCurvStructs(ctx);
        }
        // 'FeedoptPlan:70' ctx.op = Fopt.Smooth;
        ctx->op = Fopt_Smooth;
        break;
    case Fopt_Smooth:
        // 'FeedoptPlan:72' case Fopt.Smooth
        // 'FeedoptPlan:73' ctx = SmoothCurvStructs(ctx);
        SmoothCurvStructs(ctx);
        // 'FeedoptPlan:74' ctx.op = Fopt.Split;
        ctx->op = Fopt_Split;
        break;
    case Fopt_Split:
        // 'FeedoptPlan:76' case Fopt.Split
        // 'FeedoptPlan:77' ctx = SplitCurvStructs(ctx);
        SplitCurvStructs(ctx);
        // 'FeedoptPlan:78' ctx.op = Fopt.Opt;
        ctx->op = Fopt_Opt;
    ...
    case Fopt_Opt: {
        ...
        if (ctx->q_split.isempty()) {
            ...
            // 'FeedoptPlan:99' if ctx.go_next
            if (ctx->go_next) {
                // 'FeedoptPlan:100' ctx.k0 = ctx.k0 + 1;
                ctx->k0++;
                // 'FeedoptPlan:101' ctx.n_optimized = ctx.n_optimized + 1;
                ctx->n_optimized++;
            }
                ...
                if (ctx->try_push_again) {
                    ...
                    if (b_first.zspdmode == ZSpdMode_ZN) {
                        ...
                        *opt_struct = b_first;
                        ...
                        bool exitg1;
                        bool success;
                        // 'FeedoptPlan:122' k1temp = int32(ctx.k0 + ctx.cfg.NHorz - 1);
                        k1temp = (ctx->k0 + ctx->cfg.NHorz) - 1;
                        // 'FeedoptPlan:123' if k1temp > ctx.q_split.size
                        if (static_cast<double>(k1temp) > ctx->q_split.size()) {
                            // 'FeedoptPlan:124' ctx.reached_end = true;
                            ctx->reached_end = true;
                            // 'FeedoptPlan:125' k1 = int32(ctx.q_split.size);
                            k1 = static_cast<int>(ctx->q_split.size());
                        } else {
                            // 'FeedoptPlan:126' else
                            // 'FeedoptPlan:127' k1 = int32(k1temp);
                            k1 = k1temp;
                        }
                        // 'FeedoptPlan:130' ctx.at_1 = 0;
                        ctx->at_1 = 0.0;
                        // 'FeedoptPlan:131' ctx.v_1 = 0;
                        ctx->v_1 = 0.0;
                        // 'FeedoptPlan:133' nopt = 0;
                        nopt = 0U;
                        // 'FeedoptPlan:134' DebugLog(DebugCfg.Global, 'FEEDRATE PLANNING...\n')
                        //  1 -> stdout
                        //  2 -> stderr
                        // 'DebugLog:5' if IsEnabledDebugLog(cfg)
                        // 'IsEnabledDebugLog:4' value = false;
                        ...
                        // 'FeedoptPlan:135' kend = ctx.k0;
                        kend = ctx->k0;
                        // 'FeedoptPlan:136' for k = ctx.k0:k1
                        k = ctx->k0;
                        exitg1 = false;
                        while ((!exitg1) && (k <= k1)) {
                            // 'FeedoptPlan:137' NextCurv = ctx.q_split.get(k);
                            ctx->q_split.get(k, &NextCurv);
                            // 'FeedoptPlan:138' if NextCurv.zspdmode == ZSpdMode.NZ
                            if (NextCurv.zspdmode == ZSpdMode_NZ) {
                                ...
                                //  end of all segments
                                // 'FeedoptPlan:143' [v_0, at_0] = CalcZeroStartConstraints(ctx,
                                // NextCurv, 1);
                                CalcZeroStartConstraints(
                                ...
                                // 'FeedoptPlan:144' ctx.at_1 = -at_0;
                                ctx->at_1 = -at_0;
                                // 'FeedoptPlan:145' ctx.v_1 = v_0;
                                // 'FeedoptPlan:146' ctx.zero_end = true;
                                ctx->zero_end = true;
                                // 'FeedoptPlan:147' ctx.reached_end = true;
                                ctx->reached_end = true;
                                // 'FeedoptPlan:148' kend = k;
                                kend = k;
                                exitg1 = true;
                            } else {
                                if (NextCurv.zspdmode == ZSpdMode_NN) {
                                    // 'FeedoptPlan:150' elseif NextCurv.zspdmode == ZSpdMode.NN
                                    // 'FeedoptPlan:151' nopt = nopt + 1;
                                    nopt++;
                                    // 'FeedoptPlan:152' OptSegment(nopt) = NextCurv;
                                    OptSegment[static_cast<int>(nopt) - 1] = NextCurv;
                                    // 'FeedoptPlan:154' if IsEnabledDebugLog(DebugCfg.Global)
                                    // 'IsEnabledDebugLog:4' value = false;
                                    // 'IsEnabledDebugLog:6' if bitget(DebugConfig, int32(cfg))
                                    if (u != 0UL) {
                                        // 'IsEnabledDebugLog:7' value = true;
                                        // 'FeedoptPlan:155' PrintCurvStruct(ctx,
                                        // OptSegment(k-ctx.k0+1))
                                        PrintCurvStruct(&ctx->q_splines, ctx->cfg.GaussLegendreX,
                                                        ctx->cfg.GaussLegendreW,
                                                        &OptSegment[k - ctx->k0]);
                                    }
                                    // 'FeedoptPlan:157' if k < k1
                                    if ((k < k1) && (u != 0UL)) {
                                        // 'FeedoptPlan:158' DebugLog(DebugCfg.Global, ...
                                        // 'FeedoptPlan:159'
                                        // '-----------------------------------\n')
                                        //  1 -> stdout
                                        //  2 -> stderr
                                    ...
                                    // 'FeedoptPlan:161' else
                                    // 'FeedoptPlan:162' error('Wrong ZspdMode');
                                }
                                k++;
                            }
                        }
                        // 'FeedoptPlan:165' DebugLog(DebugCfg.Global, ...
                        // 'FeedoptPlan:166' '================================================\n')
                        //  1 -> stdout
                        //  2 -> stderr
                        // 'DebugLog:5' if IsEnabledDebugLog(cfg)
                        ...
                        // 'FeedoptPlan:168' Retry = 0;
                        Retry = 0;
                        // 'FeedoptPlan:169' success = false;
                        success = false;
                        // 'FeedoptPlan:170' Coeff = [];
                        Coeff.set_size(0, 0);
                        // 'FeedoptPlan:172' while Retry < 100 && ~success
                        while ((Retry < 100) && (!success)) {
                            int d_ctx;
                            // 'FeedoptPlan:173' [ctx, Coeff, ~, success] = FeedratePlanning_v4(ctx,
                            // OptSegment, ctx.cfg.amax, ctx.cfg.jmax,... 'FeedoptPlan:174'
                            // ctx.BasisVal, ctx.BasisValD, ctx.BasisValDD, ctx.BasisIntegr,...
                            // 'FeedoptPlan:175'                         ctx.Bl, ctx.u_vec,
                            ...
                            success = b_success;
                            // 'FeedoptPlan:177' if success == 0 && ctx.zero_start
                            if ((!b_success) && ctx->zero_start) {
                                // 'FeedoptPlan:178' DebugLog(DebugCfg.Warning, 'ZeroStart at k = %d
                                // failed, halving jerk\n', ctx.k0-1);
                                //  1 -> stdout
                                //  2 -> stderr
                                // 'DebugLog:5' if IsEnabledDebugLog(cfg)
                                ...
                                // 'FeedoptPlan:179' [v_0, at_0] = CalcZeroStartConstraints(ctx,
                                ...
                                CalcZeroStartConstraints(
                                    ...
                                    &ctx->at_0);
                                // 'FeedoptPlan:180' ctx.v_0 = v_0;
                                // 'FeedoptPlan:181' ctx.at_0 = at_0;
                            }
                            // 'FeedoptPlan:184' if success == 0 && ctx.zero_end
                            if ((!b_success) && ctx->zero_end) {
                                // 'FeedoptPlan:185' DebugLog(DebugCfg.Warning, 'ZeroEnd at k = %d
                                // failed, halving jerk\n', kend);
                                //  1 -> stdout
                                //  2 -> stderr
                                // 'DebugLog:5' if IsEnabledDebugLog(cfg)
                                ...
                                // 'FeedoptPlan:186' [v_0, at_0] = CalcZeroStartConstraints(ctx,
                                ...
                                // 'FeedoptPlan:187' ctx.at_1 = -at_0;
                                ctx->at_1 = -b_at_0;
                                // 'FeedoptPlan:188' ctx.v_1 = v_0;
                            }
                            // 'FeedoptPlan:191' Retry = Retry + 1;
                            Retry++;
                        }
                        // 'FeedoptPlan:194' if coder.target('matlab')
                        // 'FeedoptPlan:198' ctx.zero_start = false;
                        ctx->zero_start = false;
                        // 'FeedoptPlan:199' ctx.zero_end = false;
                        ctx->zero_end = false;
                        // 'FeedoptPlan:201' ctx.Coeff = Coeff;
                        ...
                        // 'FeedoptPlan:202' if success == 0
                        if (!success) {
                            int i11;
                            // 'FeedoptPlan:203' for nprint = 1:ctx.cfg.NHorz
                            i11 = ctx->cfg.NHorz;
                            for (int nprint{0}; nprint < i11; nprint++) {
                                // 'FeedoptPlan:204' if IsEnabledDebugLog(DebugCfg.Global)
                                // 'IsEnabledDebugLog:4' value = false;
                                // 'IsEnabledDebugLog:6' if bitget(DebugConfig, int32(cfg))
                                if ((static_cast<unsigned long>(DebugConfig) & 8UL) != 0UL) {
                                    // 'IsEnabledDebugLog:7' value = true;
                                    // 'FeedoptPlan:205' PrintCurvStruct(ctx, OptSegment(1));
                                    PrintCurvStruct(&ctx->q_splines, ctx->cfg.GaussLegendreX,
                                                    ctx->cfg.GaussLegendreW, &OptSegment[0]);
                                }
                            }
                            // 'FeedoptPlan:208' if coder.target('MATLAB')
                            // 'FeedoptPlan:210' else
                            // 'FeedoptPlan:211' DebugLog(DebugCfg.Global, 'OPTIMIZATION
                            // FAILED!\n');
                            //  1 -> stdout
                            //  2 -> stderr
                            // 'DebugLog:5' if IsEnabledDebugLog(cfg)
                            ...
                            // 'FeedoptPlan:212' ctx.errcode = FeedoptPlanError.OptimizationFailed;
                            ctx->errcode = FeedoptPlanError_OptimizationFailed;
                            // 'FeedoptPlan:214' ctx.op = Fopt.Finished;
                            ctx->op = Fopt_Finished;
                        } else {
                            guard1 = true;
                        }
                    ...
                    // 'FeedoptPlan:220' ctx.Coeff(:, 1:end-1) = ctx.Coeff(:, 2:end);
    ...
    case Fopt_Finished:
        // 'FeedoptPlan:235' case Fopt.Finished
        // 'FeedoptPlan:236' ctx.op = Fopt.Finished;
        ctx->op = Fopt_Finished;
        break;
    default:
        // 'FeedoptPlan:238' otherwise
        // 'FeedoptPlan:239' DebugLog(DebugCfg.Global, 'FEEDOPT: WRONG STATE\n')
        //  1 -> stdout
        //  2 -> stderr
        // 'DebugLog:5' if IsEnabledDebugLog(cfg)
        // 'IsEnabledDebugLog:4' value = false;
        ...
        // 'FeedoptPlan:240' ctx.op = Fopt.Finished;
        ctx->op = Fopt_Finished;
        break;
    }
    if (guard1) {
        int loop_ub;
        // 'FeedoptPlan:223' optimized = true;
        b_optimized = true;
        // 'FeedoptPlan:224' opt_struct = ctx.q_split.get(ctx.n_optimized + 1);
        ctx->q_split.get(ctx->n_optimized + 1, opt_struct);
        // 'FeedoptPlan:225' opt_struct.Coeff = ctx.Coeff(:, 1);
        loop_ub = ctx->Coeff.size(0);
        opt_struct->Coeff.set_size(loop_ub);
        for (int i1{0}; i1 < loop_ub; i1++) {
            opt_struct->Coeff[i1] = ctx->Coeff[i1];
        }
        // 'FeedoptPlan:227' if opt_struct.zspdmode == ZSpdMode.NZ
        if (opt_struct->zspdmode == ZSpdMode_NZ) {
            // 'FeedoptPlan:228' ctx.reached_end = false;
