                   ...
                   ::coder::array<double, 1U> &A_din)
{
    ...
    if (G90) {
        ...
        scalarLB = (delta.size(0) / 2) << 1;
        vectorUB = scalarLB - 2;
        for (i5 = 0; i5 <= vectorUB; i5 += 2) {
            __m128d r2;
            r2 = _mm_loadu_pd(&delta[i5]);
            _mm_storeu_pd(&delta_CCW[i5], _mm_sub_pd(_mm_set1_pd(360.0), r2));
        }
        ...
        if (y) {
            ...
            if (b_y) {
                ...
                for (int i9{0}; i9 < l_loop_ub; i9++) {
                    x[i9] = (static_cast<int>(AngleMode_Closest) == A_mode[i9]);
                }
                d_y = (x.size(0) != 0);
                if (d_y) {
                    int d_k;
        ...
        case 0: {
            ...
            if (A_prev.size(0) == delta.size(0)) {
                ...
                b_scalarLB = (A_prev.size(0) / 2) << 1;
                b_vectorUB = b_scalarLB - 2;
                for (i10 = 0; i10 <= b_vectorUB; i10 += 2) {
                    __m128d r3;
                    r3 = _mm_loadu_pd(&delta[i10]);
                    _mm_storeu_pd(&A_din[i10],
                                  _mm_add_pd(_mm_loadu_pd((const double *)&A_prev[i10]), r3));
                }
                for (i10 = b_scalarLB; i10 < i_loop_ub; i10++) {
                    A_din[i10] = A_prev[i10] + delta[i10];
                }
            } else {
        ...
        case 1: {
            ...
            if (A_prev.size(0) == delta_CCW.size(0)) {
                ...
                c_scalarLB = (A_prev.size(0) / 2) << 1;
                c_vectorUB = c_scalarLB - 2;
                for (i11 = 0; i11 <= c_vectorUB; i11 += 2) {
                    __m128d r4;
                    r4 = _mm_loadu_pd(&delta_CCW[i11]);
                    _mm_storeu_pd(&A_din[i11],
                                  _mm_sub_pd(_mm_loadu_pd((const double *)&A_prev[i11]), r4));
                }
                for (i11 = c_scalarLB; i11 < j_loop_ub; i11++) {
                    A_din[i11] = A_prev[i11] - delta_CCW[i11];
                }
            } else {
        ...
        case 2: {
            ...
            if (c_y) {
                ...
                d_scalarLB = (delta_CCW.size(0) / 2) << 1;
                d_vectorUB = d_scalarLB - 2;
                for (i12 = 0; i12 <= d_vectorUB; i12 += 2) {
                    __m128d r5;
                    r5 = _mm_loadu_pd(&delta_CCW[i12]);
                    _mm_storeu_pd(&delta[i12], _mm_mul_pd(r5, _mm_set1_pd(-1.0)));
                }
                for (i12 = d_scalarLB; i12 < m_loop_ub; i12++) {
            ...
            if (A_prev.size(0) == delta.size(0)) {
                ...
                e_scalarLB = (A_prev.size(0) / 2) << 1;
                e_vectorUB = e_scalarLB - 2;
                for (i13 = 0; i13 <= e_vectorUB; i13 += 2) {
                    __m128d r6;
                    r6 = _mm_loadu_pd(&delta[i13]);
                    _mm_storeu_pd(&A_din[i13],
                                  _mm_add_pd(_mm_loadu_pd((const double *)&A_prev[i13]), r6));
                }
                for (i13 = e_scalarLB; i13 < n_loop_ub; i13++) {
