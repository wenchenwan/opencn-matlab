                   ...
                   const double ctx_cfg_GaussLegendreW[5], CurvStruct *b_CurvStruct, double d1)
{
    ...
    if (b_CurvStruct->Type == CurveType_Spline) {
        ...
        if (d1 != 0.0) {
            ...
            r1 = _mm_set1_pd(1.0);
            r2 = _mm_set1_pd(c_expl_temp);
            r3 = _mm_set1_pd(u1_tilda);
            r4 = _mm_set1_pd(2.0);
            _mm_storeu_pd(&d_expl_temp[0], _mm_div_pd(_mm_add_pd(_mm_mul_pd(r2, _mm_sub_pd(r1, r)),
                                                                 _mm_mul_pd(r3, _mm_add_pd(r, r1))),
                                                      r4));
            r = _mm_loadu_pd((const double *)&ctx_cfg_GaussLegendreX[2]);
            _mm_storeu_pd(&d_expl_temp[2], _mm_div_pd(_mm_add_pd(_mm_mul_pd(r2, _mm_sub_pd(r1, r)),
                                                                 _mm_mul_pd(r3, _mm_add_pd(r, r1))),
                                                      r4));
            d_expl_temp[4] = (c_expl_temp * (1.0 - ctx_cfg_GaussLegendreX[4]) +
                              u1_tilda * (ctx_cfg_GaussLegendreX[4] + 1.0)) /
                             2.0;
                ...
                if (Lcum < d1) {
                    ...
                    while (std::abs(uk - uk_old) > 1.0E-6) {
                        ...
                        for (int i5{0}; i5 < loop_ub; i5++) {
                            b_expl_temp.sp.CoeffX[i5] = e_expl_temp.sp.CoeffX[i5];
                        }
                        ...
                        for (int i6{0}; i6 < b_loop_ub; i6++) {
                            b_expl_temp.sp.CoeffY[i6] = e_expl_temp.sp.CoeffY[i6];
                        }
                        ...
                        for (int i7{0}; i7 < c_loop_ub; i7++) {
                            b_expl_temp.sp.CoeffZ[i7] = e_expl_temp.sp.CoeffZ[i7];
                        }
                        __m128d r5;
                        __m128d r6;
                        __m128d r7;
                        ...
                        r6 = _mm_set1_pd(uk);
                        r7 = _mm_set1_pd(u1);
                        _mm_storeu_pd(&b_uk[0],
                                      _mm_div_pd(_mm_add_pd(_mm_mul_pd(r6, _mm_sub_pd(r1, r5)),
                                                            _mm_mul_pd(r7, _mm_add_pd(r5, r1))),
                                                 r4));
                        r5 = _mm_loadu_pd((const double *)&ctx_cfg_GaussLegendreX[2]);
                        _mm_storeu_pd(&b_uk[2],
                                      _mm_div_pd(_mm_add_pd(_mm_mul_pd(r6, _mm_sub_pd(r1, r5)),
                                                            _mm_mul_pd(r7, _mm_add_pd(r5, r1))),
                                                 r4));
                        b_uk[4] = (uk * (1.0 - ctx_cfg_GaussLegendreX[4]) +
                                   u1 * (ctx_cfg_GaussLegendreX[4] + 1.0)) /
                                  2.0;
