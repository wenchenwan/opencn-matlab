               ...
               double r0D[10][3], double r1D[10][3], double r2D[10][3], double r3D[10][3])
{
    ...
    if (c_assert(
        ...
        for (int k{0}; k < 10; k++) {
            ...
            r6 = _mm_set1_pd(d2);
            _mm_storeu_pd(&sphiTCP0[k][0], _mm_mul_pd(r, r6));
            _mm_storeu_pd(&cphiTEcrCP0[k][0], _mm_mul_pd(r1, r2));
            r8 = _mm_mul_pd(r1, r6);
            _mm_storeu_pd(&sphiTEcrCP0[k][0], r8);
            _mm_storeu_pd(
                &r0D[k][0],
                _mm_add_pd(
                    _mm_loadu_pd((const double *)&CurvStruct_CorrectedHelixCenter[0]),
        ...
        for (int b_k{0}; b_k < 10; b_k++) {
            ...
            double d4;
            double d6;
            r7 = _mm_loadu_pd(&sphiTCP0[b_k][0]);
            r9 = _mm_loadu_pd(&cphiTEcrCP0[b_k][0]);
            _mm_storeu_pd(&r1D[b_k][0],
                          _mm_add_pd(_mm_add_pd(_mm_mul_pd(_mm_set1_pd(-CurvStruct_theta), r7),
            ...
            _mm_storeu_pd(&r3D[b_k][0], _mm_sub_pd(_mm_mul_pd(r5, r7), _mm_mul_pd(r5, r9)));
            d4 = sphiTCP0[b_k][2];
    ...
    if (c_assert(
        ...
        for (i4 = 0; i4 <= vectorUB; i4 += 2) {
            _mm_storeu_pd(&phi_vec[i4], _mm_mul_pd(_mm_set1_pd(CurvStruct_theta),
                                                   _mm_loadu_pd((const double *)&u_vec[i4])));
        }
        ...
        for (int i9{0}; i9 < h_loop_ub; i9++) {
            __m128d r;
            r = _mm_loadu_pd(&CP0[0]);
            _mm_storeu_pd(&cphiTCP0[3 * i9], _mm_mul_pd(r, _mm_set1_pd(cphi[i9])));
        ...
        for (int i10{0}; i10 < i_loop_ub; i10++) {
            ...
            _mm_storeu_pd(&sphiTCP0[3 * i10], _mm_mul_pd(r1, _mm_set1_pd(sphi[i10])));
        ...
        for (int i11{0}; i11 < j_loop_ub; i11++) {
            ...
            _mm_storeu_pd(&cphiTEcrCP0[3 * i11], _mm_mul_pd(r2, _mm_set1_pd(cphi[i11])));
        ...
        for (int i12{0}; i12 < k_loop_ub; i12++) {
            ...
            _mm_storeu_pd(&sphiTEcrCP0[3 * i12], _mm_mul_pd(r3, _mm_set1_pd(sphi[i12])));
        ...
        if ((cphiTCP0.size(1) == sphiTEcrCP0.size(1)) && (i14 == r4.size(1))) {
            int m_loop_ub;
            ...
            for (int i15{0}; i15 < m_loop_ub; i15++) {
                __m128d r5;
                __m128d r6;
                r5 = _mm_loadu_pd(&cphiTCP0[3 * i15]);
                r6 = _mm_loadu_pd(&sphiTEcrCP0[3 * i15]);
                _mm_storeu_pd(
                    &b[3 * i15],
                    _mm_add_pd(
                        _mm_add_pd(r5, r6),
        ...
        if (sphiTCP0.size(1) == cphiTEcrCP0.size(1)) {
            ...
            for (int i17{0}; i17 < n_loop_ub; i17++) {
                __m128d r8;
                __m128d r9;
                r8 = _mm_loadu_pd(&sphiTCP0[3 * i17]);
                r9 = _mm_loadu_pd(&cphiTEcrCP0[3 * i17]);
                _mm_storeu_pd(&c_a[3 * i17],
        ...
        if (cphiTCP0.size(1) == sphiTEcrCP0.size(1)) {
            ...
            for (int i19{0}; i19 < o_loop_ub; i19++) {
                __m128d r12;
                __m128d r13;
                r12 = _mm_loadu_pd(&cphiTCP0[3 * i19]);
                r13 = _mm_loadu_pd(&sphiTEcrCP0[3 * i19]);
                _mm_storeu_pd(&r2D[3 * i19], _mm_sub_pd(_mm_mul_pd(_mm_set1_pd(-a_tmp), r12),
        ...
        if (sphiTCP0.size(1) == cphiTEcrCP0.size(1)) {
            ...
            for (int i20{0}; i20 < p_loop_ub; i20++) {
                ...
                __m128d r15;
                __m128d r16;
                r14 = _mm_loadu_pd(&sphiTCP0[3 * i20]);
                r15 = _mm_loadu_pd(&cphiTEcrCP0[3 * i20]);
                r16 = _mm_set1_pd(b_a_tmp);
                _mm_storeu_pd(&r3D[3 * i20],
