                    ...
                    ::coder::array<double, 1U> &beq)
{
    ...
    for (int i8{0}; i8 < d_loop_ub; i8++) {
        ...
        for (i10 = 0; i10 <= vectorUB; i10 += 2) {
            ...
            _mm_storeu_pd(&R1[i10 + R1.size(0) * i8],
                          _mm_add_pd(r4, _mm_mul_pd(_mm_set1_pd(0.5), r3)));
        }
        for (i10 = scalarLB; i10 < f_loop_ub; i10++) {
            R1[i10 + R1.size(0) * i8] = r1[i10 + r1.size(0) * i8] + 0.5 * r2[i10 + r2.size(0) * i8];
        }
    }
    ...
    for (int i12{0}; i12 < h_loop_ub; i12++) {
        ...
        for (i14 = 0; i14 <= b_vectorUB; i14 += 2) {
            ...
            _mm_storeu_pd(&R2[i14 + R2.size(0) * i12],
                          _mm_add_pd(r6, _mm_mul_pd(_mm_set1_pd(0.5), r5)));
        }
        for (i14 = b_scalarLB; i14 < j_loop_ub; i14++) {
            R2[i14 + R2.size(0) * i12] =
                r1[i14 + r1.size(0) * i12] + 0.5 * r2[i14 + r2.size(0) * i12];
        }
    }
    ...
    for (int i16{0}; i16 < l_loop_ub; i16++) {
        ...
        for (i18 = 0; i18 <= c_vectorUB; i18 += 2) {
            ...
            _mm_storeu_pd(&R3[i18 + R3.size(0) * i16],
                          _mm_add_pd(r8, _mm_mul_pd(_mm_set1_pd(0.5), r7)));
        }
        for (i18 = c_scalarLB; i18 < m_loop_ub; i18++) {
            R3[i18 + R3.size(0) * i16] =
                r1[i18 + r1.size(0) * i16] + 0.5 * r2[i18 + r2.size(0) * i16];
        }
    }
    //
    ...
    for (int i17{0}; i17 < n_loop_ub; i17++) {
        ...
        for (i20 = 0; i20 <= d_vectorUB; i20 += 2) {
            ...
            _mm_storeu_pd(&varargin_3[i20 + varargin_3.size(0) * i17],
    ...
    for (int i19{0}; i19 < p_loop_ub; i19++) {
        ...
        for (i22 = 0; i22 <= e_vectorUB; i22 += 2) {
            ...
            _mm_storeu_pd(&varargin_5[i22 + varargin_5.size(0) * i19],
    ...
    for (int i21{0}; i21 < r_loop_ub; i21++) {
        ...
        for (i23 = 0; i23 <= f_vectorUB; i23 += 2) {
            ...
            _mm_storeu_pd(&varargin_7[i23 + varargin_7.size(0) * i21],
    ...
    for (i45 = 0; i45 <= g_vectorUB; i45 += 2) {
        __m128d r12;
        r12 = _mm_loadu_pd(&r1D_sqnorm[i45]);
        _mm_storeu_pd(&b[i45], _mm_div_pd(_mm_set1_pd(x), r12));
    }
    for (i45 = g_scalarLB; i45 < y_loop_ub; i45++) {
    ...
    r13 = _mm_loadu_pd(&r1D[0]);
    _mm_storeu_pd(&c_r1D[0], _mm_div_pd(r13, _mm_set1_pd(d)));
    c_r1D[2] = r1D[2] / d;
    ...
    coder::internal::blas::mtimes(c_r1D, g_a, r14);
    ...
    for (int c_k{0}; c_k <= i57 - 2; c_k++) {
        ...
        r15.set_size(3, r1Dn.size(1));
        c_N = r1Dn.size(1);
        for (int e_k{0}; e_k < c_N; e_k++) {
            r15[3 * e_k] = std::pow(r1Dn[3 * e_k], 2.0);
            r15[3 * e_k + 1] = std::pow(r1Dn[3 * e_k + 1], 2.0);
            r15[3 * e_k + 2] = std::pow(r1Dn[3 * e_k + 2], 2.0);
        }
        __m128d r17;
        double d3;
        int e_r1D;
        int g_r1D;
        coder::sum(r15, r1D_sqnorm);
        //  squared norm
        b_x = std::pow(CurvStructs[c_k + 1].FeedRate, 2.0);
        e_r1D = r1D.size(1);
        g_r1D = r1D.size(1);
        d3 = coder::b_norm(*(double(*)[3]) & r1D[3 * (g_r1D - 1)]);
        r17 = _mm_loadu_pd(&r1D[3 * (e_r1D - 1)]);
        _mm_storeu_pd(&t_1[0], _mm_div_pd(r17, _mm_set1_pd(d3)));
        ...
        for (int i68{0}; i68 < yb_loop_ub; i68++) {
            ...
            for (i71 = 0; i71 <= h_vectorUB; i71 += 2) {
                __m128d r18;
                __m128d r19;
                r18 = _mm_loadu_pd(&r2[i71 + r2.size(0) * i68]);
                r19 = _mm_loadu_pd(&r1[i71 + r1.size(0) * i68]);
                _mm_storeu_pd(&R1[i71 + R1.size(0) * i68],
                              _mm_add_pd(r19, _mm_mul_pd(_mm_set1_pd(0.5), r18)));
            }
            for (i71 = h_scalarLB; i71 < cc_loop_ub; i71++) {
                R1[i71 + R1.size(0) * i68] =
                    r1[i71 + r1.size(0) * i68] + 0.5 * r2[i71 + r2.size(0) * i68];
            }
        }
        bc_loop_ub = r2Dn.size(1);
        b_r2Dn.set_size(r2Dn.size(1));
        for (int i70{0}; i70 < bc_loop_ub; i70++) {
            b_r2Dn[i70] = r2Dn[3 * i70 + 1];
        }
        coder::bsxfun(b_r2Dn, BasisVal, r1);
        ...
        for (int i73{0}; i73 < ec_loop_ub; i73++) {
            ...
            for (i75 = 0; i75 <= i_vectorUB; i75 += 2) {
                __m128d r20;
                __m128d r21;
                r20 = _mm_loadu_pd(&r2[i75 + r2.size(0) * i73]);
                r21 = _mm_loadu_pd(&r1[i75 + r1.size(0) * i73]);
                _mm_storeu_pd(&R2[i75 + R2.size(0) * i73],
                              _mm_add_pd(r21, _mm_mul_pd(_mm_set1_pd(0.5), r20)));
            }
            for (i75 = i_scalarLB; i75 < gc_loop_ub; i75++) {
                R2[i75 + R2.size(0) * i73] =
                    r1[i75 + r1.size(0) * i73] + 0.5 * r2[i75 + r2.size(0) * i73];
            }
        }
        fc_loop_ub = r2Dn.size(1);
        b_r2Dn.set_size(r2Dn.size(1));
        for (int i74{0}; i74 < fc_loop_ub; i74++) {
            b_r2Dn[i74] = r2Dn[3 * i74 + 2];
        }
        coder::bsxfun(b_r2Dn, BasisVal, r1);
        ...
        for (int i77{0}; i77 < ic_loop_ub; i77++) {
            ...
            for (i79 = 0; i79 <= j_vectorUB; i79 += 2) {
                __m128d r22;
                __m128d r23;
                r22 = _mm_loadu_pd(&r2[i79 + r2.size(0) * i77]);
                r23 = _mm_loadu_pd(&r1[i79 + r1.size(0) * i77]);
                _mm_storeu_pd(&R3[i79 + R3.size(0) * i77],
                              _mm_add_pd(r23, _mm_mul_pd(_mm_set1_pd(0.5), r22)));
            }
            for (i79 = j_scalarLB; i79 < jc_loop_ub; i79++) {
                R3[i79 + R3.size(0) * i77] =
                    r1[i79 + r1.size(0) * i77] + 0.5 * r2[i79 + r2.size(0) * i77];
            }
        }
        //
        varargin_3.set_size(R1.size(0), R1.size(1));
        ...
        for (int i78{0}; i78 < kc_loop_ub; i78++) {
            ...
            k_scalarLB = (R1.size(0) / 2) << 1;
            k_vectorUB = k_scalarLB - 2;
            for (i81 = 0; i81 <= k_vectorUB; i81 += 2) {
                __m128d r24;
                r24 = _mm_loadu_pd(&R1[i81 + R1.size(0) * i78]);
                _mm_storeu_pd(&varargin_3[i81 + varargin_3.size(0) * i78],
                              _mm_mul_pd(r24, _mm_set1_pd(-1.0)));
            }
            for (i81 = k_scalarLB; i81 < lc_loop_ub; i81++) {
                varargin_3[i81 + varargin_3.size(0) * i78] = -R1[i81 + R1.size(0) * i78];
            }
        }
        varargin_5.set_size(R2.size(0), R2.size(1));
        mc_loop_ub = R2.size(1);
        for (int i80{0}; i80 < mc_loop_ub; i80++) {
            int i83;
            int l_scalarLB;
            int l_vectorUB;
            int nc_loop_ub;
            ...
            for (i83 = 0; i83 <= l_vectorUB; i83 += 2) {
                __m128d r25;
                r25 = _mm_loadu_pd(&R2[i83 + R2.size(0) * i80]);
                _mm_storeu_pd(&varargin_5[i83 + varargin_5.size(0) * i80],
                              _mm_mul_pd(r25, _mm_set1_pd(-1.0)));
            }
            for (i83 = l_scalarLB; i83 < nc_loop_ub; i83++) {
                varargin_5[i83 + varargin_5.size(0) * i80] = -R2[i83 + R2.size(0) * i80];
            }
        }
        varargin_7.set_size(R3.size(0), R3.size(1));
        ...
        for (int i82{0}; i82 < oc_loop_ub; i82++) {
            ...
            m_scalarLB = (R3.size(0) / 2) << 1;
            m_vectorUB = m_scalarLB - 2;
            for (i84 = 0; i84 <= m_vectorUB; i84 += 2) {
                __m128d r26;
                r26 = _mm_loadu_pd(&R3[i84 + R3.size(0) * i82]);
                _mm_storeu_pd(&varargin_7[i84 + varargin_7.size(0) * i82],
                              _mm_mul_pd(r26, _mm_set1_pd(-1.0)));
            }
            for (i84 = m_scalarLB; i84 < pc_loop_ub; i84++) {
                varargin_7[i84 + varargin_7.size(0) * i82] = -R3[i84 + R3.size(0) * i82];
            }
        }
        if ((BasisVal.size(0) != 0) && (BasisVal.size(1) != 0)) {
        ...
        coder::internal::blas::mtimes(t_1, g_a, r14);
        wc_loop_ub = r14.size(1);
        for (int i116{0}; i116 < wc_loop_ub; i116++) {
            Aeq[(i110 + Aeq.size(0) * ((i114 + i116) - 1)) + 1] = r14[i116];
        }
        //
        w_a = r2Dn[0];
        x_a = 0.5 * r1Dn[0];
        ...
        coder::internal::blas::mtimes(t_1, g_a, r14);
        yc_loop_ub = r14.size(1);
        for (int i119{0}; i119 < yc_loop_ub; i119++) {
            Aeq[(i110 + Aeq.size(0) * ((i117 + i119) - 1)) + 1] = -r14[i119];
        }
        //
    ...
    r16 = _mm_loadu_pd(&r1D[3 * (d_r1D - 1)]);
    _mm_storeu_pd(&c_r1D[0], _mm_div_pd(r16, _mm_set1_pd(d2)));
    ...
    ac_loop_ub = r14.size(1);
    for (int i69{0}; i69 < ac_loop_ub; i69++) {
        Aeq[unnamed_idx_1 + Aeq.size(0) * (i61 + i69)] = r14[i69];
    }
    int b_beq;
    //
    b_beq = beq.size(0) - 1;
