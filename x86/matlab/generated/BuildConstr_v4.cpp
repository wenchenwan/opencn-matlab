                    ...
                    ::coder::array<double, 1U> &beq)
{
    ...
    if ((r1.size(0) == r2.size(0)) && (r1.size(1) == r2.size(1))) {
        ...
        for (int i9{0}; i9 < e_loop_ub; i9++) {
            ...
            for (i11 = 0; i11 <= vectorUB; i11 += 2) {
                ...
                _mm_storeu_pd(&R1[i11 + R1.size(0) * i9],
    ...
    if ((r1.size(0) == r2.size(0)) && (r1.size(1) == r2.size(1))) {
        ...
        for (int i13{0}; i13 < i_loop_ub; i13++) {
            ...
            for (i15 = 0; i15 <= b_vectorUB; i15 += 2) {
                ...
                _mm_storeu_pd(&R2[i15 + R2.size(0) * i13],
    ...
    for (int i20{0}; i20 < q_loop_ub; i20++) {
        ...
        for (i23 = 0; i23 <= e_vectorUB; i23 += 2) {
            ...
            _mm_storeu_pd(&varargin_5[i23 + varargin_5.size(0) * i20],
    ...
    for (int i22{0}; i22 < s_loop_ub; i22++) {
        ...
        for (i24 = 0; i24 <= f_vectorUB; i24 += 2) {
            ...
            _mm_storeu_pd(&varargin_7[i24 + varargin_7.size(0) * i22],
    ...
    for (i46 = 0; i46 <= g_vectorUB; i46 += 2) {
        __m128d r12;
        r12 = _mm_loadu_pd(&r1D_sqnorm[i46]);
        _mm_storeu_pd(&b[i46], _mm_div_pd(_mm_set1_pd(x), r12));
    }
    ...
    for (int b_k{0}; b_k <= i57 - 2; b_k++) {
        ...
        if (ctx_cfg_UseDynamicBreakpoints) {
            double varargin_2;
            varargin_2 =
                LengthCurv(ctx_q_splines, ctx_cfg_NGridLengthSpline, CurvStructs[b_k + 1].Type,
                           CurvStructs[b_k + 1].P0, CurvStructs[b_k + 1].P1,
                           CurvStructs[b_k + 1].CorrectedHelixCenter, CurvStructs[b_k + 1].evec,
        ...
        if ((r1.size(0) == r2.size(0)) && (r1.size(1) == r2.size(1))) {
            ...
            for (int i71{0}; i71 < cc_loop_ub; i71++) {
                ...
                for (i73 = 0; i73 <= h_vectorUB; i73 += 2) {
                    __m128d r17;
                    __m128d r18;
                    r17 = _mm_loadu_pd(&r2[i73 + r2.size(0) * i71]);
                    r18 = _mm_loadu_pd(&r1[i73 + r1.size(0) * i71]);
                    _mm_storeu_pd(&R1[i73 + R1.size(0) * i71],
                                  _mm_add_pd(r18, _mm_mul_pd(_mm_set1_pd(0.5), r17)));
                }
                for (i73 = h_scalarLB; i73 < ec_loop_ub; i73++) {
        ...
        if ((r1.size(0) == r2.size(0)) && (r1.size(1) == r2.size(1))) {
            ...
            for (int i75{0}; i75 < gc_loop_ub; i75++) {
                ...
                for (i77 = 0; i77 <= i_vectorUB; i77 += 2) {
                    __m128d r19;
                    __m128d r20;
                    r19 = _mm_loadu_pd(&r2[i77 + r2.size(0) * i75]);
                    r20 = _mm_loadu_pd(&r1[i77 + r1.size(0) * i75]);
                    _mm_storeu_pd(&R2[i77 + R2.size(0) * i75],
                                  _mm_add_pd(r20, _mm_mul_pd(_mm_set1_pd(0.5), r19)));
                }
                for (i77 = i_scalarLB; i77 < ic_loop_ub; i77++) {
        ...
        for (int i82{0}; i82 < oc_loop_ub; i82++) {
            ...
            for (i85 = 0; i85 <= l_vectorUB; i85 += 2) {
                __m128d r24;
                r24 = _mm_loadu_pd(&R2[i85 + R2.size(0) * i82]);
                _mm_storeu_pd(&varargin_5[i85 + varargin_5.size(0) * i82],
                              _mm_mul_pd(r24, _mm_set1_pd(-1.0)));
            }
            for (i85 = l_scalarLB; i85 < pc_loop_ub; i85++) {
                varargin_5[i85 + varargin_5.size(0) * i82] = -R2[i85 + R2.size(0) * i82];
            }
        }
        varargin_7.set_size(R3.size(0), R3.size(1));
        ...
        for (int i84{0}; i84 < qc_loop_ub; i84++) {
            ...
            for (i86 = 0; i86 <= m_vectorUB; i86 += 2) {
                __m128d r25;
                r25 = _mm_loadu_pd(&R3[i86 + R3.size(0) * i84]);
                _mm_storeu_pd(&varargin_7[i86 + varargin_7.size(0) * i84],
                              _mm_mul_pd(r25, _mm_set1_pd(-1.0)));
            }
            for (i86 = m_scalarLB; i86 < rc_loop_ub; i86++) {
                varargin_7[i86 + varargin_7.size(0) * i84] = -R3[i86 + R3.size(0) * i84];
            }
        }
        if ((BasisVal.size(0) != 0) && (BasisVal.size(1) != 0)) {
        ...
        n_scalarLB = (r1D_sqnorm.size(1) / 2) << 1;
        n_vectorUB = n_scalarLB - 2;
        for (i104 = 0; i104 <= n_vectorUB; i104 += 2) {
            __m128d r26;
            r26 = _mm_loadu_pd(&r1D_sqnorm[i104]);
            _mm_storeu_pd(&b[i103 + i104], _mm_div_pd(_mm_set1_pd(b_x), r26));
        }
        for (i104 = n_scalarLB; i104 < uc_loop_ub; i104++) {
        ...
        if (BasisVal.size(1) == BasisValD.size(1)) {
            ...
            coder::internal::blas::mtimes(t_1, g_a, r27);
            yc_loop_ub = r27.size(1);
            for (int i117{0}; i117 < yc_loop_ub; i117++) {
                Aeq[(static_cast<int>(u) + Aeq.size(0) * (i115 + i117)) + 1] = r27[i117];
            }
        } else {
