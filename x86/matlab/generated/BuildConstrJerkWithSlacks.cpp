                               ...
                               ::coder::array<double, 1U> &b)
{
    ...
    if ((r2.size(0) == r3.size(0)) && (r2.size(1) == r3.size(1)) && (i6 == r4.size(0)) &&
        (i7 == r4.size(1))) {
        int e_loop_ub;
        e_loop_ub = r2.size(1);
        for (int i8{0}; i8 < e_loop_ub; i8++) {
            ...
            for (i9 = 0; i9 <= c_vectorUB; i9 += 2) {
                ...
                __m128d r6;
                __m128d r7;
                r5 = _mm_loadu_pd(&r3[i9 + r3.size(0) * i8]);
                r6 = _mm_loadu_pd(&r2[i9 + r2.size(0) * i8]);
                r7 = _mm_loadu_pd(&r4[i9 + r4.size(0) * i8]);
                _mm_storeu_pd(&r2[i9 + r2.size(0) * i8],
                              _mm_add_pd(_mm_add_pd(r6, _mm_mul_pd(_mm_set1_pd(1.5), r5)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r7)));
            }
            for (i9 = c_scalarLB; i9 < f_loop_ub; i9++) {
                r2[i9 + r2.size(0) * i8] =
                    (r2[i9 + r2.size(0) * i8] + 1.5 * r3[i9 + r3.size(0) * i8]) +
                    0.5 * r4[i9 + r4.size(0) * i8];
            }
        }
    ...
    if ((r2.size(0) == r3.size(0)) && (r2.size(1) == r3.size(1)) && (i13 == r4.size(0)) &&
        (i14 == r4.size(1))) {
        int j_loop_ub;
        j_loop_ub = r2.size(1);
        for (int i15{0}; i15 < j_loop_ub; i15++) {
            ...
            for (i16 = 0; i16 <= d_vectorUB; i16 += 2) {
                __m128d r10;
                __m128d r8;
                __m128d r9;
                r8 = _mm_loadu_pd(&r3[i16 + r3.size(0) * i15]);
                r9 = _mm_loadu_pd(&r2[i16 + r2.size(0) * i15]);
                r10 = _mm_loadu_pd(&r4[i16 + r4.size(0) * i15]);
                _mm_storeu_pd(&r2[i16 + r2.size(0) * i15],
                              _mm_add_pd(_mm_add_pd(r9, _mm_mul_pd(_mm_set1_pd(1.5), r8)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r10)));
            }
            for (i16 = d_scalarLB; i16 < k_loop_ub; i16++) {
                r2[i16 + r2.size(0) * i15] =
                    (r2[i16 + r2.size(0) * i15] + 1.5 * r3[i16 + r3.size(0) * i15]) +
                    0.5 * r4[i16 + r4.size(0) * i15];
            }
        }
    ...
    if ((r2.size(0) == r3.size(0)) && (r2.size(1) == r3.size(1)) && (i20 == r4.size(0)) &&
        (i21 == r4.size(1))) {
        int o_loop_ub;
        o_loop_ub = r2.size(1);
        for (int i22{0}; i22 < o_loop_ub; i22++) {
            ...
            for (i23 = 0; i23 <= e_vectorUB; i23 += 2) {
                ...
                __m128d r12;
                __m128d r13;
                r11 = _mm_loadu_pd(&r3[i23 + r3.size(0) * i22]);
                r12 = _mm_loadu_pd(&r2[i23 + r2.size(0) * i22]);
                r13 = _mm_loadu_pd(&r4[i23 + r4.size(0) * i22]);
                _mm_storeu_pd(&r2[i23 + r2.size(0) * i22],
                              _mm_add_pd(_mm_add_pd(r12, _mm_mul_pd(_mm_set1_pd(1.5), r11)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r13)));
            }
            for (i23 = e_scalarLB; i23 < p_loop_ub; i23++) {
                r2[i23 + r2.size(0) * i22] =
                    (r2[i23 + r2.size(0) * i22] + 1.5 * r3[i23 + r3.size(0) * i22]) +
                    0.5 * r4[i23 + r4.size(0) * i22];
            }
        }
    ...
    for (int i24{0}; i24 < q_loop_ub; i24++) {
        ...
        for (i26 = 0; i26 <= f_vectorUB; i26 += 2) {
            __m128d r14;
            r14 = _mm_loadu_pd(&R1[i26 + R1.size(0) * i24]);
            _mm_storeu_pd(&varargin_2[i26 + varargin_2.size(0) * i24],
                          _mm_mul_pd(r14, _mm_set1_pd(-1.0)));
        }
        for (i26 = f_scalarLB; i26 < r_loop_ub; i26++) {
            varargin_2[i26 + varargin_2.size(0) * i24] = -R1[i26 + R1.size(0) * i24];
        }
    }
    ...
    for (int i25{0}; i25 < s_loop_ub; i25++) {
        ...
        for (i28 = 0; i28 <= g_vectorUB; i28 += 2) {
            __m128d r15;
            r15 = _mm_loadu_pd(&R2[i28 + R2.size(0) * i25]);
            _mm_storeu_pd(&varargin_4[i28 + varargin_4.size(0) * i25],
                          _mm_mul_pd(r15, _mm_set1_pd(-1.0)));
        }
        for (i28 = g_scalarLB; i28 < t_loop_ub; i28++) {
            varargin_4[i28 + varargin_4.size(0) * i25] = -R2[i28 + R2.size(0) * i25];
        }
    }
    ...
    for (int i27{0}; i27 < u_loop_ub; i27++) {
        ...
        for (i29 = 0; i29 <= h_vectorUB; i29 += 2) {
            __m128d r16;
            r16 = _mm_loadu_pd(&R3[i29 + R3.size(0) * i27]);
            _mm_storeu_pd(&varargin_6[i29 + varargin_6.size(0) * i27],
                          _mm_mul_pd(r16, _mm_set1_pd(-1.0)));
        }
        for (i29 = h_scalarLB; i29 < v_loop_ub; i29++) {
            varargin_6[i29 + varargin_6.size(0) * i27] = -R3[i29 + R3.size(0) * i27];
        }
    }
    ...
    for (int i46{0}; i46 < ab_loop_ub; i46++) {
        r17[i46 + BasisVal_idx_0] = jmax[0];
    }
    bb_loop_ub = BasisVal.size(0);
    for (int i47{0}; i47 < bb_loop_ub; i47++) {
        r17[(i47 + BasisVal_idx_0) + b_BasisVal_idx_0] = jmax[1];
    }
    cb_loop_ub = BasisVal.size(0);
    for (int i48{0}; i48 < cb_loop_ub; i48++) {
        r17[((i48 + BasisVal_idx_0) + b_BasisVal_idx_0) + c_BasisVal_idx_0] = jmax[1];
    }
    db_loop_ub = BasisVal.size(0);
    for (int i49{0}; i49 < db_loop_ub; i49++) {
        r17[(((i49 + BasisVal_idx_0) + b_BasisVal_idx_0) + c_BasisVal_idx_0) + d_BasisVal_idx_0] =
            jmax[2];
    }
    ...
    for (int c_k{0}; c_k <= i52 - 2; c_k++) {
        ...
        for (int i77{0}; i77 < xb_loop_ub; i77++) {
            ...
            for (i79 = 0; i79 <= n_vectorUB; i79 += 2) {
                ...
                _mm_storeu_pd(&varargin_2[i79 + varargin_2.size(0) * i77],
                              _mm_mul_pd(r29, _mm_set1_pd(-1.0)));
            }
            for (i79 = n_scalarLB; i79 < yb_loop_ub; i79++) {
                varargin_2[i79 + varargin_2.size(0) * i77] = -R1[i79 + R1.size(0) * i77];
            }
        }
        varargin_4.set_size(R2.size(0), R2.size(1));
