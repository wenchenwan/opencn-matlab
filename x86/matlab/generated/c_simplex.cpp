               ...
               int *status)
{
    ...
    c_loop_ub = ii.size(0);
    scalarLB = (ii.size(0) / 4) << 2;
    vectorUB = scalarLB - 4;
    for (i2 = 0; i2 <= vectorUB; i2 += 4) {
        __m128i r;
        r = _mm_loadu_si128((const __m128i *)&ii[i2]);
    ...
    for (i3 = 0; i3 <= b_vectorUB; i3 += 4) {
        __m128i r1;
        r1 = _mm_loadu_si128((const __m128i *)&jj[i3]);
        _mm_storeu_si128((__m128i *)&Ajs[i3], _mm_sub_epi32(r1, _mm_set1_epi32(1)));
    }
