                        ...
                        ::coder::array<double, 1U> &b)
{
    ...
    for (int k{0}; k < inner; k++) {
        ...
        for (c_i = 0; c_i <= vectorUB; c_i += 2) {
            ...
            _mm_storeu_pd(
                &y_tmp[c_i],
                _mm_add_pd(r,
                           _mm_mul_pd(_mm_loadu_pd((double *)&BasisVal[c_i + BasisVal.size(0) * k]),
                                      _mm_set1_pd(Coeff[k]))));
        }
    ...
    for (int i7{0}; i7 < f_loop_ub; i7++) {
        ...
        for (i8 = 0; i8 <= c_vectorUB; i8 += 2) {
            ...
            __m128d r6;
            __m128d r7;
            r5 = _mm_loadu_pd(&r3[i8 + r3.size(0) * i7]);
            r6 = _mm_loadu_pd(&r2[i8 + r2.size(0) * i7]);
            r7 = _mm_loadu_pd(&r4[i8 + r4.size(0) * i7]);
            _mm_storeu_pd(&r2[i8 + r2.size(0) * i7],
                          _mm_add_pd(_mm_add_pd(r6, _mm_mul_pd(_mm_set1_pd(1.5), r5)),
                                     _mm_mul_pd(_mm_set1_pd(0.5), r7)));
        }
        for (i8 = c_scalarLB; i8 < g_loop_ub; i8++) {
            r2[i8 + r2.size(0) * i7] = (r2[i8 + r2.size(0) * i7] + 1.5 * r3[i8 + r3.size(0) * i7]) +
                                       0.5 * r4[i8 + r4.size(0) * i7];
        }
    }
    ...
    for (int i12{0}; i12 < k_loop_ub; i12++) {
        ...
        for (i13 = 0; i13 <= d_vectorUB; i13 += 2) {
            ...
            __m128d r8;
            __m128d r9;
            r8 = _mm_loadu_pd(&r3[i13 + r3.size(0) * i12]);
            r9 = _mm_loadu_pd(&r2[i13 + r2.size(0) * i12]);
            r10 = _mm_loadu_pd(&r4[i13 + r4.size(0) * i12]);
            _mm_storeu_pd(&r2[i13 + r2.size(0) * i12],
                          _mm_add_pd(_mm_add_pd(r9, _mm_mul_pd(_mm_set1_pd(1.5), r8)),
                                     _mm_mul_pd(_mm_set1_pd(0.5), r10)));
        }
        for (i13 = d_scalarLB; i13 < l_loop_ub; i13++) {
            r2[i13 + r2.size(0) * i12] =
                (r2[i13 + r2.size(0) * i12] + 1.5 * r3[i13 + r3.size(0) * i12]) +
                0.5 * r4[i13 + r4.size(0) * i12];
        }
    }
    ...
    for (int i17{0}; i17 < p_loop_ub; i17++) {
        ...
        for (i18 = 0; i18 <= e_vectorUB; i18 += 2) {
            __m128d r11;
            __m128d r12;
            __m128d r13;
            r11 = _mm_loadu_pd(&r3[i18 + r3.size(0) * i17]);
            r12 = _mm_loadu_pd(&r2[i18 + r2.size(0) * i17]);
            r13 = _mm_loadu_pd(&r4[i18 + r4.size(0) * i17]);
            _mm_storeu_pd(&r2[i18 + r2.size(0) * i17],
                          _mm_add_pd(_mm_add_pd(r12, _mm_mul_pd(_mm_set1_pd(1.5), r11)),
                                     _mm_mul_pd(_mm_set1_pd(0.5), r13)));
        }
        for (i18 = e_scalarLB; i18 < q_loop_ub; i18++) {
            r2[i18 + r2.size(0) * i17] =
                (r2[i18 + r2.size(0) * i17] + 1.5 * r3[i18 + r3.size(0) * i17]) +
                0.5 * r4[i18 + r4.size(0) * i17];
        }
    }
    ...
    for (int i19{0}; i19 < r_loop_ub; i19++) {
        ...
        for (i21 = 0; i21 <= f_vectorUB; i21 += 2) {
            __m128d r14;
            r14 = _mm_loadu_pd(&R1[i21 + R1.size(0) * i19]);
            _mm_storeu_pd(&b_varargin_2[i21 + b_varargin_2.size(0) * i19],
                          _mm_mul_pd(r14, _mm_set1_pd(-1.0)));
        }
        for (i21 = f_scalarLB; i21 < s_loop_ub; i21++) {
            b_varargin_2[i21 + b_varargin_2.size(0) * i19] = -R1[i21 + R1.size(0) * i19];
        }
    }
    ...
    for (int i20{0}; i20 < t_loop_ub; i20++) {
        ...
        for (i23 = 0; i23 <= g_vectorUB; i23 += 2) {
            __m128d r15;
            r15 = _mm_loadu_pd(&R2[i23 + R2.size(0) * i20]);
            _mm_storeu_pd(&varargin_4[i23 + varargin_4.size(0) * i20],
                          _mm_mul_pd(r15, _mm_set1_pd(-1.0)));
        }
        for (i23 = g_scalarLB; i23 < u_loop_ub; i23++) {
            varargin_4[i23 + varargin_4.size(0) * i20] = -R2[i23 + R2.size(0) * i20];
        }
    }
    ...
    for (int i22{0}; i22 < v_loop_ub; i22++) {
        ...
        for (i24 = 0; i24 <= h_vectorUB; i24 += 2) {
            __m128d r16;
            r16 = _mm_loadu_pd(&R3[i24 + R3.size(0) * i22]);
            _mm_storeu_pd(&varargin_6[i24 + varargin_6.size(0) * i22],
                          _mm_mul_pd(r16, _mm_set1_pd(-1.0)));
        }
        for (i24 = h_scalarLB; i24 < w_loop_ub; i24++) {
            varargin_6[i24 + varargin_6.size(0) * i22] = -R3[i24 + R3.size(0) * i22];
        }
    }
    ...
    for (int i41{0}; i41 < bb_loop_ub; i41++) {
        r17[i41 + BasisVal_idx_0] = jmax[0];
    }
    cb_loop_ub = BasisVal.size(0);
    for (int i42{0}; i42 < cb_loop_ub; i42++) {
        r17[(i42 + BasisVal_idx_0) + b_BasisVal_idx_0] = jmax[1];
    }
    db_loop_ub = BasisVal.size(0);
    for (int i43{0}; i43 < db_loop_ub; i43++) {
        r17[((i43 + BasisVal_idx_0) + b_BasisVal_idx_0) + c_BasisVal_idx_0] = jmax[1];
    }
    eb_loop_ub = BasisVal.size(0);
    for (int i44{0}; i44 < eb_loop_ub; i44++) {
        r17[(((i44 + BasisVal_idx_0) + b_BasisVal_idx_0) + c_BasisVal_idx_0) + d_BasisVal_idx_0] =
            jmax[2];
    }
    fb_loop_ub = BasisVal.size(0);
    for (int i45{0}; i45 < fb_loop_ub; i45++) {
        r17[((((i45 + BasisVal_idx_0) + b_BasisVal_idx_0) + c_BasisVal_idx_0) + d_BasisVal_idx_0) +
    ...
    for (int c_k{0}; c_k <= i47 - 2; c_k++) {
        ...
        for (int d_k{0}; d_k < b_inner; d_k++) {
            ...
            for (e_i = 0; e_i <= i_vectorUB; e_i += 2) {
                ...
                _mm_storeu_pd(
                    ...
                    _mm_add_pd(
                        r18,
                        _mm_mul_pd(_mm_loadu_pd((double *)&BasisVal[e_i + BasisVal.size(0) * d_k]),
        ...
        for (e_k = j_scalarLB; e_k < i49; e_k++) {
        ...
        for (int i53{0}; i53 < lb_loop_ub; i53++) {
            ...
            for (i54 = 0; i54 <= k_vectorUB; i54 += 2) {
                __m128d r20;
                __m128d r21;
                __m128d r22;
                r20 = _mm_loadu_pd(&r3[i54 + r3.size(0) * i53]);
                r21 = _mm_loadu_pd(&r2[i54 + r2.size(0) * i53]);
                r22 = _mm_loadu_pd(&r4[i54 + r4.size(0) * i53]);
                _mm_storeu_pd(&r2[i54 + r2.size(0) * i53],
                              _mm_add_pd(_mm_add_pd(r21, _mm_mul_pd(_mm_set1_pd(1.5), r20)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r22)));
            }
            for (i54 = k_scalarLB; i54 < mb_loop_ub; i54++) {
                r2[i54 + r2.size(0) * i53] =
                    (r2[i54 + r2.size(0) * i53] + 1.5 * r3[i54 + r3.size(0) * i53]) +
                    0.5 * r4[i54 + r4.size(0) * i53];
            }
        }
        coder::bsxfun(r2, y_tmp, R1);
        sqrt_calls++;
        ...
        for (int i58{0}; i58 < qb_loop_ub; i58++) {
            ...
            for (i59 = 0; i59 <= l_vectorUB; i59 += 2) {
                __m128d r23;
                __m128d r24;
                __m128d r25;
                r23 = _mm_loadu_pd(&r3[i59 + r3.size(0) * i58]);
                r24 = _mm_loadu_pd(&r2[i59 + r2.size(0) * i58]);
                r25 = _mm_loadu_pd(&r4[i59 + r4.size(0) * i58]);
                _mm_storeu_pd(&r2[i59 + r2.size(0) * i58],
                              _mm_add_pd(_mm_add_pd(r24, _mm_mul_pd(_mm_set1_pd(1.5), r23)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r25)));
            }
            for (i59 = l_scalarLB; i59 < rb_loop_ub; i59++) {
                r2[i59 + r2.size(0) * i58] =
                    (r2[i59 + r2.size(0) * i58] + 1.5 * r3[i59 + r3.size(0) * i58]) +
                    0.5 * r4[i59 + r4.size(0) * i58];
            }
        }
        coder::bsxfun(r2, y_tmp, R2);
        sqrt_calls++;
        ...
        for (int i63{0}; i63 < vb_loop_ub; i63++) {
            ...
            for (i64 = 0; i64 <= m_vectorUB; i64 += 2) {
                __m128d r26;
                __m128d r27;
                __m128d r28;
                r26 = _mm_loadu_pd(&r3[i64 + r3.size(0) * i63]);
                r27 = _mm_loadu_pd(&r2[i64 + r2.size(0) * i63]);
                r28 = _mm_loadu_pd(&r4[i64 + r4.size(0) * i63]);
                _mm_storeu_pd(&r2[i64 + r2.size(0) * i63],
                              _mm_add_pd(_mm_add_pd(r27, _mm_mul_pd(_mm_set1_pd(1.5), r26)),
                                         _mm_mul_pd(_mm_set1_pd(0.5), r28)));
            }
            for (i64 = m_scalarLB; i64 < wb_loop_ub; i64++) {
                r2[i64 + r2.size(0) * i63] =
                    (r2[i64 + r2.size(0) * i63] + 1.5 * r3[i64 + r3.size(0) * i63]) +
                    0.5 * r4[i64 + r4.size(0) * i63];
            }
        }
        coder::bsxfun(r2, y_tmp, R3);
        //      R2 = (BasisVal .* r3D(2, :)' + ...
        ...
        for (int i65{0}; i65 < xb_loop_ub; i65++) {
            ...
            for (i67 = 0; i67 <= n_vectorUB; i67 += 2) {
                __m128d r29;
                r29 = _mm_loadu_pd(&R1[i67 + R1.size(0) * i65]);
                _mm_storeu_pd(&b_varargin_2[i67 + b_varargin_2.size(0) * i65],
                              _mm_mul_pd(r29, _mm_set1_pd(-1.0)));
            }
            for (i67 = n_scalarLB; i67 < yb_loop_ub; i67++) {
                b_varargin_2[i67 + b_varargin_2.size(0) * i65] = -R1[i67 + R1.size(0) * i65];
            }
        }
        varargin_4.set_size(R2.size(0), R2.size(1));
        ac_loop_ub = R2.size(1);
        for (int i66{0}; i66 < ac_loop_ub; i66++) {
            int bc_loop_ub;
            int i69;
            int o_scalarLB;
            int o_vectorUB;
            ...
            for (i69 = 0; i69 <= o_vectorUB; i69 += 2) {
                __m128d r30;
                r30 = _mm_loadu_pd(&R2[i69 + R2.size(0) * i66]);
                _mm_storeu_pd(&varargin_4[i69 + varargin_4.size(0) * i66],
                              _mm_mul_pd(r30, _mm_set1_pd(-1.0)));
            }
            for (i69 = o_scalarLB; i69 < bc_loop_ub; i69++) {
                varargin_4[i69 + varargin_4.size(0) * i66] = -R2[i69 + R2.size(0) * i66];
            }
        }
        varargin_6.set_size(R3.size(0), R3.size(1));
        ...
        for (int i68{0}; i68 < cc_loop_ub; i68++) {
            ...
            for (i70 = 0; i70 <= p_vectorUB; i70 += 2) {
                __m128d r31;
                r31 = _mm_loadu_pd(&R3[i70 + R3.size(0) * i68]);
                _mm_storeu_pd(&varargin_6[i70 + varargin_6.size(0) * i68],
                              _mm_mul_pd(r31, _mm_set1_pd(-1.0)));
            }
            for (i70 = p_scalarLB; i70 < dc_loop_ub; i70++) {
                varargin_6[i70 + varargin_6.size(0) * i68] = -R3[i70 + R3.size(0) * i68];
            }
        }
        if ((R1.size(0) != 0) && (R1.size(1) != 0)) {
            b_result = R1.size(1);
