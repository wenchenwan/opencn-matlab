        ...
        for (int i4{0}; i4 < b_loop_ub; i4++) {
            J[i4 + J.size(0) * i3] = (r3D[i4 * stride_0_0 + r3D.size(0) * aux_0_1] * r[aux_1_1] +
                                      3.0 * r2D[i4 * stride_2_0 + r2D.size(0) * aux_2_1] *
                                          ud_vec[aux_3_1] * udd_vec[aux_4_1]) +
...
// Return Type  : void
//
void times(::coder::array<double, 2U> &V, const ::coder::array<double, 2U> &r1D,
           const ::coder::array<double, 2U> &ud_vec)
{
    int aux_0_1;
    int aux_1_1;
    ...
    for (int i1{0}; i1 < loop_ub; i1++) {
        ...
        for (i2 = 0; i2 <= vectorUB; i2 += 2) {
            ...
            _mm_storeu_pd(&V[i2 + V.size(0) * i1], _mm_mul_pd(r, _mm_set1_pd(ud_vec[aux_1_1])));
        }
        for (i2 = scalarLB; i2 < b_loop_ub; i2++) {
            V[i2 + V.size(0) * i1] = r1D[i2 + r1D.size(0) * aux_0_1] * ud_vec[aux_1_1];
        }
        aux_1_1 += stride_1_1;
        aux_0_1 += stride_0_1;
    }
