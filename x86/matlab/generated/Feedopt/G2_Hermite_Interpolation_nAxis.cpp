    ...
    if ((kappa0 == 0.0) && (kappa1 == 0.0)) {
        ...
        *alpha1 = X_idx_1;
            ...
            *status = 3;
            ...
            if (Idx_size > 1) {
                ...
                for (int b_k{0}; b_k < Idx_size; b_k++) {
                    ...
                    CostInt_data[b_k] = EvalCostIntegral(alpha0_t_data[i40 - 1], beta0_u_data[b_k],
                                                         alpha1_t_data[i40 - 1], beta1_u_data[b_k],
        ...
        for (i34 = 0; i34 <= d_vectorUB; i34 += 2) {
            ...
            _mm_storeu_pd(
                &t11_data[i34],
                _mm_div_pd(_mm_set1_pd(1.0), _mm_add_pd(_mm_set1_pd(u_CoefPS),
                                                        _mm_mul_pd(_mm_set1_pd(x_CoefPS), r33))));
        }
        for (i34 = d_scalarLB; i34 < trueCount; i34++) {
            t11_data[i34] = 1.0 / (u_CoefPS + x_CoefPS * alpha1_t_data[i34]);
        }
        // 'CalcAlpha0:34' t12 = c0+t4+t7+t8;
        r3.set_size(trueCount);
        for (int i38{0}; i38 < trueCount; i38++) {
            r3[i38] = std::pow(alpha1_t_data[i38], 2.0);
        }
        r9.set_size(trueCount);
        for (int i39{0}; i39 < trueCount; i39++) {
            r9[i39] = std::pow(alpha1_t_data[i39], 3.0);
        }
        if (trueCount == 1) {
        ...
        if ((trueCount == r3.size(0)) && (d_trueCount == r9.size(0))) {
            ...
            for (i44 = 0; i44 <= g_vectorUB; i44 += 2) {
                __m128d r42;
                __m128d r43;
                __m128d r44;
                r42 = _mm_loadu_pd(&alpha1_t_data[i44]);
                r43 = _mm_loadu_pd(&r3[i44]);
                r44 = _mm_loadu_pd(&r9[i44]);
                _mm_storeu_pd(
                    &t12_data[i44],
                    _mm_add_pd(_mm_add_pd(_mm_add_pd(_mm_set1_pd(ab_CoefPS),
                                                     _mm_mul_pd(r42, _mm_set1_pd(bb_CoefPS))),
                                          _mm_mul_pd(_mm_set1_pd(cb_CoefPS), r43)),
                               _mm_mul_pd(_mm_set1_pd(db_CoefPS), r44)));
            }
            for (i44 = g_scalarLB; i44 < trueCount; i44++) {
                t12_data[i44] =
                    ((ab_CoefPS + alpha1_t_data[i44] * bb_CoefPS) + cb_CoefPS * r3[i44]) +
                    db_CoefPS * r9[i44];
            }
        } else {
        ...
        for (int i42{0}; i42 < trueCount; i42++) {
            r3[i42] = std::pow(t11_data[i42], 2.0);
        }
        r9.set_size(trueCount);
        for (int i43{0}; i43 < trueCount; i43++) {
            r9[i43] = std::pow(t10_data[i43], 2.0);
        }
        if (trueCount == 1) {
            e_trueCount = t5_size;
        ...
        if ((trueCount == t5_size) && (trueCount == t12_size) && (e_trueCount == f_trueCount) &&
            ...
            (i_trueCount == k_trueCount) && (trueCount == t5_size) && (l_trueCount == trueCount) &&
            (r9.size(0) == trueCount) && (t12_size == i45) && (trueCount == b_t12_size) &&
            (n_trueCount == o_trueCount) && (t_trueCount == x_trueCount)) {
            double eb_CoefPS;
            double fb_CoefPS;
            double gb_CoefPS;
            double hb_CoefPS;
            ...
            for (i47 = 0; i47 <= h_vectorUB; i47 += 2) {
                ...
                r50 = _mm_loadu_pd(&r3[i47]);
                r51 = _mm_loadu_pd(&r9[i47]);
                _mm_storeu_pd(
                    &alpha0_t_data[i47],
                    _mm_div_pd(
                        _mm_mul_pd(
                            _mm_add_pd(
                                _mm_sub_pd(
                                    _mm_add_pd(_mm_add_pd(_mm_set1_pd(eb_CoefPS),
                                                          _mm_mul_pd(r45, _mm_set1_pd(fb_CoefPS))),
                                               _mm_mul_pd(_mm_set1_pd(gb_CoefPS), r46)),
                                    _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(hb_CoefPS), r47), r48)),
                                _mm_mul_pd(_mm_mul_pd(_mm_mul_pd(_mm_set1_pd(ib_CoefPS), r49), r50),
                                           r48)),
                            _mm_set1_pd(-1.0)),
                        _mm_sub_pd(
                            _mm_sub_pd(
                                _mm_add_pd(_mm_add_pd(_mm_set1_pd(jb_CoefPS),
                                                      _mm_mul_pd(r45, _mm_set1_pd(kb_CoefPS))),
                                           _mm_mul_pd(_mm_set1_pd(lb_CoefPS), r46)),
                                _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(mb_CoefPS), r49), r47)),
                            _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(nb_CoefPS), r47),
            ...
            for (i47 = h_scalarLB; i47 < trueCount; i47++) {
                ...
                double d8;
                double d9;
                d5 = alpha1_t_data[i47];
                d6 = t5_data[i47];
                d7 = t11_data[i47];
                d8 = t12_data[i47];
                d9 = t10_data[i47];
                alpha0_t_data[i47] =
                    -((((eb_CoefPS + d5 * fb_CoefPS) + gb_CoefPS * d6) - hb_CoefPS * d7 * d8) +
                      ib_CoefPS * d9 * r3[i47] * d8) /
                    ((((jb_CoefPS + d5 * kb_CoefPS) + lb_CoefPS * d6) - mb_CoefPS * d9 * d7) -
                     nb_CoefPS * d7 * (d8 - r9[i47] * d7));
            }
        } else {
