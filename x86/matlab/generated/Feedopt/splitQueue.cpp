            ...
            if (curv.Info.Type == CurveType_TransP5) {
                ...
                if ((curv.Info.zspdmode == ZSpdMode_ZN) || (curv.Info.zspdmode == ZSpdMode_ZZ)) {
                    ...
                    while (searchJps) {
                        ...
                        for (i5 = scalarLB; i5 < b_loop_ub; i5++) {
                        ...
                        for (i7 = 0; i7 <= b_vectorUB; i7 += 2) {
                            ...
                            _mm_storeu_pd(&b_u[i7], _mm_div_pd(_mm_mul_pd(_mm_set1_pd(jps), r2),
                                                               _mm_set1_pd(6.0)));
                        }
                        for (i7 = b_scalarLB; i7 < d_loop_ub; i7++) {
                            b_u[i7] = jps * r1[i7] / 6.0;
                        }
                        ...
                        for (i9 = 0; i9 <= c_vectorUB; i9 += 2) {
                            ...
                            _mm_storeu_pd(&ud_vec[i9], _mm_div_pd(_mm_mul_pd(_mm_set1_pd(jps), r3),
                                                                  _mm_set1_pd(2.0)));
                        }
                        ...
                        for (i10 = d_scalarLB; i10 < g_loop_ub; i10++) {
                        ...
                        if (b_r1D.size(1) == ud_vec.size(1)) {
                            ...
                            for (int i11{0}; i11 < h_loop_ub; i11++) {
                                ...
                                e_scalarLB = (b_r1D.size(0) / 2) << 1;
                                e_vectorUB = e_scalarLB - 2;
                                for (i13 = 0; i13 <= e_vectorUB; i13 += 2) {
                                    __m128d r5;
                                    r5 = _mm_loadu_pd(&b_r1D[i13 + b_r1D.size(0) * i11]);
                                    _mm_storeu_pd(&V[i13 + V.size(0) * i11],
                                                  _mm_mul_pd(r5, _mm_set1_pd(ud_vec[i11])));
                                }
                                for (i13 = e_scalarLB; i13 < j_loop_ub; i13++) {
                                    V[i13 + V.size(0) * i11] =
                        ...
                        if ((r2D.size(1) == r1.size(1)) && (b_r1D.size(1) == udd_vec.size(1)) &&
                            ...
                            for (int i16{0}; i16 < k_loop_ub; i16++) {
                                ...
                                for (i18 = 0; i18 <= f_vectorUB; i18 += 2) {
                                    __m128d r6;
                                    __m128d r7;
                                    r6 = _mm_loadu_pd(&r2D[i18 + r2D.size(0) * i16]);
                                    r7 = _mm_loadu_pd(&b_r1D[i18 + b_r1D.size(0) * i16]);
                                    _mm_storeu_pd(
                                        &A[i18 + A.size(0) * i16],
                                        _mm_add_pd(_mm_mul_pd(r6, _mm_set1_pd(r1[i16])),
                                                   _mm_mul_pd(r7, _mm_set1_pd(udd_vec[i16]))));
                                }
                                for (i18 = f_scalarLB; i18 < m_loop_ub; i18++) {
                                    A[i18 + A.size(0) * i16] =
                                        r2D[i18 + r2D.size(0) * i16] * r1[i16] +
                        ...
                        if ((r3D.size(1) == r1.size(1)) && (r2D.size(1) == ud_vec.size(1)) &&
                            ...
                            for (int i28{0}; i28 < n_loop_ub; i28++) {
                                ...
                                for (i30 = 0; i30 <= g_vectorUB; i30 += 2) {
                                    ...
                                    __m128d r8;
                                    __m128d r9;
                                    r8 = _mm_loadu_pd(&r3D[i30 + r3D.size(0) * i28]);
                                    r9 = _mm_loadu_pd(&r2D[i30 + r2D.size(0) * i28]);
                                    r10 = _mm_loadu_pd(&b_r1D[i30 + b_r1D.size(0) * i28]);
                                    _mm_storeu_pd(
                                        &J[i30 + J.size(0) * i28],
                                        _mm_add_pd(
                                            _mm_add_pd(
                                                _mm_mul_pd(r8, _mm_set1_pd(r1[i28])),
                                                _mm_mul_pd(
                                                    _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(3.0), r9),
                                                               _mm_set1_pd(ud_vec[i28])),
                                                    _mm_set1_pd(udd_vec[i28]))),
                                            _mm_mul_pd(r10, _mm_set1_pd(jps))));
                                }
                                for (i30 = g_scalarLB; i30 < p_loop_ub; i30++) {
                                    J[i30 + J.size(0) * i28] =
                        ...
                        for (int i32{0}; i32 < r_loop_ub; i32++) {
                            x[i32] = (r1[i32] > curvS.Info.FeedRate);
                        }
                        b_y = false;
                        g_k = 0;
                        exitg1 = false;
                        ...
                        if (b_y) {
                            ...
                            s_loop_ub = r1.size(1);
                            h_scalarLB = (r1.size(1) / 2) << 1;
                            h_vectorUB = h_scalarLB - 2;
                            for (i35 = 0; i35 <= h_vectorUB; i35 += 2) {
                                __m128d r12;
                                r12 = _mm_loadu_pd(&r1[i35]);
                                _mm_storeu_pd(&r1[i35],
                                              _mm_sub_pd(r12, _mm_set1_pd(curvS.Info.FeedRate)));
                            }
                            for (i35 = h_scalarLB; i35 < s_loop_ub; i35++) {
                                r1[i35] = r1[i35] - curvS.Info.FeedRate;
                            }
                            ...
                            int j_k;
                            bool c_y;
                            r11.set_size(A.size(0), A.size(1));
                            if ((A.size(0) != 0) && (A.size(1) != 0)) {
                                ...
                                for (int h_k{0}; h_k < i33; h_k++) {
                                    ...
                                    for (int i_k{0}; i_k < i34; i_k++) {
                                        r11[i_k + r11.size(0) * h_k] =
                                            std::abs(A[i_k + A.size(0) * h_k]);
                                    }
                                }
                            }
                            if (r11.size(0) == ctx->cfg.indTot.size[0]) {
                                int t_loop_ub;
                                b_x.set_size(r11.size(0), r11.size(1));
                                t_loop_ub = r11.size(1);
                                for (int i36{0}; i36 < t_loop_ub; i36++) {
                                    int u_loop_ub;
                                    u_loop_ub = r11.size(0);
                                    for (int i37{0}; i37 < u_loop_ub; i37++) {
                                        b_x[i37 + b_x.size(0) * i36] =
                                            (r11[i37 + r11.size(0) * i36] > amax_data[i37]);
                                    }
                                }
                            } else {
                                e_binary_expand_op(b_x, r11, amax_data, ctx);
                            }
                            c_y = false;
                            j_k = 0;
                            exitg1 = false;
                            ...
                            if (c_y) {
                                ...
                                if (r11.size(0) == ctx->cfg.indTot.size[0]) {
                                    int bb_loop_ub;
                                    int w_loop_ub;
                                    r13.set_size(r11.size(0), r11.size(1));
                                    w_loop_ub = r11.size(1);
                                    for (int i41{0}; i41 < w_loop_ub; i41++) {
                                        int i45;
                                        int i_scalarLB;
                                        int i_vectorUB;
                                        int y_loop_ub;
                                        y_loop_ub = r11.size(0);
                                        i_scalarLB = (r11.size(0) / 2) << 1;
                                        i_vectorUB = i_scalarLB - 2;
                                        for (i45 = 0; i45 <= i_vectorUB; i45 += 2) {
                                            __m128d r14;
                                            __m128d r15;
                                            r14 = _mm_loadu_pd(&r11[i45 + r11.size(0) * i41]);
                                            r15 = _mm_loadu_pd(&amax_data[i45]);
                                            _mm_storeu_pd(&r13[i45 + r13.size(0) * i41],
                                                          _mm_sub_pd(r14, r15));
                                        }
                                        for (i45 = i_scalarLB; i45 < y_loop_ub; i45++) {
                                            r13[i45 + r13.size(0) * i41] =
                                                r11[i45 + r11.size(0) * i41] - amax_data[i45];
                                        }
                                    }
                                    r11.set_size(r13.size(0), r13.size(1));
                                    bb_loop_ub = r13.size(1);
                                    for (int i44{0}; i44 < bb_loop_ub; i44++) {
                                        int cb_loop_ub;
                                        cb_loop_ub = r13.size(0);
                                        for (int i46{0}; i46 < cb_loop_ub; i46++) {
                                            r11[i46 + r11.size(0) * i44] =
                                                r13[i46 + r13.size(0) * i44];
                                        }
                                    }
                                    coder::internal::b_maximum(r11, &a_delta, &b_iindx);
                                } else {
                                    binary_expand_op(r11, amax_data, ctx, &a_delta, &b_iindx);
                                }
                                ...
                                int m_k;
                                bool d_y;
                                r11.set_size(J.size(0), J.size(1));
                                ...
                                if (r11.size(0) == ctx->cfg.indTot.size[0]) {
                                    int x_loop_ub;
                                    b_x.set_size(r11.size(0), r11.size(1));
                                    x_loop_ub = r11.size(1);
                                    for (int i42{0}; i42 < x_loop_ub; i42++) {
                                        int ab_loop_ub;
                                        ab_loop_ub = r11.size(0);
                                        for (int i43{0}; i43 < ab_loop_ub; i43++) {
                                            b_x[i43 + b_x.size(0) * i42] =
                                                (r11[i43 + r11.size(0) * i42] > jmax_data[i43]);
                                        }
                                    }
                                } else {
                                    e_binary_expand_op(b_x, r11, jmax_data, ctx);
                                }
                                d_y = false;
                                m_k = 0;
                                exitg1 = false;
                                ...
                                if (d_y) {
                                    ...
                                    if (r11.size(0) == ctx->cfg.indTot.size[0]) {
                                        int db_loop_ub;
                                        int fb_loop_ub;
                                        r16.set_size(r11.size(0), r11.size(1));
                                        db_loop_ub = r11.size(1);
                                        for (int i47{0}; i47 < db_loop_ub; i47++) {
                                            int eb_loop_ub;
                                            int i49;
                                            int j_scalarLB;
                                            int j_vectorUB;
                                            eb_loop_ub = r11.size(0);
                                            j_scalarLB = (r11.size(0) / 2) << 1;
                                            j_vectorUB = j_scalarLB - 2;
                                            for (i49 = 0; i49 <= j_vectorUB; i49 += 2) {
                                                __m128d r17;
                                                __m128d r18;
                                                r17 = _mm_loadu_pd(&r11[i49 + r11.size(0) * i47]);
                                                r18 = _mm_loadu_pd(&jmax_data[i49]);
                                                _mm_storeu_pd(&r16[i49 + r16.size(0) * i47],
                                                              _mm_sub_pd(r17, r18));
                                            }
                                            for (i49 = j_scalarLB; i49 < eb_loop_ub; i49++) {
                                                r16[i49 + r16.size(0) * i47] =
                                                    r11[i49 + r11.size(0) * i47] - jmax_data[i49];
                                            }
                                        }
                                        r11.set_size(r16.size(0), r16.size(1));
                                        fb_loop_ub = r16.size(1);
                                        for (int i48{0}; i48 < fb_loop_ub; i48++) {
                                            int gb_loop_ub;
                                            gb_loop_ub = r16.size(0);
                                            for (int i50{0}; i50 < gb_loop_ub; i50++) {
                                                r11[i50 + r11.size(0) * i48] =
                                                    r16[i50 + r16.size(0) * i48];
                                            }
                                        }
                                        coder::internal::b_maximum(r11, &j_delta, &c_iindx);
                                    } else {
                                        binary_expand_op(r11, jmax_data, ctx, &j_delta, &c_iindx);
                                    }
