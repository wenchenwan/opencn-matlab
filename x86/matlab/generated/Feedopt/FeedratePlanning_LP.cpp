                         ...
                         ::coder::array<double, 2U> &Coeff, bool *success, int *status)
{
    ...
    f_loop_ub = ii.size(0);
    b_scalarLB = (ii.size(0) / 4) << 2;
    b_vectorUB = b_scalarLB - 4;
    for (i6 = 0; i6 <= b_vectorUB; i6 += 4) {
        __m128i r2;
        r2 = _mm_loadu_si128((const __m128i *)&ii[i6]);
        _mm_storeu_si128((__m128i *)&Ais[i6], _mm_sub_epi32(r2, _mm_set1_epi32(1)));
    }
    for (i6 = b_scalarLB; i6 < f_loop_ub; i6++) {
        Ais[i6] = ii[i6] - 1;
    }
    // 'c_simplex:15' Ajs = int32(Ajsd)-1;
    Ajs.set_size(jj.size(0));
    g_loop_ub = jj.size(0);
    c_scalarLB = (jj.size(0) / 4) << 2;
    c_vectorUB = c_scalarLB - 4;
    for (i7 = 0; i7 <= c_vectorUB; i7 += 4) {
        __m128i r3;
        r3 = _mm_loadu_si128((const __m128i *)&jj[i7]);
        _mm_storeu_si128((__m128i *)&Ajs[i7], _mm_sub_epi32(r3, _mm_set1_epi32(1)));
    }
    for (i7 = c_scalarLB; i7 < g_loop_ub; i7++) {
        Ajs[i7] = jj[i7] - 1;
    }
    ...
    if (b_status != 0) {
        ...
        a21_tmp = Aeq[b_r1 % 2 + Aeq.size(0) * (b_r1 / 2)];
        a21 = Aeq[b_r2 % 2 + Aeq.size(0) * (b_r2 / 2)] / a21_tmp;
        X_tmp = Aeq[(b_r1 + 2) % 2 + Aeq.size(0) * ((b_r1 + 2) / 2)];
        X[1] = (beq[b_r2] - beq[b_r1] * a21) /
               (Aeq[(b_r2 + 2) % 2 + Aeq.size(0) * ((b_r2 + 2) / 2)] - a21 * X_tmp);
        X[0] = (beq[b_r1] - X[1] * X_tmp) / a21_tmp;
        Coeff0[0] = X[0];
        Coeff0[1] = X[1];
        ...
        b_a21 = Aeq[((c_r2 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) - 2)) - 1] /
                Aeq[((c_r1 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) - 2)) - 1];
        Y_idx_1 = (beq[(c_r2 + beq.size(0)) - 1] - beq[(c_r1 + beq.size(0)) - 1] * b_a21) /
                  (Aeq[((c_r2 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) + -1)) - 1] -
                   b_a21 * Aeq[((c_r1 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) + -1)) - 1]);
        b_Coeff0 = Coeff0.size(0) - 1;
        Coeff0[Coeff0.size(0) - 2] =
            (beq[(c_r1 + beq.size(0)) - 1] -
             Y_idx_1 * Aeq[((c_r1 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) + -1)) - 1]) /
            Aeq[((c_r1 + Aeq.size(0)) + Aeq.size(0) * (Aeq.size(1) - 2)) - 1];
    ...
    coder::b_sparse(e_result, &r4);
    r4.vertcat(b_Aeq, &Atot);
    // 'c_simplex:7' c_prof_out('join_A');
    //      MaxCoeff = FeedoptLimits.MaxNCoeff*FeedoptLimits.MaxNHorz;
    // 'c_simplex:9' [NCoeff, NHorz] = size(f);
    // 'c_simplex:10' C = zeros(NCoeff, NHorz);
    ...
    Ais.set_size(ii.size(0));
    n_loop_ub = ii.size(0);
    d_scalarLB = (ii.size(0) / 4) << 2;
    d_vectorUB = d_scalarLB - 4;
    for (i29 = 0; i29 <= d_vectorUB; i29 += 4) {
        __m128i r5;
        r5 = _mm_loadu_si128((const __m128i *)&ii[i29]);
        _mm_storeu_si128((__m128i *)&Ais[i29], _mm_sub_epi32(r5, _mm_set1_epi32(1)));
    }
    for (i29 = d_scalarLB; i29 < n_loop_ub; i29++) {
        Ais[i29] = ii[i29] - 1;
    }
    // 'c_simplex:15' Ajs = int32(Ajsd)-1;
    Ajs.set_size(jj.size(0));
    o_loop_ub = jj.size(0);
    e_scalarLB = (jj.size(0) / 4) << 2;
    e_vectorUB = e_scalarLB - 4;
    for (i30 = 0; i30 <= e_vectorUB; i30 += 4) {
        __m128i r6;
        r6 = _mm_loadu_si128((const __m128i *)&jj[i30]);
        _mm_storeu_si128((__m128i *)&Ajs[i30], _mm_sub_epi32(r6, _mm_set1_epi32(1)));
    }
    for (i30 = e_scalarLB; i30 < o_loop_ub; i30++) {
        Ajs[i30] = jj[i30] - 1;
    }
    // 'c_simplex:17' coder.varsize('Avs', [Inf, 1], [1, 0]);
    // 'c_simplex:18' coder.varsize('Ais', [Inf, 1], [1, 0]);
    ...
    for (int d_k{0}; d_k < inner; d_k++) {
        __m128d r7;
        __m128d r8;
        r7 = _mm_loadu_pd(&continuity[2 * d_k]);
        r8 = _mm_loadu_pd(&X[0]);
        _mm_storeu_pd(&X[0], _mm_add_pd(r8, _mm_mul_pd(r7, _mm_set1_pd(Coeff[d_k]))));
    }
