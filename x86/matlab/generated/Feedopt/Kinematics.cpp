                         ...
                         int *a_j_size) const
{
    ...
    *a_j_size = a_t_size;
    ...
    scalarLB = (a_t_size / 2) << 1;
    vectorUB = scalarLB - 2;
    for (int i1{0}; i1 <= vectorUB; i1 += 2) {
        __m128d r;
        __m128d r1;
        r = _mm_loadu_pd(&b_JP[i1]);
        r1 = _mm_loadu_pd(&b_J[i1]);
        _mm_storeu_pd(&a_j_data[i1], _mm_add_pd(r, r1));
    }
    for (int i1{scalarLB}; i1 < a_t_size; i1++) {
                            ...
                            int *a_t_size) const
{
    ...
    *a_t_size = a_j_size;
    ...
    for (int i1{0}; i1 <= vectorUB; i1 += 2) {
        ...
        r = _mm_loadu_pd(&b_JP[i1]);
        r1 = _mm_loadu_pd(&b_J[i1]);
        _mm_storeu_pd(&a_t_data[i1], _mm_add_pd(r, r1));
    }
    for (int i1{scalarLB}; i1 < a_j_size; i1++) {
                       ...
                       double a_j_data[], int *a_j_size, double j_j_data[], int *j_j_size) const
{
    ...
    *j_j_size = j_t_size;
    ...
    for (int i3{0}; i3 <= vectorUB; i3 += 2) {
        __m128d r;
        __m128d r1;
        r = _mm_loadu_pd(&b_JP[i3]);
        r1 = _mm_loadu_pd(&b_J[i3]);
        _mm_storeu_pd(&a_j_data[i3], _mm_add_pd(r, r1));
    }
    for (int i3{scalarLB}; i3 < a_t_size; i3++) {
