                  ...
                  double curv_a_param, double curv_b_param)
{
    ...
    if ((curv_Info.Type == CurveType_Helix) || (curv_Info.Type == CurveType_Line)) {
            ...
            if ((r1.size(0) == y.size(0)) && (i10 == r2.size(0))) {
                ...
                for (int i11{0}; i11 < 9; i11++) {
                    ...
                    for (i12 = 0; i12 <= vectorUB; i12 += 2) {
                        ...
                        __m128d r7;
                        __m128d r8;
                        r6 = _mm_loadu_pd(&r1[i12 + r1.size(0) * i11]);
                        r7 = _mm_loadu_pd(&y[i12 + y.size(0) * i11]);
                        r8 = _mm_loadu_pd(&r2[i12 + r2.size(0) * i11]);
                        _mm_storeu_pd(&y[i12 + y.size(0) * i11],
                                      _mm_add_pd(_mm_mul_pd(r6, r7), r8));
                    }
                    for (i12 = scalarLB; i12 < d_loop_ub; i12++) {
                        y[i12 + y.size(0) * i11] =
        ...
        r3 = _mm_loadu_pd(&Integrand[0]);
        r4 = _mm_sqrt_pd(r3);
        _mm_storeu_pd(&Integrand[0], r4);
        r5 = _mm_loadu_pd(&b_y1[0]);
        _mm_storeu_pd(&x[0], _mm_mul_pd(r4, r5));
        r3 = _mm_loadu_pd(&Integrand[2]);
        r4 = _mm_sqrt_pd(r3);
        _mm_storeu_pd(&Integrand[2], r4);
        r5 = _mm_loadu_pd(&b_y1[2]);
        _mm_storeu_pd(&x[2], _mm_mul_pd(r4, r5));
        r3 = _mm_loadu_pd(&Integrand[4]);
        r4 = _mm_sqrt_pd(r3);
        _mm_storeu_pd(&Integrand[4], r4);
        r5 = _mm_loadu_pd(&b_y1[4]);
        _mm_storeu_pd(&x[4], _mm_mul_pd(r4, r5));
        r3 = _mm_loadu_pd(&Integrand[6]);
        r4 = _mm_sqrt_pd(r3);
        _mm_storeu_pd(&Integrand[6], r4);
        r5 = _mm_loadu_pd(&b_y1[6]);
        _mm_storeu_pd(&x[6], _mm_mul_pd(r4, r5));
