...
{
    ::coder::array<int, 1U> t21_indCart;
    ::coder::array<int, 1U> t21_indRot;
    int b_loop_ub;
    int loop_ub;
    ...
    t21_indCart.set_size(cfg.indCart.size[0]);
    loop_ub = cfg.indCart.size[0];
    for (int i{0}; i < loop_ub; i++) {
        t21_indCart[i] = cfg.indCart.data[i];
    }
    t21_indRot.set_size(cfg.indRot.size[0]);
    b_loop_ub = cfg.indRot.size[0];
    for (int i1{0}; i1 < b_loop_ub; i1++) {
        t21_indRot[i1] = cfg.indRot.data[i1];
    }
    b_EvalCurvStructNoCtx(cfg.maskTot.data, cfg.maskTot.size, cfg.maskCart.data, cfg.maskCart.size,
                          cfg.maskRot.data, cfg.maskRot.size, t21_indCart, t21_indRot,
