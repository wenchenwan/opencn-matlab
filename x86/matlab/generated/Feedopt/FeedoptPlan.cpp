    ...
    *opt_struct = opt_struct_tmp;
    ...
    case Fopt_Compress: {
        ...
        if (ctx->cfg.Compressing.Skip) {
            ...
            for (int i50{0}; i50 < bc_loop_ub; i50++) {
                r.cfg.indCart[i50] = ctx->cfg.indCart.data[i50];
            }
            r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
    ...
    case Fopt_Split: {
        ...
        for (int i41{0}; i41 < sb_loop_ub; i41++) {
            r.cfg.maskCart[i41] = ctx->cfg.maskCart[i41];
        }
        r.cfg.maskRot.set_size(1, ctx->cfg.maskRot.size(1));
    ...
    case Fopt_Opt: {
        ...
        for (int i42{0}; i42 < tb_loop_ub; i42++) {
            b_ctx.cfg.maskCart[i42] = ctx->cfg.maskCart[i42];
        }
        ...
        for (int i52{0}; i52 < dc_loop_ub; i52++) {
