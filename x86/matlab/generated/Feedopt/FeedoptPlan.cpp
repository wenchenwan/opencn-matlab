...
void FeedoptPlan(FeedoptContext *ctx, bool *optimized, CurvStruct *opt_struct)
{
    ...
    *opt_struct = opt_struct_tmp;
    ...
    case Fopt_Check: {
        ...
        o_loop_ub = ctx->cfg.maskCart.size[1];
        if (0 <= o_loop_ub - 1) {
            std::copy(&ctx->cfg.maskCart.data[0], &ctx->cfg.maskCart.data[o_loop_ub],
                      &r.cfg.maskCart.data[0]);
        }
        r.cfg.maskRot.size[0] = 1;
        r.cfg.maskRot.size[1] = ctx->cfg.maskRot.size[1];
        y_loop_ub = ctx->cfg.maskRot.size[1];
        if (0 <= y_loop_ub - 1) {
            std::copy(&ctx->cfg.maskRot.data[0], &ctx->cfg.maskRot.data[y_loop_ub],
                      &r.cfg.maskRot.data[0]);
        }
        r.cfg.indCart.set_size(ctx->cfg.indCart.size[0]);
        hb_loop_ub = ctx->cfg.indCart.size[0];
        for (int i28{0}; i28 < hb_loop_ub; i28++) {
            r.cfg.indCart[i28] = ctx->cfg.indCart.data[i28];
        }
        r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        lb_loop_ub = ctx->cfg.indRot.size[0];
        for (int i32{0}; i32 < lb_loop_ub; i32++) {
            r.cfg.indRot[i32] = ctx->cfg.indRot.data[i32];
        }
        ...
        for (int k{0}; k < i37; k++) {
            ...
            d = 180.0 - r.cfg.CuspThreshold;
            coder::b_cosd(&d);
            if (c / (coder::b_norm(r0D1) * coder::b_norm(r1D1)) < d) {
        ...
        if (ctx->cfg.Compressing.Skip) {
            ...
            for (int i16{0}; i16 < s_loop_ub; i16++) {
                ...
                for (int i21{0}; i21 < ab_loop_ub; i21++) {
                    r.BasisValDD[i21 + r.BasisValDD.size(0) * i16] =
                        ctx->BasisValDD[i21 + ctx->BasisValDD.size(0) * i16];
                }
            }
            ...
            for (int i22{0}; i22 < bb_loop_ub; i22++) {
                ...
                for (int i29{0}; i29 < ib_loop_ub; i29++) {
                    r.BasisValDDD[i29 + r.BasisValDDD.size(0) * i22] =
                        ctx->BasisValDDD[i29 + ctx->BasisValDDD.size(0) * i22];
                }
            }
            r.BasisIntegr.set_size(ctx->BasisIntegr.size(0));
            jb_loop_ub = ctx->BasisIntegr.size(0);
            for (int i30{0}; i30 < jb_loop_ub; i30++) {
                r.BasisIntegr[i30] = ctx->BasisIntegr[i30];
            }
            r.Bl = ctx->Bl;
            r.u_vec.set_size(1, ctx->u_vec.size(1));
            ob_loop_ub = ctx->u_vec.size(1);
            for (int i35{0}; i35 < ob_loop_ub; i35++) {
                r.u_vec[i35] = ctx->u_vec[i35];
            }
            r.q_spline = ctx->q_spline;
            ...
            for (int i41{0}; i41 < bc_loop_ub; i41++) {
                r.cfg.indCart[i41] = ctx->cfg.indCart.data[i41];
            }
            r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
            ec_loop_ub = ctx->cfg.indRot.size[0];
            for (int i45{0}; i45 < ec_loop_ub; i45++) {
                r.cfg.indRot[i45] = ctx->cfg.indRot.data[i45];
            }
            r.cfg.NumberAxis = ctx->cfg.NumberAxis;
            r.cfg.NCart = ctx->cfg.NCart;
            r.cfg.NRot = ctx->cfg.NRot;
            r.cfg.D.set_size(ctx->cfg.D.size[0]);
            hc_loop_ub = ctx->cfg.D.size[0];
            for (int i48{0}; i48 < hc_loop_ub; i48++) {
                r.cfg.D[i48] = ctx->cfg.D.data[i48];
            }
            r.cfg.coeffD = ctx->cfg.coeffD;
            r.cfg.kin_params.size[0] = ctx->cfg.kin_params.size[0];
            kc_loop_ub = ctx->cfg.kin_params.size[0];
            if (0 <= kc_loop_ub - 1) {
                std::copy(&ctx->cfg.kin_params.data[0], &ctx->cfg.kin_params.data[kc_loop_ub],
                          &r.cfg.kin_params.data[0]);
            }
            for (int i51{0}; i51 < 5; i51++) {
                r.cfg.kin_type[i51] = ctx->cfg.kin_type[i51];
            }
            ...
            for (int i54{0}; i54 < 6; i54++) {
                r.cfg.vmax[i54] = ctx->cfg.vmax[i54];
                r.cfg.amax[i54] = ctx->cfg.amax[i54];
                r.cfg.jmax[i54] = ctx->cfg.jmax[i54];
            }
            ...
            for (int i57{0}; i57 < 9; i57++) {
                r.cfg.LogFileName[i57] = ctx->cfg.LogFileName[i57];
            }
            ...
            nc_loop_ub = ctx->Coeff.size(1);
            for (int i60{0}; i60 < nc_loop_ub; i60++) {
                int qc_loop_ub;
                qc_loop_ub = ctx->Coeff.size(0);
                for (int i63{0}; i63 < qc_loop_ub; i63++) {
                    r.Coeff[i63 + r.Coeff.size(0) * i60] =
                        ctx->Coeff[i63 + ctx->Coeff.size(0) * i60];
                }
            }
            ...
            tc_loop_ub = r.BasisVal.size(1);
            for (int i66{0}; i66 < tc_loop_ub; i66++) {
                int yc_loop_ub;
                yc_loop_ub = r.BasisVal.size(0);
                for (int i71{0}; i71 < yc_loop_ub; i71++) {
                    ctx->BasisVal[i71 + ctx->BasisVal.size(0) * i66] =
                        r.BasisVal[i71 + r.BasisVal.size(0) * i66];
                }
            }
            ctx->BasisValD.set_size(r.BasisValD.size(0), r.BasisValD.size(1));
            ad_loop_ub = r.BasisValD.size(1);
            for (int i72{0}; i72 < ad_loop_ub; i72++) {
                int fd_loop_ub;
                fd_loop_ub = r.BasisValD.size(0);
                for (int i77{0}; i77 < fd_loop_ub; i77++) {
                    ctx->BasisValD[i77 + ctx->BasisValD.size(0) * i72] =
                        r.BasisValD[i77 + r.BasisValD.size(0) * i72];
                }
            }
            ctx->BasisValDD.set_size(r.BasisValDD.size(0), r.BasisValDD.size(1));
            gd_loop_ub = r.BasisValDD.size(1);
            for (int i78{0}; i78 < gd_loop_ub; i78++) {
                int ld_loop_ub;
                ld_loop_ub = r.BasisValDD.size(0);
                for (int i83{0}; i83 < ld_loop_ub; i83++) {
                    ctx->BasisValDD[i83 + ctx->BasisValDD.size(0) * i78] =
                        r.BasisValDD[i83 + r.BasisValDD.size(0) * i78];
                }
            }
            ctx->BasisValDDD.set_size(r.BasisValDDD.size(0), r.BasisValDDD.size(1));
            md_loop_ub = r.BasisValDDD.size(1);
            for (int i84{0}; i84 < md_loop_ub; i84++) {
                int rd_loop_ub;
                rd_loop_ub = r.BasisValDDD.size(0);
                for (int i89{0}; i89 < rd_loop_ub; i89++) {
                    ctx->BasisValDDD[i89 + ctx->BasisValDDD.size(0) * i84] =
                        r.BasisValDDD[i89 + r.BasisValDDD.size(0) * i84];
                }
            }
            ctx->BasisIntegr.set_size(r.BasisIntegr.size(0));
            sd_loop_ub = r.BasisIntegr.size(0);
            for (int i90{0}; i90 < sd_loop_ub; i90++) {
                ctx->BasisIntegr[i90] = r.BasisIntegr[i90];
            }
            ctx->Bl = r.Bl;
            ctx->u_vec.set_size(1, r.u_vec.size(1));
            vd_loop_ub = r.u_vec.size(1);
            for (int i93{0}; i93 < vd_loop_ub; i93++) {
                ctx->u_vec[i93] = r.u_vec[i93];
            }
            ...
            yd_loop_ub = r.cfg.maskTot.size[1];
            if (0 <= yd_loop_ub - 1) {
                std::copy(&r.cfg.maskTot.data[0], &r.cfg.maskTot.data[yd_loop_ub],
                          &ctx->cfg.maskTot.data[0]);
            }
            ctx->cfg.maskCart.size[0] = 1;
            ctx->cfg.maskCart.size[1] = r.cfg.maskCart.size[1];
            ce_loop_ub = r.cfg.maskCart.size[1];
            if (0 <= ce_loop_ub - 1) {
                std::copy(&r.cfg.maskCart.data[0], &r.cfg.maskCart.data[ce_loop_ub],
                          &ctx->cfg.maskCart.data[0]);
            }
            ctx->cfg.maskRot.size[0] = 1;
            ctx->cfg.maskRot.size[1] = r.cfg.maskRot.size[1];
            fe_loop_ub = r.cfg.maskRot.size[1];
            if (0 <= fe_loop_ub - 1) {
                std::copy(&r.cfg.maskRot.data[0], &r.cfg.maskRot.data[fe_loop_ub],
                          &ctx->cfg.maskRot.data[0]);
            }
            ctx->cfg.indCart.size[0] = r.cfg.indCart.size(0);
            ie_loop_ub = r.cfg.indCart.size(0);
            for (int i96{0}; i96 < ie_loop_ub; i96++) {
                ctx->cfg.indCart.data[i96] = r.cfg.indCart[i96];
            }
            ctx->cfg.indRot.size[0] = r.cfg.indRot.size(0);
            le_loop_ub = r.cfg.indRot.size(0);
            for (int i99{0}; i99 < le_loop_ub; i99++) {
                ctx->cfg.indRot.data[i99] = r.cfg.indRot[i99];
            }
            ctx->cfg.NumberAxis = r.cfg.NumberAxis;
            ctx->cfg.NCart = r.cfg.NCart;
            ctx->cfg.NRot = r.cfg.NRot;
            ctx->cfg.D.size[0] = r.cfg.D.size(0);
            oe_loop_ub = r.cfg.D.size(0);
            for (int i102{0}; i102 < oe_loop_ub; i102++) {
                ctx->cfg.D.data[i102] = r.cfg.D[i102];
            }
            ctx->cfg.coeffD = r.cfg.coeffD;
            ctx->cfg.kin_params.size[0] = r.cfg.kin_params.size[0];
            re_loop_ub = r.cfg.kin_params.size[0];
            if (0 <= re_loop_ub - 1) {
                std::copy(&r.cfg.kin_params.data[0], &r.cfg.kin_params.data[re_loop_ub],
                          &ctx->cfg.kin_params.data[0]);
            }
            for (int i105{0}; i105 < 5; i105++) {
                ctx->cfg.kin_type[i105] = r.cfg.kin_type[i105];
            }
            ...
            for (int i108{0}; i108 < 6; i108++) {
                ctx->cfg.vmax[i108] = r.cfg.vmax[i108];
                ctx->cfg.amax[i108] = r.cfg.amax[i108];
                ctx->cfg.jmax[i108] = r.cfg.jmax[i108];
            }
            ...
            for (int i111{0}; i111 < 9; i111++) {
                ctx->cfg.LogFileName[i111] = r.cfg.LogFileName[i111];
            }
            ...
            ue_loop_ub = r.Coeff.size(1);
            for (int i114{0}; i114 < ue_loop_ub; i114++) {
                int xe_loop_ub;
                xe_loop_ub = r.Coeff.size(0);
                for (int i117{0}; i117 < xe_loop_ub; i117++) {
                    ctx->Coeff[i117 + ctx->Coeff.size(0) * i114] =
                        r.Coeff[i117 + r.Coeff.size(0) * i114];
                }
            }
    ...
    case Fopt_Smooth: {
        ...
        for (int i7{0}; i7 < h_loop_ub; i7++) {
            int l_loop_ub;
            l_loop_ub = ctx->BasisValD.size(0);
            for (int i11{0}; i11 < l_loop_ub; i11++) {
                r.BasisValD[i11 + r.BasisValD.size(0) * i7] =
                    ctx->BasisValD[i11 + ctx->BasisValD.size(0) * i7];
            }
        }
        r.BasisValDD.set_size(ctx->BasisValDD.size(0), ctx->BasisValDD.size(1));
        ...
        for (int i13{0}; i13 < p_loop_ub; i13++) {
            int t_loop_ub;
            t_loop_ub = ctx->BasisValDD.size(0);
            for (int i17{0}; i17 < t_loop_ub; i17++) {
                r.BasisValDD[i17 + r.BasisValDD.size(0) * i13] =
                    ctx->BasisValDD[i17 + ctx->BasisValDD.size(0) * i13];
            }
        }
        r.BasisValDDD.set_size(ctx->BasisValDDD.size(0), ctx->BasisValDDD.size(1));
        v_loop_ub = ctx->BasisValDDD.size(1);
        for (int i19{0}; i19 < v_loop_ub; i19++) {
            int cb_loop_ub;
            cb_loop_ub = ctx->BasisValDDD.size(0);
            for (int i23{0}; i23 < cb_loop_ub; i23++) {
                r.BasisValDDD[i23 + r.BasisValDDD.size(0) * i19] =
                    ctx->BasisValDDD[i23 + ctx->BasisValDDD.size(0) * i19];
            }
        }
        r.BasisIntegr.set_size(ctx->BasisIntegr.size(0));
        eb_loop_ub = ctx->BasisIntegr.size(0);
        for (int i25{0}; i25 < eb_loop_ub; i25++) {
            r.BasisIntegr[i25] = ctx->BasisIntegr[i25];
        }
        r.Bl = ctx->Bl;
        r.u_vec.set_size(1, ctx->u_vec.size(1));
        mb_loop_ub = ctx->u_vec.size(1);
        for (int i33{0}; i33 < mb_loop_ub; i33++) {
            r.u_vec[i33] = ctx->u_vec[i33];
        }
        r.q_spline = ctx->q_spline;
        r.q_gcode = ctx->q_gcode;
        r.q_compress = ctx->q_compress;
        ...
        if (0 <= sb_loop_ub - 1) {
            std::copy(&ctx->cfg.maskCart.data[0], &ctx->cfg.maskCart.data[sb_loop_ub],
                      &r.cfg.maskCart.data[0]);
        }
        r.cfg.maskRot.size[0] = 1;
        r.cfg.maskRot.size[1] = ctx->cfg.maskRot.size[1];
        ...
        if (0 <= vb_loop_ub - 1) {
            std::copy(&ctx->cfg.maskRot.data[0], &ctx->cfg.maskRot.data[vb_loop_ub],
                      &r.cfg.maskRot.data[0]);
        }
        r.cfg.indCart.set_size(ctx->cfg.indCart.size[0]);
        yb_loop_ub = ctx->cfg.indCart.size[0];
        for (int i39{0}; i39 < yb_loop_ub; i39++) {
            r.cfg.indCart[i39] = ctx->cfg.indCart.data[i39];
        }
        r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        cc_loop_ub = ctx->cfg.indRot.size[0];
        for (int i43{0}; i43 < cc_loop_ub; i43++) {
            r.cfg.indRot[i43] = ctx->cfg.indRot.data[i43];
        }
        r.cfg.NumberAxis = ctx->cfg.NumberAxis;
        r.cfg.NCart = ctx->cfg.NCart;
        r.cfg.NRot = ctx->cfg.NRot;
        r.cfg.D.set_size(ctx->cfg.D.size[0]);
        fc_loop_ub = ctx->cfg.D.size[0];
        for (int i46{0}; i46 < fc_loop_ub; i46++) {
            r.cfg.D[i46] = ctx->cfg.D.data[i46];
        }
        r.cfg.coeffD = ctx->cfg.coeffD;
        r.cfg.kin_params.size[0] = ctx->cfg.kin_params.size[0];
        ic_loop_ub = ctx->cfg.kin_params.size[0];
        if (0 <= ic_loop_ub - 1) {
            std::copy(&ctx->cfg.kin_params.data[0], &ctx->cfg.kin_params.data[ic_loop_ub],
                      &r.cfg.kin_params.data[0]);
        }
        for (int i49{0}; i49 < 5; i49++) {
            r.cfg.kin_type[i49] = ctx->cfg.kin_type[i49];
        }
        r.cfg.NDiscr = ctx->cfg.NDiscr;
        r.cfg.NBreak = ctx->cfg.NBreak;
        r.cfg.UseDynamicBreakpoints = ctx->cfg.UseDynamicBreakpoints;
        ...
        for (int i52{0}; i52 < 6; i52++) {
            r.cfg.vmax[i52] = ctx->cfg.vmax[i52];
            r.cfg.amax[i52] = ctx->cfg.amax[i52];
            r.cfg.jmax[i52] = ctx->cfg.jmax[i52];
        }
        r.cfg.LeeSplineDegree = ctx->cfg.LeeSplineDegree;
        r.cfg.SplineDegree = ctx->cfg.SplineDegree;
        r.cfg.CutOff = ctx->cfg.CutOff;
        ...
        for (int i55{0}; i55 < 9; i55++) {
            r.cfg.LogFileName[i55] = ctx->cfg.LogFileName[i55];
        }
        r.errcode = ctx->errcode;
        r.jmax_increase_count = ctx->jmax_increase_count;
        r.zero_start = ctx->zero_start;
        ...
        r.programmed_stop = ctx->programmed_stop;
        r.Coeff.set_size(ctx->Coeff.size(0), ctx->Coeff.size(1));
        lc_loop_ub = ctx->Coeff.size(1);
        for (int i58{0}; i58 < lc_loop_ub; i58++) {
            int oc_loop_ub;
            oc_loop_ub = ctx->Coeff.size(0);
            for (int i61{0}; i61 < oc_loop_ub; i61++) {
                r.Coeff[i61 + r.Coeff.size(0) * i58] = ctx->Coeff[i61 + ctx->Coeff.size(0) * i58];
            }
        }
        r.Skipped = ctx->Skipped;
        r.kin = ctx->kin;
        smoothCurvStructs(&r);
        ctx->BasisVal.set_size(r.BasisVal.size(0), r.BasisVal.size(1));
        rc_loop_ub = r.BasisVal.size(1);
        for (int i64{0}; i64 < rc_loop_ub; i64++) {
            int uc_loop_ub;
            uc_loop_ub = r.BasisVal.size(0);
            for (int i67{0}; i67 < uc_loop_ub; i67++) {
                ctx->BasisVal[i67 + ctx->BasisVal.size(0) * i64] =
                    r.BasisVal[i67 + r.BasisVal.size(0) * i64];
            }
        }
        ctx->BasisValD.set_size(r.BasisValD.size(0), r.BasisValD.size(1));
        ...
        for (int i69{0}; i69 < wc_loop_ub; i69++) {
            int bd_loop_ub;
            bd_loop_ub = r.BasisValD.size(0);
            for (int i73{0}; i73 < bd_loop_ub; i73++) {
                ctx->BasisValD[i73 + ctx->BasisValD.size(0) * i69] =
                    r.BasisValD[i73 + r.BasisValD.size(0) * i69];
            }
        }
        ctx->BasisValDD.set_size(r.BasisValDD.size(0), r.BasisValDD.size(1));
        ...
        for (int i75{0}; i75 < dd_loop_ub; i75++) {
            int hd_loop_ub;
            hd_loop_ub = r.BasisValDD.size(0);
            for (int i79{0}; i79 < hd_loop_ub; i79++) {
                ctx->BasisValDD[i79 + ctx->BasisValDD.size(0) * i75] =
                    r.BasisValDD[i79 + r.BasisValDD.size(0) * i75];
            }
        }
        ctx->BasisValDDD.set_size(r.BasisValDDD.size(0), r.BasisValDDD.size(1));
        ...
        for (int i81{0}; i81 < jd_loop_ub; i81++) {
            int nd_loop_ub;
            nd_loop_ub = r.BasisValDDD.size(0);
            for (int i85{0}; i85 < nd_loop_ub; i85++) {
                ctx->BasisValDDD[i85 + ctx->BasisValDDD.size(0) * i81] =
                    r.BasisValDDD[i85 + r.BasisValDDD.size(0) * i81];
            }
        }
        ctx->BasisIntegr.set_size(r.BasisIntegr.size(0));
        pd_loop_ub = r.BasisIntegr.size(0);
        for (int i87{0}; i87 < pd_loop_ub; i87++) {
            ctx->BasisIntegr[i87] = r.BasisIntegr[i87];
        }
        ctx->Bl = r.Bl;
        ctx->u_vec.set_size(1, r.u_vec.size(1));
        td_loop_ub = r.u_vec.size(1);
        for (int i91{0}; i91 < td_loop_ub; i91++) {
            ctx->u_vec[i91] = r.u_vec[i91];
        }
        ctx->q_spline = r.q_spline;
        ctx->q_gcode = r.q_gcode;
        ctx->q_compress = r.q_compress;
        ...
        ctx->cfg.maskTot.size[0] = 1;
        ctx->cfg.maskTot.size[1] = r.cfg.maskTot.size[1];
        wd_loop_ub = r.cfg.maskTot.size[1];
        if (0 <= wd_loop_ub - 1) {
            std::copy(&r.cfg.maskTot.data[0], &r.cfg.maskTot.data[wd_loop_ub],
                      &ctx->cfg.maskTot.data[0]);
        }
        ctx->cfg.maskCart.size[0] = 1;
        ctx->cfg.maskCart.size[1] = r.cfg.maskCart.size[1];
        ae_loop_ub = r.cfg.maskCart.size[1];
        if (0 <= ae_loop_ub - 1) {
            std::copy(&r.cfg.maskCart.data[0], &r.cfg.maskCart.data[ae_loop_ub],
                      &ctx->cfg.maskCart.data[0]);
        }
        ctx->cfg.maskRot.size[0] = 1;
        ctx->cfg.maskRot.size[1] = r.cfg.maskRot.size[1];
        de_loop_ub = r.cfg.maskRot.size[1];
        if (0 <= de_loop_ub - 1) {
            std::copy(&r.cfg.maskRot.data[0], &r.cfg.maskRot.data[de_loop_ub],
                      &ctx->cfg.maskRot.data[0]);
        }
        ctx->cfg.indCart.size[0] = r.cfg.indCart.size(0);
        ge_loop_ub = r.cfg.indCart.size(0);
        for (int i94{0}; i94 < ge_loop_ub; i94++) {
            ctx->cfg.indCart.data[i94] = r.cfg.indCart[i94];
        }
        ctx->cfg.indRot.size[0] = r.cfg.indRot.size(0);
        je_loop_ub = r.cfg.indRot.size(0);
        for (int i97{0}; i97 < je_loop_ub; i97++) {
            ctx->cfg.indRot.data[i97] = r.cfg.indRot[i97];
        }
        ctx->cfg.NumberAxis = r.cfg.NumberAxis;
        ctx->cfg.NCart = r.cfg.NCart;
        ctx->cfg.NRot = r.cfg.NRot;
        ctx->cfg.D.size[0] = r.cfg.D.size(0);
        me_loop_ub = r.cfg.D.size(0);
        for (int i100{0}; i100 < me_loop_ub; i100++) {
            ctx->cfg.D.data[i100] = r.cfg.D[i100];
        }
        ctx->cfg.coeffD = r.cfg.coeffD;
        ctx->cfg.kin_params.size[0] = r.cfg.kin_params.size[0];
        pe_loop_ub = r.cfg.kin_params.size[0];
        if (0 <= pe_loop_ub - 1) {
            std::copy(&r.cfg.kin_params.data[0], &r.cfg.kin_params.data[pe_loop_ub],
                      &ctx->cfg.kin_params.data[0]);
        }
        for (int i103{0}; i103 < 5; i103++) {
            ctx->cfg.kin_type[i103] = r.cfg.kin_type[i103];
        }
        ctx->cfg.NDiscr = r.cfg.NDiscr;
        ctx->cfg.NBreak = r.cfg.NBreak;
        ctx->cfg.UseDynamicBreakpoints = r.cfg.UseDynamicBreakpoints;
        ...
        for (int i106{0}; i106 < 6; i106++) {
            ctx->cfg.vmax[i106] = r.cfg.vmax[i106];
            ctx->cfg.amax[i106] = r.cfg.amax[i106];
            ctx->cfg.jmax[i106] = r.cfg.jmax[i106];
        }
        ctx->cfg.LeeSplineDegree = r.cfg.LeeSplineDegree;
        ctx->cfg.SplineDegree = r.cfg.SplineDegree;
        ctx->cfg.CutOff = r.cfg.CutOff;
        ...
        ctx->cfg.opt = r.cfg.opt;
        for (int i109{0}; i109 < 9; i109++) {
            ctx->cfg.LogFileName[i109] = r.cfg.LogFileName[i109];
        }
        ctx->errcode = r.errcode;
        ctx->jmax_increase_count = r.jmax_increase_count;
        ctx->zero_start = r.zero_start;
        ...
        ctx->programmed_stop = r.programmed_stop;
        ctx->Coeff.set_size(r.Coeff.size(0), r.Coeff.size(1));
        se_loop_ub = r.Coeff.size(1);
        for (int i112{0}; i112 < se_loop_ub; i112++) {
            int ve_loop_ub;
            ve_loop_ub = r.Coeff.size(0);
            for (int i115{0}; i115 < ve_loop_ub; i115++) {
                ctx->Coeff[i115 + ctx->Coeff.size(0) * i112] =
                    r.Coeff[i115 + r.Coeff.size(0) * i112];
            }
        }
        ctx->Skipped = r.Skipped;
        ctx->kin = r.kin;
    ...
    case Fopt_Split: {
        ...
        if (0 <= m_loop_ub - 1) {
            std::copy(&ctx->cfg.maskCart.data[0], &ctx->cfg.maskCart.data[m_loop_ub],
                      &r.cfg.maskCart.data[0]);
        }
        r.cfg.maskRot.size[0] = 1;
        r.cfg.maskRot.size[1] = ctx->cfg.maskRot.size[1];
        w_loop_ub = ctx->cfg.maskRot.size[1];
        if (0 <= w_loop_ub - 1) {
            std::copy(&ctx->cfg.maskRot.data[0], &ctx->cfg.maskRot.data[w_loop_ub],
                      &r.cfg.maskRot.data[0]);
        }
        r.cfg.indCart.set_size(ctx->cfg.indCart.size[0]);
        fb_loop_ub = ctx->cfg.indCart.size[0];
        for (int i26{0}; i26 < fb_loop_ub; i26++) {
            r.cfg.indCart[i26] = ctx->cfg.indCart.data[i26];
        }
        r.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        kb_loop_ub = ctx->cfg.indRot.size[0];
        for (int i31{0}; i31 < kb_loop_ub; i31++) {
            r.cfg.indRot[i31] = ctx->cfg.indRot.data[i31];
        }
        r.cfg.NumberAxis = ctx->cfg.NumberAxis;
        r.cfg.NCart = ctx->cfg.NCart;
        r.cfg.NRot = ctx->cfg.NRot;
        for (int i36{0}; i36 < 6; i36++) {
            r.cfg.vmax[i36] = ctx->cfg.vmax[i36];
            r.cfg.amax[i36] = ctx->cfg.amax[i36];
            r.cfg.jmax[i36] = ctx->cfg.jmax[i36];
        }
        r.cfg.LSplit = ctx->cfg.LSplit;
        r.cfg.LSplitZero = ctx->cfg.LSplitZero;
        r.cfg.dt = ctx->cfg.dt;
        ...
        if (!ctx->q_smooth.isempty()) {
            ...
            i38 = static_cast<int>(b_N);
            if (0 <= static_cast<int>(b_N) - 1) {
                paramsDefaultCurv(
                    ...
                    params_R0, params_R1, params_Cprim, &e_expl_temp, params_evec, &f_expl_temp,
                    &g_expl_temp, params_CoeffP5, &h_expl_temp);
            }
            for (int b_k{0}; b_k < i38; b_k++) {
                // 'splitQueue:12' ctx = splitCurvStruct( ctx, ctx.q_smooth.get( k ) );
                ctx->q_smooth.get(b_k + 1U, &curv);
                ...
                b_constrCurvStruct(
                    ...
                    params_spline_Bl_breakpoints, params_spline_Bl_handle, params_spline_Bl_order,
                    params_spline_coeff, params_spline_knots, params_spline_Ltot, params_spline_Lk,
                    params_R0, params_R1, params_Cprim, params_evec, params_CoeffP5, &curvE);
                ...
                if (curv.Info.Type == CurveType_TransP5) {
                    ...
                    if ((curv.Info.zspdmode == ZSpdMode_ZN) ||
                        ...
                        b_curv = curv;
                        cutZeroStart(&r.q_spline, r.cfg.maskTot.data, r.cfg.maskTot.size,
                                     r.cfg.maskCart.data, r.cfg.maskCart.size, r.cfg.maskRot.data,
                                     r.cfg.maskRot.size, r.cfg.indCart, r.cfg.indRot,
                                     r.cfg.NumberAxis, r.cfg.NCart, r.cfg.NRot, r.cfg.vmax,
                                     r.cfg.amax, r.cfg.jmax, r.cfg.LSplitZero, r.cfg.dt,
                                     r.cfg.GaussLegendreX, r.cfg.GaussLegendreW, &r.kin, &b_curv,
                                     &curvS, &curv);
                    ...
                    if ((curv.Info.zspdmode == ZSpdMode_NZ) ||
                        ...
                        c_curv = curv;
                        cutZeroEnd(&r.q_spline, r.cfg.maskTot.data, r.cfg.maskTot.size,
                                   r.cfg.maskCart.data, r.cfg.maskCart.size, r.cfg.maskRot.data,
                                   r.cfg.maskRot.size, r.cfg.indCart, r.cfg.indRot,
                                   r.cfg.NumberAxis, r.cfg.NCart, r.cfg.NRot, r.cfg.vmax,
                                   r.cfg.amax, r.cfg.jmax, r.cfg.LSplitZero, r.cfg.dt,
                                   r.cfg.GaussLegendreX, r.cfg.GaussLegendreW, &r.kin, &c_curv,
                                   &curv, &curvE);
                    ...
                    L = LengthCurv(&r.q_spline, r.cfg.maskTot.data, r.cfg.maskTot.size,
                                   r.cfg.maskCart.data, r.cfg.maskCart.size, r.cfg.maskRot.data,
                                   r.cfg.maskRot.size, r.cfg.indCart, r.cfg.indRot,
                                   r.cfg.NumberAxis, r.cfg.NCart, r.cfg.NRot, r.cfg.GaussLegendreX,
                                   r.cfg.GaussLegendreW, curv.Info, curv.R0, curv.R1,
                    ...
                    e_k = 0;
                    exitg1 = false;
                    while ((!exitg1) && (e_k <= static_cast<int>(d_N - 1.0) - 1)) {
                        double u1_tilda;
                        // 'splitCurvStruct:42' u1_tilda  = cutCurvStructU( ctx, curv, u0, L_split,
                        // false );
                        u1_tilda = cutCurvStructU(
                            &r.q_spline, r.cfg.maskTot.data, r.cfg.maskTot.size,
                            r.cfg.maskCart.data, r.cfg.maskCart.size, r.cfg.maskRot.data,
                            r.cfg.maskRot.size, r.cfg.indCart, r.cfg.indRot, r.cfg.NumberAxis,
                            r.cfg.NCart, r.cfg.NRot, r.cfg.GaussLegendreX, r.cfg.GaussLegendreW,
                            &curv, L_split);
    ...
    case Fopt_Opt: {
        ...
        for (int i8{0}; i8 < i_loop_ub; i8++) {
            ...
            for (int i12{0}; i12 < n_loop_ub; i12++) {
                b_ctx.BasisValD[i12 + b_ctx.BasisValD.size(0) * i8] =
                    ctx->BasisValD[i12 + ctx->BasisValD.size(0) * i8];
            }
        }
        ...
        for (int i14{0}; i14 < q_loop_ub; i14++) {
            ...
            for (int i18{0}; i18 < u_loop_ub; i18++) {
                b_ctx.BasisValDD[i18 + b_ctx.BasisValDD.size(0) * i14] =
                    ctx->BasisValDD[i18 + ctx->BasisValDD.size(0) * i14];
            }
        }
        ...
        for (int i20{0}; i20 < x_loop_ub; i20++) {
            ...
            for (int i24{0}; i24 < db_loop_ub; i24++) {
                b_ctx.BasisValDDD[i24 + b_ctx.BasisValDDD.size(0) * i20] =
                    ctx->BasisValDDD[i24 + ctx->BasisValDDD.size(0) * i20];
            }
        }
        b_ctx.BasisIntegr.set_size(ctx->BasisIntegr.size(0));
        gb_loop_ub = ctx->BasisIntegr.size(0);
        for (int i27{0}; i27 < gb_loop_ub; i27++) {
            b_ctx.BasisIntegr[i27] = ctx->BasisIntegr[i27];
        }
        b_ctx.Bl = ctx->Bl;
        b_ctx.u_vec.set_size(1, ctx->u_vec.size(1));
        nb_loop_ub = ctx->u_vec.size(1);
        for (int i34{0}; i34 < nb_loop_ub; i34++) {
            b_ctx.u_vec[i34] = ctx->u_vec[i34];
        }
        ...
        if (0 <= wb_loop_ub - 1) {
            std::copy(&ctx->cfg.maskRot.data[0], &ctx->cfg.maskRot.data[wb_loop_ub],
                      &b_ctx.cfg.maskRot.data[0]);
        }
        b_ctx.cfg.indCart.set_size(ctx->cfg.indCart.size[0]);
        ac_loop_ub = ctx->cfg.indCart.size[0];
        for (int i40{0}; i40 < ac_loop_ub; i40++) {
            b_ctx.cfg.indCart[i40] = ctx->cfg.indCart.data[i40];
        }
        b_ctx.cfg.indRot.set_size(ctx->cfg.indRot.size[0]);
        dc_loop_ub = ctx->cfg.indRot.size[0];
        for (int i44{0}; i44 < dc_loop_ub; i44++) {
            b_ctx.cfg.indRot[i44] = ctx->cfg.indRot.data[i44];
        }
        ...
        gc_loop_ub = ctx->cfg.D.size[0];
        for (int i47{0}; i47 < gc_loop_ub; i47++) {
            b_ctx.cfg.D[i47] = ctx->cfg.D.data[i47];
        }
        b_ctx.cfg.coeffD = ctx->cfg.coeffD;
        b_ctx.cfg.kin_params.size[0] = ctx->cfg.kin_params.size[0];
        jc_loop_ub = ctx->cfg.kin_params.size[0];
        if (0 <= jc_loop_ub - 1) {
            std::copy(&ctx->cfg.kin_params.data[0], &ctx->cfg.kin_params.data[jc_loop_ub],
                      &b_ctx.cfg.kin_params.data[0]);
        }
        for (int i50{0}; i50 < 5; i50++) {
            b_ctx.cfg.kin_type[i50] = ctx->cfg.kin_type[i50];
        }
        ...
        for (int i53{0}; i53 < 6; i53++) {
            b_ctx.cfg.vmax[i53] = ctx->cfg.vmax[i53];
            b_ctx.cfg.amax[i53] = ctx->cfg.amax[i53];
            b_ctx.cfg.jmax[i53] = ctx->cfg.jmax[i53];
        }
        ...
        for (int i56{0}; i56 < 9; i56++) {
            b_ctx.cfg.LogFileName[i56] = ctx->cfg.LogFileName[i56];
        }
        ...
        mc_loop_ub = ctx->Coeff.size(1);
        for (int i59{0}; i59 < mc_loop_ub; i59++) {
            int pc_loop_ub;
            pc_loop_ub = ctx->Coeff.size(0);
            for (int i62{0}; i62 < pc_loop_ub; i62++) {
                b_ctx.Coeff[i62 + b_ctx.Coeff.size(0) * i59] =
                    ctx->Coeff[i62 + ctx->Coeff.size(0) * i59];
            }
        }
        ...
        sc_loop_ub = b_ctx.BasisVal.size(1);
        for (int i65{0}; i65 < sc_loop_ub; i65++) {
            int vc_loop_ub;
            vc_loop_ub = b_ctx.BasisVal.size(0);
            for (int i68{0}; i68 < vc_loop_ub; i68++) {
                ctx->BasisVal[i68 + ctx->BasisVal.size(0) * i65] =
                    b_ctx.BasisVal[i68 + b_ctx.BasisVal.size(0) * i65];
            }
        }
        ctx->BasisValD.set_size(b_ctx.BasisValD.size(0), b_ctx.BasisValD.size(1));
        xc_loop_ub = b_ctx.BasisValD.size(1);
        for (int i70{0}; i70 < xc_loop_ub; i70++) {
            int cd_loop_ub;
            cd_loop_ub = b_ctx.BasisValD.size(0);
            for (int i74{0}; i74 < cd_loop_ub; i74++) {
                ctx->BasisValD[i74 + ctx->BasisValD.size(0) * i70] =
                    b_ctx.BasisValD[i74 + b_ctx.BasisValD.size(0) * i70];
            }
        }
        ctx->BasisValDD.set_size(b_ctx.BasisValDD.size(0), b_ctx.BasisValDD.size(1));
        ed_loop_ub = b_ctx.BasisValDD.size(1);
        for (int i76{0}; i76 < ed_loop_ub; i76++) {
            int id_loop_ub;
            id_loop_ub = b_ctx.BasisValDD.size(0);
            for (int i80{0}; i80 < id_loop_ub; i80++) {
                ctx->BasisValDD[i80 + ctx->BasisValDD.size(0) * i76] =
                    b_ctx.BasisValDD[i80 + b_ctx.BasisValDD.size(0) * i76];
            }
        }
        ctx->BasisValDDD.set_size(b_ctx.BasisValDDD.size(0), b_ctx.BasisValDDD.size(1));
        kd_loop_ub = b_ctx.BasisValDDD.size(1);
        for (int i82{0}; i82 < kd_loop_ub; i82++) {
            int od_loop_ub;
            od_loop_ub = b_ctx.BasisValDDD.size(0);
            for (int i86{0}; i86 < od_loop_ub; i86++) {
                ctx->BasisValDDD[i86 + ctx->BasisValDDD.size(0) * i82] =
                    b_ctx.BasisValDDD[i86 + b_ctx.BasisValDDD.size(0) * i82];
            }
        }
        ctx->BasisIntegr.set_size(b_ctx.BasisIntegr.size(0));
        qd_loop_ub = b_ctx.BasisIntegr.size(0);
        for (int i88{0}; i88 < qd_loop_ub; i88++) {
            ctx->BasisIntegr[i88] = b_ctx.BasisIntegr[i88];
        }
        ctx->Bl = b_ctx.Bl;
        ctx->u_vec.set_size(1, b_ctx.u_vec.size(1));
        ud_loop_ub = b_ctx.u_vec.size(1);
        for (int i92{0}; i92 < ud_loop_ub; i92++) {
            ctx->u_vec[i92] = b_ctx.u_vec[i92];
        }
        ...
        xd_loop_ub = b_ctx.cfg.maskTot.size[1];
        if (0 <= xd_loop_ub - 1) {
            std::copy(&b_ctx.cfg.maskTot.data[0], &b_ctx.cfg.maskTot.data[xd_loop_ub],
                      &ctx->cfg.maskTot.data[0]);
        }
        ctx->cfg.maskCart.size[0] = 1;
        ctx->cfg.maskCart.size[1] = b_ctx.cfg.maskCart.size[1];
        be_loop_ub = b_ctx.cfg.maskCart.size[1];
        if (0 <= be_loop_ub - 1) {
            std::copy(&b_ctx.cfg.maskCart.data[0], &b_ctx.cfg.maskCart.data[be_loop_ub],
                      &ctx->cfg.maskCart.data[0]);
        }
        ctx->cfg.maskRot.size[0] = 1;
        ctx->cfg.maskRot.size[1] = b_ctx.cfg.maskRot.size[1];
        ee_loop_ub = b_ctx.cfg.maskRot.size[1];
        if (0 <= ee_loop_ub - 1) {
            std::copy(&b_ctx.cfg.maskRot.data[0], &b_ctx.cfg.maskRot.data[ee_loop_ub],
                      &ctx->cfg.maskRot.data[0]);
        }
        ctx->cfg.indCart.size[0] = b_ctx.cfg.indCart.size(0);
        he_loop_ub = b_ctx.cfg.indCart.size(0);
        for (int i95{0}; i95 < he_loop_ub; i95++) {
            ctx->cfg.indCart.data[i95] = b_ctx.cfg.indCart[i95];
        }
        ctx->cfg.indRot.size[0] = b_ctx.cfg.indRot.size(0);
        ke_loop_ub = b_ctx.cfg.indRot.size(0);
        for (int i98{0}; i98 < ke_loop_ub; i98++) {
            ctx->cfg.indRot.data[i98] = b_ctx.cfg.indRot[i98];
        }
        ...
        ne_loop_ub = b_ctx.cfg.D.size(0);
        for (int i101{0}; i101 < ne_loop_ub; i101++) {
            ctx->cfg.D.data[i101] = b_ctx.cfg.D[i101];
        }
        ctx->cfg.coeffD = b_ctx.cfg.coeffD;
        ctx->cfg.kin_params.size[0] = b_ctx.cfg.kin_params.size[0];
        qe_loop_ub = b_ctx.cfg.kin_params.size[0];
        if (0 <= qe_loop_ub - 1) {
            std::copy(&b_ctx.cfg.kin_params.data[0], &b_ctx.cfg.kin_params.data[qe_loop_ub],
                      &ctx->cfg.kin_params.data[0]);
        }
        for (int i104{0}; i104 < 5; i104++) {
            ctx->cfg.kin_type[i104] = b_ctx.cfg.kin_type[i104];
        }
        ...
        for (int i107{0}; i107 < 6; i107++) {
            ctx->cfg.vmax[i107] = b_ctx.cfg.vmax[i107];
            ctx->cfg.amax[i107] = b_ctx.cfg.amax[i107];
            ctx->cfg.jmax[i107] = b_ctx.cfg.jmax[i107];
        }
        ...
        for (int i110{0}; i110 < 9; i110++) {
            ctx->cfg.LogFileName[i110] = b_ctx.cfg.LogFileName[i110];
        }
        ...
        te_loop_ub = b_ctx.Coeff.size(1);
        for (int i113{0}; i113 < te_loop_ub; i113++) {
            int we_loop_ub;
            we_loop_ub = b_ctx.Coeff.size(0);
            for (int i116{0}; i116 < we_loop_ub; i116++) {
                ctx->Coeff[i116 + ctx->Coeff.size(0) * i113] =
                    b_ctx.Coeff[i116 + b_ctx.Coeff.size(0) * i113];
            }
        }
