    ...
    ::coder::array<double, 1U> &beq, ::coder::array<double, 2U> &continuity)
{
    ...
    for (int b_k{0}; b_k < i24; b_k++) {
        ...
        for (int i42{0}; i42 < fb_loop_ub; i42++) {
            ...
            for (int i45{0}; i45 < gb_loop_ub; i45++) {
                double d_varargin_1;
                d_varargin_1 = r1D_a[i45 + r1D_a.size(0) * i42];
                r[i45 + r.size(0) * i42] = std::pow(d_varargin_1, 2.0);
            }
        }
        if (r.size(0) == b_trueCount) {
            ...
            for (int i47{0}; i47 < hb_loop_ub; i47++) {
                for (int i49{0}; i49 < b_trueCount; i49++) {
                    v_max[i49 + v_max.size(0) * i47] =
                        ctx_cfg_vmax[b_tmp_data[i49] - 1] / r[i49 + r.size(0) * i47];
                }
            }
        } else {
        ...
        kb_loop_ub = r1D_r.size(1);
        b_r1D_r.set_size(ctx_cfg_indCart.size(0), r1D_r.size(1));
        for (int i53{0}; i53 < kb_loop_ub; i53++) {
            int nb_loop_ub;
            nb_loop_ub = ctx_cfg_indCart.size(0);
            for (int i58{0}; i58 < nb_loop_ub; i58++) {
                b_r1D_r[i58 + b_r1D_r.size(0) * i53] =
                    r1D_r[(ctx_cfg_indCart[i58] + r1D_r.size(0) * i53) - 1];
            }
        }
        ...
        for (int i62{0}; i62 < pb_loop_ub; i62++) {
            double e_varargin_1;
            e_varargin_1 = windowCurv[b_k].Info.FeedRate / r2[i62];
            v_max[b_v_max + v_max.size(0) * i62] = std::pow(e_varargin_1, 2.0);
        }
        ...
        for (i71 = 0; i71 <= vectorUB; i71 += 4) {
            __m128i r7;
            r7 = _mm_loadu_si128((const __m128i *)&r1[i71]);
            _mm_storeu_si128((__m128i *)&indAL[i71], _mm_add_epi32(r7, _mm_set1_epi32(f_y)));
        }
        for (i71 = scalarLB; i71 < n_loop_ub; i71++) {
            indAL[i71] = r1[i71] + f_y;
        }
    ...
    if (windowCurv.size(1) > 1) {
        ...
        for (int i50{0}; i50 < input_sizes_idx_1; i50++) {
            for (int i52{0}; i52 < b_result; i52++) {
                ramp[i52 + ramp.size(0) * i50] = 1.0;
            }
        }
        mb_loop_ub = b_input_sizes_idx_1;
        for (int i55{0}; i55 < mb_loop_ub; i55++) {
            for (int i57{0}; i57 < b_result; i57++) {
                ramp[i57 + ramp.size(0) * input_sizes_idx_1] = vel_ramp[i57];
            }
        }
        for (int i56{0}; i56 < c_input_sizes_idx_1; i56++) {
            for (int i61{0}; i61 < b_result; i61++) {
                ramp[i61 + ramp.size(0) * ((i56 + input_sizes_idx_1) + b_input_sizes_idx_1)] =
                    acc_ramp[i61 + b_result * i56];
            }
        }
        for (int i60{0}; i60 < d_input_sizes_idx_1; i60++) {
            for (int i63{0}; i63 < b_result; i63++) {
                ramp[i63 + ramp.size(0) * (((i60 + input_sizes_idx_1) + b_input_sizes_idx_1) +
                                           c_input_sizes_idx_1)] = varargin_4[i63 + b_result * i60];
            }
        }
        ...
        if (b.size(0) == ramp.size(0) * ramp.size(1)) {
            ...
            for (i65 = 0; i65 <= c_vectorUB; i65 += 2) {
                __m128d r5;
                __m128d r6;
                r5 = _mm_loadu_pd(&b[i65]);
                r6 = _mm_loadu_pd(&ramp[i65]);
                _mm_storeu_pd(&b[i65], _mm_mul_pd(r5, r6));
            }
            for (i65 = c_scalarLB; i65 < rb_loop_ub; i65++) {
    ...
    for (int i48{0}; i48 < ib_loop_ub; i48++) {
        int jb_loop_ub;
        jb_loop_ub = c_varargin_1.size(0);
        for (int i51{0}; i51 < jb_loop_ub; i51++) {
            A[i51 + A.size(0) * i48] = c_varargin_1[i51 + c_varargin_1.size(0) * i48];
        }
    }
    ...
    sb_loop_ub = v2_vec.size(1);
    tb_loop_ub = at_norm.size(1);
    ...
    for (int i69{0}; i69 < tb_loop_ub; i69++) {
        continuity[2 * i69 + 1] = at_norm[2 * i69 + 1];
    }
