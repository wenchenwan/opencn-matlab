                 ...
                 ::coder::array<double, 1U> &beq, ::coder::array<double, 2U> &continuity)
{
    ...
    for (int b_k{0}; b_k < i22; b_k++) {
        ...
        b_iv[0] = 0;
        b_iv[1] = r1D.size(1) - 1;
        p_loop_ub = r1D.size(0);
        x.set_size(r1D.size(0), 2);
        iv1[0] = 0;
        iv1[1] = normR1D.size(1) - 1;
        for (int i27{0}; i27 < 2; i27++) {
            for (int i29{0}; i29 < p_loop_ub; i29++) {
                x[i29 + x.size(0) * i27] = r1D[i29 + r1D.size(0) * b_iv[i27]];
            }
            d_y[i27] = normR1D[iv1[i27]];
        }
        s_loop_ub = x.size(0);
        b_scalarLB = (s_loop_ub / 2) << 1;
        b_vectorUB = b_scalarLB - 2;
        for (int i32{0}; i32 < 2; i32++) {
            int i33;
            for (i33 = 0; i33 <= b_vectorUB; i33 += 2) {
                __m128d r4;
                r4 = _mm_loadu_pd(&x[i33 + x.size(0) * i32]);
                _mm_storeu_pd(&t_vec[(i33 + t_vec.size(0) * i32) + t_vec.size(0) * 2 * b_k],
                              _mm_div_pd(r4, _mm_set1_pd(d_y[i32])));
            }
            for (i33 = b_scalarLB; i33 < s_loop_ub; i33++) {
                t_vec[(i33 + t_vec.size(0) * i32) + t_vec.size(0) * 2 * b_k] =
                    x[i33 + x.size(0) * i32] / d_y[i32];
            }
        }
        //  Maximum constraint on the speed
        // 'buildConstr:55' v_max = ( windowCurv( k ).Info.FeedRate ./ normR1D ).^2;
        v_max.set_size(1, normR1D.size(1));
        u_loop_ub = normR1D.size(1);
        ...
        if (0 <= ctx_cfg_NumberAxis - 1) {
            ...
            i39 = r1D.size(1);
            w_loop_ub = r1D.size(1);
            y_loop_ub = e_y.size(1);
            ab_loop_ub = BasisVal.size(1);
        }
        for (int j{0}; j < ctx_cfg_NumberAxis; j++) {
            ...
            for (int i44{0}; i44 < w_loop_ub; i44++) {
                r3[i44] = 0.5 * r1D[j + r1D.size(0) * i44];
            }
            r6.set_size(e_y.size(1));
            for (int i49{0}; i49 < y_loop_ub; i49++) {
                r6[i49] = static_cast<int>(e_y[i49]) + g_y;
            }
            if (r2D.size(1) == 1) {
                i53 = BasisVal.size(0);
            } else {
            ...
            if ((BasisVal.size(0) == r2D.size(1)) && (r3.size(0) == BasisValD.size(0)) &&
                (i53 == i58) && (BasisVal.size(1) == BasisValD.size(1))) {
                for (int i61{0}; i61 < ab_loop_ub; i61++) {
                    int lb_loop_ub;
                    lb_loop_ub = r2D.size(1);
                    for (int i63{0}; i63 < lb_loop_ub; i63++) {
                        Acc[r6[i63] + Acc.size(0) * i61] =
                            r2D[j + r2D.size(0) * i63] * BasisVal[i63 + BasisVal.size(0) * i61] +
                            r3[i63] * BasisValD[i63 + BasisValD.size(0) * i61];
                    }
                }
            } else {
                binary_expand_op(Acc, r6, r2D, j, r2D.size(1), BasisVal, r3, BasisValD);
            }
        }
        ...
        for (i42 = 0; i42 <= vectorUB; i42 += 4) {
            __m128i r5;
            r5 = _mm_loadu_si128((const __m128i *)&r1[i42]);
            _mm_storeu_si128((__m128i *)&indAL[i42], _mm_add_epi32(r5, _mm_set1_epi32(f_y)));
        }
        for (i42 = scalarLB; i42 < l_loop_ub; i42++) {
            indAL[i42] = r1[i42] + f_y;
        }
        ...
        if (N < 1) {
            ...
            e_y.set_size(1, N);
            fb_loop_ub = N - 1;
            for (int i56{0}; i56 <= fb_loop_ub; i56++) {
                e_y[i56] = i56 + 1U;
            }
        }
        c_k = ((static_cast<double>(b_k) + 1.0) - 1.0) * static_cast<double>(N);
        indAC_tmp.set_size(1, e_y.size(1));
        hb_loop_ub = e_y.size(1);
        for (int i60{0}; i60 < hb_loop_ub; i60++) {
            indAC_tmp[i60] = static_cast<int>(static_cast<double>(e_y[i60]) + c_k);
        }
        // 'buildConstr:65' A( indAL, indAC )   = [ BasisVal; -BasisVal ; Acc ; -Acc ];
        r6.set_size(indAL.size(1));
        kb_loop_ub = indAL.size(1);
        d_scalarLB = (indAL.size(1) / 4) << 2;
        d_vectorUB = d_scalarLB - 4;
        for (i65 = 0; i65 <= d_vectorUB; i65 += 4) {
            __m128i r9;
            r9 = _mm_loadu_si128((const __m128i *)&indAL[i65]);
            _mm_storeu_si128((__m128i *)&r6[i65], _mm_sub_epi32(r9, _mm_set1_epi32(1)));
        }
        for (i65 = d_scalarLB; i65 < kb_loop_ub; i65++) {
            r6[i65] = indAL[i65] - 1;
        }
        r10.set_size(indAC_tmp.size(1));
        nb_loop_ub = indAC_tmp.size(1);
        e_scalarLB = (indAC_tmp.size(1) / 4) << 2;
        e_vectorUB = e_scalarLB - 4;
        for (i67 = 0; i67 <= e_vectorUB; i67 += 4) {
            __m128i r11;
            r11 = _mm_loadu_si128((const __m128i *)&indAC_tmp[i67]);
            _mm_storeu_si128((__m128i *)&r10[i67], _mm_sub_epi32(r11, _mm_set1_epi32(1)));
        }
        for (i67 = e_scalarLB; i67 < nb_loop_ub; i67++) {
            r10[i67] = indAC_tmp[i67] - 1;
        }
        ob_loop_ub = BasisVal.size(1);
        varargin_2.set_size(BasisVal.size(0), BasisVal.size(1));
        for (int i68{0}; i68 < ob_loop_ub; i68++) {
            int f_scalarLB;
            int f_vectorUB;
            int i70;
            int qb_loop_ub;
            qb_loop_ub = BasisVal.size(0);
            f_scalarLB = (BasisVal.size(0) / 2) << 1;
            f_vectorUB = f_scalarLB - 2;
            for (i70 = 0; i70 <= f_vectorUB; i70 += 2) {
                _mm_storeu_pd(
                    &varargin_2[i70 + varargin_2.size(0) * i68],
                    _mm_mul_pd(
                        _mm_loadu_pd((const double *)&BasisVal[i70 + BasisVal.size(0) * i68]),
                        _mm_set1_pd(-1.0)));
            }
            for (i70 = f_scalarLB; i70 < qb_loop_ub; i70++) {
                varargin_2[i70 + varargin_2.size(0) * i68] =
                    -BasisVal[i70 + BasisVal.size(0) * i68];
            }
        }
        pb_loop_ub = Acc.size(1);
        varargin_4.set_size(Acc.size(0), Acc.size(1));
        for (int i69{0}; i69 < pb_loop_ub; i69++) {
            int g_scalarLB;
            int g_vectorUB;
            int i71;
            int rb_loop_ub;
            rb_loop_ub = Acc.size(0);
            g_scalarLB = (Acc.size(0) / 2) << 1;
            g_vectorUB = g_scalarLB - 2;
            for (i71 = 0; i71 <= g_vectorUB; i71 += 2) {
                __m128d r12;
                r12 = _mm_loadu_pd(&Acc[i71 + Acc.size(0) * i69]);
                _mm_storeu_pd(&varargin_4[i71 + varargin_4.size(0) * i69],
                              _mm_mul_pd(r12, _mm_set1_pd(-1.0)));
            }
            for (i71 = g_scalarLB; i71 < rb_loop_ub; i71++) {
                varargin_4[i71 + varargin_4.size(0) * i69] = -Acc[i71 + Acc.size(0) * i69];
            }
        }
        ...
        if (c_empty_non_axis_sizes || ((BasisVal.size(0) != 0) && (BasisVal.size(1) != 0))) {
            sb_loop_ub = BasisVal.size(0);
        } else {
            sb_loop_ub = 0;
        }
        if (c_empty_non_axis_sizes || ((varargin_2.size(0) != 0) && (varargin_2.size(1) != 0))) {
            tb_loop_ub = varargin_2.size(0);
        } else {
            tb_loop_ub = 0;
        }
        if (c_empty_non_axis_sizes || ((Acc.size(0) != 0) && (Acc.size(1) != 0))) {
            ub_loop_ub = Acc.size(0);
        } else {
            ub_loop_ub = 0;
        }
        if (c_empty_non_axis_sizes || ((varargin_4.size(0) != 0) && (varargin_4.size(1) != 0))) {
            vb_loop_ub = varargin_4.size(0);
        } else {
            vb_loop_ub = 0;
        }
        for (int i72{0}; i72 < c_result; i72++) {
            for (int i74{0}; i74 < sb_loop_ub; i74++) {
                A[r6[i74] + A.size(0) * r10[i72]] = BasisVal[i74 + sb_loop_ub * i72];
            }
        }
        for (int i73{0}; i73 < c_result; i73++) {
            for (int i76{0}; i76 < tb_loop_ub; i76++) {
                A[r6[i76 + sb_loop_ub] + A.size(0) * r10[i73]] = varargin_2[i76 + tb_loop_ub * i73];
            }
        }
        for (int i75{0}; i75 < c_result; i75++) {
            for (int i78{0}; i78 < ub_loop_ub; i78++) {
                A[r6[(i78 + sb_loop_ub) + tb_loop_ub] + A.size(0) * r10[i75]] =
                    Acc[i78 + ub_loop_ub * i75];
            }
        }
        for (int i77{0}; i77 < c_result; i77++) {
            for (int i79{0}; i79 < vb_loop_ub; i79++) {
                A[r6[((i79 + sb_loop_ub) + tb_loop_ub) + ub_loop_ub] + A.size(0) * r10[i77]] =
                    varargin_4[i79 + vb_loop_ub * i77];
            }
        }
        ...
        wb_loop_ub = v_max.size(1);
        for (int i80{0}; i80 < wb_loop_ub; i80++) {
            b[indAL[i80] - 1] = v_max[i80];
        }
        xb_loop_ub = v_max.size(1);
        for (int i81{0}; i81 < xb_loop_ub; i81++) {
        ...
        for (int i83{0}; i83 < m_loop_ub; i83++) {
        ...
        for (int b_j{0}; b_j <= nc; b_j++) {
            ...
            for (int d_k{0}; d_k <= inner; d_k++) {
                C[b_j] = C[b_j] + t_vec[d_k + t_vec.size(0) * 2 * b_k] *
                                      Acc[(indAT[2 * d_k] + Acc.size(0) * b_j) - 1];
            }
        }
        yb_loop_ub = C.size(1);
        for (int i84{0}; i84 < yb_loop_ub; i84++) {
        ...
        for (int c_j{0}; c_j <= b_nc; c_j++) {
            ...
            for (int e_k{0}; e_k <= b_inner; e_k++) {
                C[c_j] = C[c_j] + t_vec[(e_k + t_vec.size(0)) + t_vec.size(0) * 2 * b_k] *
                                      Acc[(indAT[2 * e_k + 1] + Acc.size(0) * c_j) - 1];
            }
        }
        ac_loop_ub = C.size(1);
        ...
        if (r6.size(0) == v2_vec.size(1)) {
            ...
            b_Aeq.set_size(4, r6.size(0));
            fc_loop_ub = r6.size(0);
            for (int i92{0}; i92 < fc_loop_ub; i92++) {
                b_Aeq[4 * i92] =
                    Aeq[(indAEL[0] + Aeq.size(0) * (r6[i92] - 1)) - 1] + b_v2_vec[4 * i92];
                b_Aeq[4 * i92 + 1] =
                    Aeq[(indAEL[1] + Aeq.size(0) * (r6[i92] - 1)) - 1] + b_v2_vec[4 * i92 + 1];
                b_Aeq[4 * i92 + 2] =
                    Aeq[(indAEL[2] + Aeq.size(0) * (r6[i92] - 1)) - 1] + -b_v2_vec[4 * i92 + 2];
                b_Aeq[4 * i92 + 3] =
                    Aeq[(indAEL[3] + Aeq.size(0) * (r6[i92] - 1)) - 1] + -b_v2_vec[4 * i92 + 3];
            }
            gc_loop_ub = b_Aeq.size(1);
            for (int i93{0}; i93 < gc_loop_ub; i93++) {
                Aeq[(indAEL[0] + Aeq.size(0) * (r6[i93] - 1)) - 1] = b_Aeq[4 * i93];
                Aeq[(indAEL[1] + Aeq.size(0) * (r6[i93] - 1)) - 1] = b_Aeq[4 * i93 + 1];
                Aeq[(indAEL[2] + Aeq.size(0) * (r6[i93] - 1)) - 1] = b_Aeq[4 * i93 + 2];
                Aeq[(indAEL[3] + Aeq.size(0) * (r6[i93] - 1)) - 1] = b_Aeq[4 * i93 + 3];
            }
        } else {
            binary_expand_op(Aeq, indAEL, r6, v2_vec, b_k, at_norm, mask_continuity);
        }
    }
    ...
    if (windowCurv.size(1) > 1) {
        ...
        for (int i43{0}; i43 < input_sizes_idx_1; i43++) {
            for (int i46{0}; i46 < b_result; i46++) {
                ramp[i46 + ramp.size(0) * i43] = 1.0;
            }
        }
        db_loop_ub = b_input_sizes_idx_1;
        for (int i48{0}; i48 < db_loop_ub; i48++) {
            for (int i51{0}; i51 < b_result; i51++) {
                ramp[i51 + ramp.size(0) * input_sizes_idx_1] = vel_ramp[i51];
            }
        }
        for (int i50{0}; i50 < c_input_sizes_idx_1; i50++) {
            for (int i55{0}; i55 < b_result; i55++) {
                ramp[i55 + ramp.size(0) * ((i50 + input_sizes_idx_1) + b_input_sizes_idx_1)] =
                    acc_ramp[i55 + b_result * i50];
            }
        }
        for (int i54{0}; i54 < d_input_sizes_idx_1; i54++) {
            for (int i57{0}; i57 < b_result; i57++) {
                ramp[i57 + ramp.size(0) * (((i54 + input_sizes_idx_1) + b_input_sizes_idx_1) +
                                           c_input_sizes_idx_1)] = varargin_4[i57 + b_result * i54];
            }
        }
        ...
        if (b.size(0) == ramp.size(0) * ramp.size(1)) {
            ...
            for (i62 = 0; i62 <= c_vectorUB; i62 += 2) {
                __m128d r7;
                __m128d r8;
                r7 = _mm_loadu_pd(&b[i62]);
                r8 = _mm_loadu_pd(&ramp[i62]);
                _mm_storeu_pd(&b[i62], _mm_mul_pd(r7, r8));
            }
            for (i62 = c_scalarLB; i62 < ib_loop_ub; i62++) {
                b[i62] = b[i62] * ramp[i62];
            }
        } else {
    ...
    for (int i28{0}; i28 < r_loop_ub; i28++) {
        ...
        for (int i31{0}; i31 < t_loop_ub; i31++) {
            b_varargin_1[i31 + b_varargin_1.size(0) * i28] = -1;
        }
    }
    ...
    for (int i35{0}; i35 < result; i35++) {
        for (int i37{0}; i37 < input_sizes_idx_0; i37++) {
            d_varargin_1[i37 + d_varargin_1.size(0) * i35] = -1.0;
        }
    }
    for (int i36{0}; i36 < result; i36++) {
        for (int i40{0}; i40 < b_input_sizes_idx_0; i40++) {
            d_varargin_1[(i40 + input_sizes_idx_0) + d_varargin_1.size(0) * i36] =
                A[i40 + b_input_sizes_idx_0 * i36];
        }
    }
    A.set_size(d_varargin_1.size(0), d_varargin_1.size(1));
    x_loop_ub = d_varargin_1.size(1);
    for (int i41{0}; i41 < x_loop_ub; i41++) {
        int bb_loop_ub;
        bb_loop_ub = d_varargin_1.size(0);
        for (int i45{0}; i45 < bb_loop_ub; i45++) {
            A[i45 + A.size(0) * i41] = d_varargin_1[i45 + d_varargin_1.size(0) * i41];
        }
    }
    ...
    eb_loop_ub = b.size(0);
    for (int i52{0}; i52 < eb_loop_ub; i52++) {
        r3[i52 + A.size(1)] = b[i52];
    }
    b.set_size(r3.size(0));
    gb_loop_ub = r3.size(0);
    for (int i59{0}; i59 < gb_loop_ub; i59++) {
        b[i59] = r3[i59];
    }
    //  Continuity equations
    // 'buildConstr:97' continuity = [ v2_vec( 2, : , 1 ); at_norm( 2, :, 1 ) ];
    jb_loop_ub = v2_vec.size(1);
    mb_loop_ub = at_norm.size(1);
    continuity.set_size(2, v2_vec.size(1));
    for (int i64{0}; i64 < jb_loop_ub; i64++) {
        continuity[2 * i64] = v2_vec[2 * i64 + 1];
    }
    for (int i66{0}; i66 < mb_loop_ub; i66++) {
        continuity[2 * i66 + 1] = at_norm[2 * i66 + 1];
    }
