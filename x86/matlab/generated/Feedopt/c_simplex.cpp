               ...
               int *status)
{
    ...
    for (i5 = 0; i5 <= vectorUB; i5 += 4) {
        __m128i r;
        r = _mm_loadu_si128((const __m128i *)&i_tmp[i5]);
    ...
    for (i8 = 0; i8 <= b_vectorUB; i8 += 4) {
        __m128i r1;
        r1 = _mm_loadu_si128((const __m128i *)&j[i8]);
        _mm_storeu_si128((__m128i *)&Ajs[i8], _mm_sub_epi32(r1, _mm_set1_epi32(1)));
    }
    for (i8 = b_scalarLB; i8 < c_loop_ub; i8++) {
        Ajs[i8] = j[i8] - 1;
    }
