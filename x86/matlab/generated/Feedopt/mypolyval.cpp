        ...
        if ((r.size(0) == y.size(0)) && (i6 == r1.size(0)) && (y.size(1) == r1.size(1))) {
            ...
            for (int i7{0}; i7 < c_loop_ub; i7++) {
                ...
                for (int i9{0}; i9 <= vectorUB; i9 += 2) {
                    ...
                    __m128d r4;
                    __m128d r5;
                    r3 = _mm_loadu_pd(&r[i9]);
                    r4 = _mm_loadu_pd(&y[i9 + y.size(0) * i7]);
                    r5 = _mm_loadu_pd(&r1[i9 + r1.size(0) * i7]);
                    _mm_storeu_pd(&r2[i9 + r2.size(0) * i7], _mm_add_pd(_mm_mul_pd(r3, r4), r5));
                }
                for (int i9{scalarLB}; i9 < d_loop_ub; i9++) {
                    r2[i9 + r2.size(0) * i7] =
                        r[i9] * y[i9 + y.size(0) * i7] + r1[i9 + r1.size(0) * i7];
                }
            }
            ...
            for (int i8{0}; i8 < e_loop_ub; i8++) {
                ...
                for (int i10{0}; i10 < f_loop_ub; i10++) {
                    r1[i10 + r1.size(0) * i8] = r2[i10 + r2.size(0) * i8];
                }
            }
        ...
        if ((r.size(0) == y.size(0)) && (i6 == r1.size(0))) {
            ...
            for (int i7{0}; i7 < 2; i7++) {
                for (int i8{0}; i8 <= vectorUB; i8 += 2) {
                    ...
                    __m128d r3;
                    __m128d r4;
                    r2 = _mm_loadu_pd(&r[i8 + r.size(0) * i7]);
                    r3 = _mm_loadu_pd(&y[i8 + y.size(0) * i7]);
                    r4 = _mm_loadu_pd(&r1[i8 + r1.size(0) * i7]);
                    _mm_storeu_pd(&y[i8 + y.size(0) * i7], _mm_add_pd(_mm_mul_pd(r2, r3), r4));
                }
                for (int i8{scalarLB}; i8 < b_loop_ub; i8++) {
                    y[i8 + y.size(0) * i7] =
    ...
    for (int b_i{0}; b_i <= i1 - 2; b_i++) {
        ...
        if ((p.size(0) == y.size(0)) && (i4 == r.size(0))) {
            ...
            for (int i5{0}; i5 <= vectorUB; i5 += 2) {
                __m128d r1;
                __m128d r2;
                r1 = _mm_loadu_pd(&y[i5]);
                r2 = _mm_loadu_pd(&r[i5]);
                _mm_storeu_pd(&y[i5], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(x), r1), r2));
            }
