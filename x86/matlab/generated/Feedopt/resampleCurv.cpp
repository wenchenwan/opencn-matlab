        ...
        if (u > 1.0) {
            ...
            if (curv_mode == ZSpdMode_NN) {
                ...
                for (int i3{0}; i3 <= vectorUB; i3 += 2) {
                    __m128d b_r;
                    __m128d r1;
                    b_r = _mm_loadu_pd((const double *)&GaussLegendreX[i3]);
                    r1 = _mm_set1_pd(1.0);
                    _mm_storeu_pd(&uval[i3], _mm_div_pd(_mm_add_pd(_mm_mul_pd(_mm_set1_pd(state->u),
                                                                              _mm_sub_pd(r1, b_r)),
                                                                   _mm_add_pd(b_r, r1)),
                                                        _mm_set1_pd(2.0)));
                }
                for (int i3{scalarLB}; i3 < b_loop_ub; i3++) {
                    uval[i3] =
                ...
                for (int b_k{b_scalarLB}; b_k < i11; b_k++) {
