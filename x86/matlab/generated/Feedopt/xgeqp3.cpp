            ...
            ::coder::array<int, 2U> &jpvt)
{
    ...
    if (A.size(0) == 0) {
        ...
        if (y < 1) {
            ...
            for (k = 0; k <= b_vectorUB; k += 4) {
                _mm_storeu_si128(
                    (__m128i *)&jpvt[k],
                    _mm_add_epi32(
                        _mm_add_epi32(_mm_set1_epi32(k), _mm_loadu_si128((const __m128i *)&iv[0])),
                        _mm_set1_epi32(1)));
            }
            for (k = b_scalarLB; k < n; k++) {
                jpvt[k] = k + 1;
            }
            tau.set_size(minmana);
