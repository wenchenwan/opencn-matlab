                ...
                CurvStruct *curv2)
{
    ...
    u = b_cutCurvStructU(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size, ctx_cfg_maskCart,
    ...
    *curv2 = *curv;
    ...
    while (searchJps) {
        ...
        if ((r2D.size(1) == r1.size(1)) && (c_r1D.size(1) == udd_vec.size(1)) &&
            (r2D.size(0) == c_r1D.size(0)) && (i23 == i24)) {
            int s_loop_ub;
            ...
            for (int i25{0}; i25 < s_loop_ub; i25++) {
                ...
                for (i27 = 0; i27 <= f_vectorUB; i27 += 2) {
                    __m128d r6;
                    __m128d r7;
                    r6 = _mm_loadu_pd(&r2D[i27 + r2D.size(0) * i25]);
                    r7 = _mm_loadu_pd(&c_r1D[i27 + c_r1D.size(0) * i25]);
                    _mm_storeu_pd(&A[i27 + A.size(0) * i25],
                                  _mm_add_pd(_mm_mul_pd(r6, _mm_set1_pd(r1[i25])),
                                             _mm_mul_pd(r7, _mm_set1_pd(udd_vec[i25]))));
                }
                for (i27 = f_scalarLB; i27 < u_loop_ub; i27++) {
                    A[i27 + A.size(0) * i25] = r2D[i27 + r2D.size(0) * i25] * r1[i25] +
                                               c_r1D[i27 + c_r1D.size(0) * i25] * udd_vec[i25];
                }
            }
        } else {
        ...
        for (int i26{0}; i26 < t_loop_ub; i26++) {
            double d_varargin_1;
            d_varargin_1 = ud[i26];
            r1[i26] = std::pow(d_varargin_1, 3.0);
        }
        if (r2D.size(1) == 1) {
            i28 = ud.size(1);
        } else {
        ...
        if ((r3D.size(1) == r1.size(1)) && (r2D.size(1) == ud.size(1)) &&
            (i28 == udd_vec.size(1)) && (r3D.size(0) == r2D.size(0)) && (i29 == i31) &&
            (c_r1D.size(1) == y.size(1)) && (i32 == c_r1D.size(0)) && (i35 == i36)) {
            int v_loop_ub;
            ...
            for (int i37{0}; i37 < v_loop_ub; i37++) {
                ...
                for (i38 = 0; i38 <= g_vectorUB; i38 += 2) {
                    ...
                    __m128d r8;
                    __m128d r9;
                    r8 = _mm_loadu_pd(&r3D[i38 + r3D.size(0) * i37]);
                    r9 = _mm_loadu_pd(&r2D[i38 + r2D.size(0) * i37]);
                    r10 = _mm_loadu_pd(&c_r1D[i38 + c_r1D.size(0) * i37]);
                    _mm_storeu_pd(
                        &c_J[i38 + c_J.size(0) * i37],
                        _mm_add_pd(
                            _mm_add_pd(_mm_mul_pd(r8, _mm_set1_pd(r1[i37])),
                                       _mm_mul_pd(_mm_mul_pd(_mm_mul_pd(_mm_set1_pd(3.0), r9),
                                                             _mm_set1_pd(ud[i37])),
                                                  _mm_set1_pd(udd_vec[i37]))),
                            _mm_mul_pd(r10, _mm_set1_pd(jps))));
                }
                for (i38 = g_scalarLB; i38 < w_loop_ub; i38++) {
                    c_J[i38 + c_J.size(0) * i37] =
                        (r3D[i38 + r3D.size(0) * i37] * r1[i37] +
                         3.0 * r2D[i38 + r2D.size(0) * i37] * ud[i37] * udd_vec[i37]) +
                        c_r1D[i38 + c_r1D.size(0) * i37] * jps;
                }
            }
        } else {
        ...
        if (curv2->Info.TRAFO) {
            ...
            for (int i40{0}; i40 < y_loop_ub; i40++) {
                int bb_loop_ub;
                bb_loop_ub = static_cast<int>(uv[0]);
                for (int i43{0}; i43 < bb_loop_ub; i43++) {
                    r1D_a[i43 + r1D_a.size(0) * i40] = 0.0;
                }
            }
            // 'Kinematics:131' for j = 1 : N
            i42 = b_r0D.size(1);
            ...
            for (int i39{0}; i39 < x_loop_ub; i39++) {
                ...
                for (int i41{0}; i41 < ab_loop_ub; i41++) {
                    r1D_a[i41 + r1D_a.size(0) * i39] = c_r1D[i41 + c_r1D.size(0) * i39];
                }
            }
        }
        ...
        for (int i44{0}; i44 < b_trueCount; i44++) {
            vmax_data[i44] = ctx_cfg_vmax[b_tmp_data[i44] - 1] * 0.5;
        }
        ...
        for (int i53{0}; i53 < d_trueCount; i53++) {
            jmax_data[i53] = ctx_cfg_jmax[d_tmp_data[i53] - 1] * 0.5;
        }
        ...
        if (0 <= r1D_r.size(1) - 1) {
            ...
            ib_loop_ub = static_cast<signed char>(ctx_cfg_indCart.size(0));
        }
        ...
        if (y.size(1) == ud.size(1)) {
            ...
            for (i56 = 0; i56 <= h_vectorUB; i56 += 2) {
                __m128d r11;
                __m128d r12;
                r11 = _mm_loadu_pd(&y[i56]);
                r12 = _mm_loadu_pd(&ud[i56]);
                _mm_storeu_pd(&feed[i56], _mm_mul_pd(r11, r12));
            }
            for (i56 = h_scalarLB; i56 < jb_loop_ub; i56++) {
                feed[i56] = y[i56] * ud[i56];
            }
        } else {
            b_times(feed, y, ud);
        }
        ...
        if (r1D_a.size(1) == ud.size(1)) {
            ...
            kb_loop_ub = r1D_a.size(1);
            for (int i57{0}; i57 < kb_loop_ub; i57++) {
                int i59;
                int i_scalarLB;
                int i_vectorUB;
                int mb_loop_ub;
                mb_loop_ub = r1D_a.size(0);
                i_scalarLB = (r1D_a.size(0) / 2) << 1;
                i_vectorUB = i_scalarLB - 2;
                for (i59 = 0; i59 <= i_vectorUB; i59 += 2) {
                    __m128d r13;
                    r13 = _mm_loadu_pd(&r1D_a[i59 + r1D_a.size(0) * i57]);
                    _mm_storeu_pd(&V[i59 + V.size(0) * i57], _mm_mul_pd(r13, _mm_set1_pd(ud[i57])));
                }
                for (i59 = i_scalarLB; i59 < mb_loop_ub; i59++) {
                    V[i59 + V.size(0) * i57] = r1D_a[i59 + r1D_a.size(0) * i57] * ud[i57];
                }
            }
        } else {
            times(V, r1D_a, ud);
        ...
        if (b_y) {
            ...
            for (i62 = 0; i62 <= j_vectorUB; i62 += 2) {
                __m128d r15;
                r15 = _mm_loadu_pd(&feed[i62]);
                _mm_storeu_pd(&b_feed[i62], _mm_sub_pd(r15, _mm_set1_pd(b_fmax)));
            }
            for (i62 = j_scalarLB; i62 < nb_loop_ub; i62++) {
                b_feed[i62] = feed[i62] - b_fmax;
            }
            ...
            if ((V.size(0) != 0) && (V.size(1) != 0)) {
                int i60;
                i60 = V.size(1);
                for (int f_k{0}; f_k < i60; f_k++) {
                    int i61;
                    i61 = r14.size(0);
                    for (int g_k{0}; g_k < i61; g_k++) {
                        r14[g_k + r14.size(0) * f_k] = std::abs(V[g_k + V.size(0) * f_k]);
                    }
                }
            }
            if (r14.size(0) == b_trueCount) {
                ...
                for (int i63{0}; i63 < ob_loop_ub; i63++) {
                    ...
                    for (int i64{0}; i64 < pb_loop_ub; i64++) {
                        b_x[i64 + b_x.size(0) * i63] =
                            (r14[i64 + r14.size(0) * i63] > vmax_data[i64]);
                    }
                }
            } else {
            ...
            if (c_y) {
                ...
                if (r14.size(0) == b_trueCount) {
                    ...
                    for (int i67{0}; i67 < qb_loop_ub; i67++) {
                        ...
                        for (i71 = 0; i71 <= k_vectorUB; i71 += 2) {
                            __m128d r17;
                            __m128d r18;
                            r17 = _mm_loadu_pd(&r14[i71 + r14.size(0) * i67]);
                            r18 = _mm_loadu_pd(&vmax_data[i71]);
                            _mm_storeu_pd(&r16[i71 + r16.size(0) * i67], _mm_sub_pd(r17, r18));
                        }
                        for (i71 = k_scalarLB; i71 < sb_loop_ub; i71++) {
                            r16[i71 + r16.size(0) * i67] =
                                r14[i71 + r14.size(0) * i67] - vmax_data[i71];
                        }
                    }
                    r14.set_size(r16.size(0), r16.size(1));
                    ub_loop_ub = r16.size(1);
                    for (int i70{0}; i70 < ub_loop_ub; i70++) {
                        int vb_loop_ub;
                        vb_loop_ub = r16.size(0);
                        for (int i72{0}; i72 < vb_loop_ub; i72++) {
                            r14[i72 + r14.size(0) * i70] = r16[i72 + r16.size(0) * i70];
                        }
                    }
                ...
                if ((A.size(0) != 0) && (A.size(1) != 0)) {
                    int i65;
                    i65 = A.size(1);
                    for (int i_k{0}; i_k < i65; i_k++) {
                        int i66;
                        i66 = r14.size(0);
                        for (int j_k{0}; j_k < i66; j_k++) {
                            r14[j_k + r14.size(0) * i_k] = std::abs(A[j_k + A.size(0) * i_k]);
                        }
                    }
                }
                ...
                if (d_y) {
                    ...
                    if (r14.size(0) == c_trueCount) {
                        ...
                        wb_loop_ub = r14.size(1);
                        for (int i75{0}; i75 < wb_loop_ub; i75++) {
                            int i79;
                            int l_scalarLB;
                            int l_vectorUB;
                            int yb_loop_ub;
                            yb_loop_ub = r14.size(0);
                            l_scalarLB = (r14.size(0) / 2) << 1;
                            l_vectorUB = l_scalarLB - 2;
                            for (i79 = 0; i79 <= l_vectorUB; i79 += 2) {
                                __m128d r20;
                                __m128d r21;
                                r20 = _mm_loadu_pd(&r14[i79 + r14.size(0) * i75]);
                                r21 = _mm_loadu_pd(&amax_data[i79]);
                                _mm_storeu_pd(&r19[i79 + r19.size(0) * i75], _mm_sub_pd(r20, r21));
                            }
                            for (i79 = l_scalarLB; i79 < yb_loop_ub; i79++) {
                                r19[i79 + r19.size(0) * i75] =
                                    r14[i79 + r14.size(0) * i75] - amax_data[i79];
                            }
                        }
                        r14.set_size(r19.size(0), r19.size(1));
                        bc_loop_ub = r19.size(1);
                        for (int i78{0}; i78 < bc_loop_ub; i78++) {
                            int cc_loop_ub;
                            cc_loop_ub = r19.size(0);
                            for (int i80{0}; i80 < cc_loop_ub; i80++) {
                                r14[i80 + r14.size(0) * i78] = r19[i80 + r19.size(0) * i78];
                            }
                        }
                    ...
                    if ((c_J.size(0) != 0) && (c_J.size(1) != 0)) {
                        int i73;
                        i73 = c_J.size(1);
                        for (int l_k{0}; l_k < i73; l_k++) {
                            int i74;
                            i74 = r14.size(0);
                            for (int m_k{0}; m_k < i74; m_k++) {
                                r14[m_k + r14.size(0) * l_k] =
                    ...
                    if (r14.size(0) == d_trueCount) {
                        int xb_loop_ub;
                        b_x.set_size(r14.size(0), r14.size(1));
                        xb_loop_ub = r14.size(1);
                        for (int i76{0}; i76 < xb_loop_ub; i76++) {
                            int ac_loop_ub;
                            ac_loop_ub = r14.size(0);
                            for (int i77{0}; i77 < ac_loop_ub; i77++) {
                                b_x[i77 + b_x.size(0) * i76] =
                                    (r14[i77 + r14.size(0) * i76] > jmax_data[i77]);
                            }
                        }
                    } else {
                    ...
                    if (e_y) {
                        ...
                        if (r14.size(0) == d_trueCount) {
                            ...
                            dc_loop_ub = r14.size(1);
                            for (int i81{0}; i81 < dc_loop_ub; i81++) {
                                int ec_loop_ub;
                                int i83;
                                int m_scalarLB;
                                int m_vectorUB;
                                ec_loop_ub = r14.size(0);
                                m_scalarLB = (r14.size(0) / 2) << 1;
                                m_vectorUB = m_scalarLB - 2;
                                for (i83 = 0; i83 <= m_vectorUB; i83 += 2) {
                                    __m128d r23;
                                    __m128d r24;
                                    r23 = _mm_loadu_pd(&r14[i83 + r14.size(0) * i81]);
                                    r24 = _mm_loadu_pd(&jmax_data[i83]);
                                    _mm_storeu_pd(&r22[i83 + r22.size(0) * i81],
                                                  _mm_sub_pd(r23, r24));
                                }
                                for (i83 = m_scalarLB; i83 < ec_loop_ub; i83++) {
                                    r22[i83 + r22.size(0) * i81] =
                                        r14[i83 + r14.size(0) * i81] - jmax_data[i83];
                                }
                            }
                            r14.set_size(r22.size(0), r22.size(1));
                            fc_loop_ub = r22.size(1);
                            for (int i82{0}; i82 < fc_loop_ub; i82++) {
                                int gc_loop_ub;
                                gc_loop_ub = r22.size(0);
                                for (int i84{0}; i84 < gc_loop_ub; i84++) {
                                    r14[i84 + r14.size(0) * i82] = r22[i84 + r22.size(0) * i82];
                                }
                            }
