        ...
        *curv1 = *curv;
        ...
        for (int j{0}; j < 3; j++) {
            ...
            for (int i6{0}; i6 < 2; i6++) {
                for (int i8{0}; i8 <= vectorUB; i8 += 2) {
                    __m128d r4;
                    r4 = _mm_loadu_pd(&rd_data[(i8 + trueCount * i6) + trueCount * 2 * j]);
                    _mm_storeu_pd(&b_rd_data[i8 + trueCount * i6],
                                  _mm_div_pd(r4, _mm_set1_pd(curv_vec[j].a_param)));
                }
                for (int i8{scalarLB}; i8 <= rd; i8++) {
            ...
            for (int i10{0}; i10 < 2; i10++) {
                for (int i12{0}; i12 < trueCount; i12++) {
                    rd_data[(i12 + trueCount * i10) + trueCount * 2 * j] =
                        b_rd_data[i12 + trueCount * i10];
                }
                for (int i14{0}; i14 <= b_vectorUB; i14 += 2) {
                    __m128d r5;
                    r5 = _mm_loadu_pd(&rdd_data[(i14 + trueCount * i10) + trueCount * 2 * j]);
                    _mm_storeu_pd(&b_rdd_data[i14 + trueCount * i10],
                                  _mm_div_pd(r5, _mm_set1_pd(c)));
                }
                for (int i14{b_scalarLB}; i14 <= rdd; i14++) {
                    b_rdd_data[i14 + trueCount * i10] =
                        rdd_data[(i14 + trueCount * i10) + trueCount * 2 * j] / c;
                }
            }
            for (int i11{0}; i11 < 2; i11++) {
                for (int i13{0}; i13 < trueCount; i13++) {
                    rdd_data[(i13 + trueCount * i11) + trueCount * 2 * j] =
                        b_rdd_data[i13 + trueCount * i11];
                }
            }
        }
        ...
        for (int b_j{0}; b_j < 3; b_j++) {
            ...
            for (int i15{0}; i15 <= c_vectorUB; i15 += 2) {
                ...
                __m128d r7;
                r6 = _mm_loadu_pd(
                    &r_data[(i15 + trueCount * (ind1_tmp - 1)) + trueCount * 2 * (b_ind1_tmp - 1)]);
                r7 = _mm_loadu_pd(
                    &r_data[(i15 + trueCount * (ind2_tmp - 1)) + trueCount * 2 * (b_ind2_tmp - 1)]);
                _mm_storeu_pd(&c_x[i15], _mm_sub_pd(r6, r7));
            }
            for (int i15{c_scalarLB}; i15 < trueCount; i15++) {
                c_x[i15] =
                    r_data[(i15 + trueCount * (ind1_tmp - 1)) + trueCount * 2 * (b_ind1_tmp - 1)] -
                    r_data[(i15 + trueCount * (ind2_tmp - 1)) + trueCount * 2 * (b_ind2_tmp - 1)];
            }
            ...
            for (int i17{0}; i17 <= d_vectorUB; i17 += 2) {
                __m128d r8;
                __m128d r9;
                r8 = _mm_loadu_pd(&rd_data[(i17 + trueCount * (ind1_tmp - 1)) +
                                           trueCount * 2 * (b_ind1_tmp - 1)]);
                r9 = _mm_loadu_pd(&rd_data[(i17 + trueCount * (ind2_tmp - 1)) +
                                           trueCount * 2 * (b_ind2_tmp - 1)]);
                _mm_storeu_pd(&c_x[i17], _mm_sub_pd(r8, r9));
            }
            for (int i17{d_scalarLB}; i17 < trueCount; i17++) {
                c_x[i17] =
                    rd_data[(i17 + trueCount * (ind1_tmp - 1)) + trueCount * 2 * (b_ind1_tmp - 1)] -
                    rd_data[(i17 + trueCount * (ind2_tmp - 1)) + trueCount * 2 * (b_ind2_tmp - 1)];
            }
            ...
            c_x.set_size(trueCount);
            for (int i19{0}; i19 <= e_vectorUB; i19 += 2) {
                __m128d r10;
                __m128d r11;
                r10 = _mm_loadu_pd(&rdd_data[(i19 + trueCount * (ind1_tmp - 1)) +
                                             trueCount * 2 * (b_ind1_tmp - 1)]);
                r11 = _mm_loadu_pd(&rdd_data[(i19 + trueCount * (ind2_tmp - 1)) +
                                             trueCount * 2 * (b_ind2_tmp - 1)]);
                _mm_storeu_pd(&c_x[i19], _mm_sub_pd(r10, r11));
            }
            for (int i19{e_scalarLB}; i19 < trueCount; i19++) {
                c_x[i19] =
                    rdd_data[(i19 + trueCount * (ind1_tmp - 1)) +
                             trueCount * 2 * (b_ind1_tmp - 1)] -
                    rdd_data[(i19 + trueCount * (ind2_tmp - 1)) + trueCount * 2 * (b_ind2_tmp - 1)];
            }
