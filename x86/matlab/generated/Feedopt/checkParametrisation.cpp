...
bool checkParametrisation(const queue_coder *queue)
{
    ...
    N = queue->size();
    // 'checkParametrisation:9' curv = queue.get( 1 );
    queue->get(&expl_temp);
    curv_Info_Type = expl_temp.Info.Type;
    curv_a_param = expl_temp.a_param;
    curv_b_param = expl_temp.b_param;
    // 'checkParametrisation:11' if( ~valid_curv_param( curv ) )
    // 'checkParametrisation:38' valide = false;
    valide = false;
    // 'checkParametrisation:39' if( curv.a_param <= 0 )
    if ((expl_temp.a_param > 0.0) && (expl_temp.b_param >= 0.0) &&
        ((expl_temp.a_param + expl_temp.b_param) - 1.0 <= 2.2204460492503131E-16)) {
        // 'checkParametrisation:40' if( curv.b_param < 0 )
        // 'checkParametrisation:41' if( curv.a_param + curv.b_param -1 > eps )
        // 'checkParametrisation:42' valide = true;
        valide = true;
    }
    if (valide) {
        unsigned int k;
        // 'checkParametrisation:13' for k = 2 : N
        k = 2U;
        int exitg1;
        do {
            exitg1 = 0;
            if (k <= N) {
                bool b_valide;
                // 'checkParametrisation:14' curvNext = queue.get( k );
                queue->get(k, &expl_temp);
                // 'checkParametrisation:16' if( ~valid_curv_param( curvNext ) )
                // 'checkParametrisation:38' valide = false;
                b_valide = false;
                // 'checkParametrisation:39' if( curv.a_param <= 0 )
                if ((expl_temp.a_param > 0.0) && (expl_temp.b_param >= 0.0) &&
                ...
                if (!b_valide) {
                    ...
                    if ((expl_temp.Info.Type == CurveType_TransP5) ||
                        ...
                        double d;
                        // 'checkParametrisation:20' else
                        // 'checkParametrisation:21' if( abs( ( curv.b_param + curv.a_param ) - 1 )
                        // > eps )
                        d = curv_b_param + curv_a_param;
                        if ((std::abs(d - 1.0) > 2.2204460492503131E-16) &&
                            (std::abs(expl_temp.b_param - d) > 2.2204460492503131E-16)) {
                            // 'checkParametrisation:22' if( abs( curvNext.b_param - ( curv.b_param
                            // + curv.a_param ) ) > eps )
                            exitg1 = 1;
                        } else {
                            guard1 = true;
                    ...
                    if (guard1) {
                        // 'checkParametrisation:28' curv = curvNext;
                        curv_Info_Type = expl_temp.Info.Type;
                        curv_a_param = expl_temp.a_param;
                        curv_b_param = expl_temp.b_param;
                        k++;
                ...
                // 'checkParametrisation:31' if( curv.b_param + curv.a_param < 1 )
                if (curv_b_param + curv_a_param >= 1.0) {
                    // 'checkParametrisation:33' valid = true;
                    valid = true;
                }
                exitg1 = 1;
            }
