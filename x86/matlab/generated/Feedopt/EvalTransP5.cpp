        ...
        if ((b.size(0) == D0.size(0)) && (b.size(1) == D0.size(1)) && (i14 == r5.size(0)) &&
            ...
            for (int i20{0}; i20 < k_loop_ub; i20++) {
                ...
                for (int i22{0}; i22 <= d_vectorUB; i22 += 2) {
                    ...
                    __m128d r7;
                    __m128d r8;
                    r6 = _mm_loadu_pd(&b[i22 + b.size(0) * i20]);
                    r7 = _mm_loadu_pd(&D0[i22 + D0.size(0) * i20]);
                    r8 = _mm_loadu_pd(&r5[i22 + r5.size(0) * i20]);
                    _mm_storeu_pd(&D0[i22 + D0.size(0) * i20], _mm_add_pd(_mm_mul_pd(r6, r7), r8));
                }
                for (int i22{d_scalarLB}; i22 < l_loop_ub; i22++) {
                    D0[i22 + D0.size(0) * i20] =
                        b[i22 + b.size(0) * i20] * D0[i22 + D0.size(0) * i20] +
                        r5[i22 + r5.size(0) * i20];
                }
            }
        } else {
        ...
        if ((b.size(0) == D1.size(0)) && (b.size(1) == D1.size(1)) && (i24 == r5.size(0)) &&
            ...
            for (int i30{0}; i30 < o_loop_ub; i30++) {
                ...
                for (int i32{0}; i32 <= e_vectorUB; i32 += 2) {
                    ...
                    __m128d r11;
                    __m128d r9;
                    r9 = _mm_loadu_pd(&b[i32 + b.size(0) * i30]);
                    r10 = _mm_loadu_pd(&D1[i32 + D1.size(0) * i30]);
                    r11 = _mm_loadu_pd(&r5[i32 + r5.size(0) * i30]);
                    _mm_storeu_pd(&D1[i32 + D1.size(0) * i30],
                                  _mm_add_pd(_mm_mul_pd(r9, r10), r11));
                }
                for (int i32{e_scalarLB}; i32 < p_loop_ub; i32++) {
                    D1[i32 + D1.size(0) * i30] =
                        b[i32 + b.size(0) * i30] * D1[i32 + D1.size(0) * i30] +
                        r5[i32 + r5.size(0) * i30];
                }
            }
        } else {
    ...
    for (int d_i{0}; d_i <= i29 - 2; d_i++) {
        ...
        if ((b.size(0) == D2.size(0)) && (b.size(1) == D2.size(1)) && (i34 == r5.size(0)) &&
            ...
            for (int i40{0}; i40 < s_loop_ub; i40++) {
                ...
                for (int i42{0}; i42 <= f_vectorUB; i42 += 2) {
                    __m128d r12;
                    __m128d r13;
                    __m128d r14;
                    r12 = _mm_loadu_pd(&b[i42 + b.size(0) * i40]);
                    r13 = _mm_loadu_pd(&D2[i42 + D2.size(0) * i40]);
                    r14 = _mm_loadu_pd(&r5[i42 + r5.size(0) * i40]);
                    _mm_storeu_pd(&D2[i42 + D2.size(0) * i40],
                                  _mm_add_pd(_mm_mul_pd(r12, r13), r14));
                }
                for (int i42{f_scalarLB}; i42 < t_loop_ub; i42++) {
                    D2[i42 + D2.size(0) * i40] =
                        b[i42 + b.size(0) * i40] * D2[i42 + D2.size(0) * i40] +
                        r5[i42 + r5.size(0) * i40];
                }
            }
        } else {
