        ...
        if ((d_b.size(0) == D0.size(0)) && (d_b.size(1) == D0.size(1)) && (i12 == e_b.size(0)) &&
            ...
            for (int i15{0}; i15 < d_loop_ub; i15++) {
                ...
                for (i16 = 0; i16 <= vectorUB; i16 += 2) {
                    ...
                    r1 = _mm_loadu_pd(&D0[i16 + D0.size(0) * i15]);
                    r2 = _mm_loadu_pd(&e_b[i16 + e_b.size(0) * i15]);
                    _mm_storeu_pd(&D0[i16 + D0.size(0) * i15], _mm_add_pd(_mm_mul_pd(r, r1), r2));
                }
        ...
        if ((d_b.size(0) == D1.size(0)) && (d_b.size(1) == D1.size(1)) && (i19 == e_b.size(0)) &&
            ...
            for (int i22{0}; i22 < f_loop_ub; i22++) {
                ...
                for (i23 = 0; i23 <= b_vectorUB; i23 += 2) {
                    ...
                    r4 = _mm_loadu_pd(&D1[i23 + D1.size(0) * i22]);
                    r5 = _mm_loadu_pd(&e_b[i23 + e_b.size(0) * i22]);
                    _mm_storeu_pd(&D1[i23 + D1.size(0) * i22], _mm_add_pd(_mm_mul_pd(r3, r4), r5));
                }
        ...
        if ((d_b.size(0) == D2.size(0)) && (d_b.size(1) == D2.size(1)) && (i26 == e_b.size(0)) &&
            ...
            for (int i29{0}; i29 < h_loop_ub; i29++) {
                ...
                for (i30 = 0; i30 <= c_vectorUB; i30 += 2) {
                    ...
                    r7 = _mm_loadu_pd(&D2[i30 + D2.size(0) * i29]);
                    r8 = _mm_loadu_pd(&e_b[i30 + e_b.size(0) * i29]);
                    _mm_storeu_pd(&D2[i30 + D2.size(0) * i29], _mm_add_pd(_mm_mul_pd(r6, r7), r8));
                }
                for (i30 = c_scalarLB; i30 < j_loop_ub; i30++) {
    ...
    for (int e_i{0}; e_i < 2; e_i++) {
        ...
        if ((d_b.size(0) == D3.size(0)) && (d_b.size(1) == D3.size(1)) && (i35 == e_b.size(0)) &&
            ...
            for (int i39{0}; i39 < n_loop_ub; i39++) {
                ...
                for (i40 = 0; i40 <= d_vectorUB; i40 += 2) {
                    __m128d r10;
                    __m128d r11;
                    __m128d r9;
                    r9 = _mm_loadu_pd(&d_b[i40 + d_b.size(0) * i39]);
                    r10 = _mm_loadu_pd(&D3[i40 + D3.size(0) * i39]);
                    r11 = _mm_loadu_pd(&e_b[i40 + e_b.size(0) * i39]);
                    _mm_storeu_pd(&D3[i40 + D3.size(0) * i39],
                                  _mm_add_pd(_mm_mul_pd(r9, r10), r11));
                }
                for (i40 = d_scalarLB; i40 < p_loop_ub; i40++) {
                 ...
                 ::coder::array<double, 1U> &r_2D, ::coder::array<double, 1U> &r_3D)
{
    ...
    for (int b_i{0}; b_i < 5; b_i++) {
        ...
        if ((CurvStruct_CoeffP5.size(0) == D0.size(0)) && (i12 == r.size(0))) {
            ...
            for (i13 = scalarLB; i13 < e_loop_ub; i13++) {
    ...
    for (int e_i{0}; e_i < 2; e_i++) {
        ...
        if ((p5_3D.size(0) == D3.size(0)) && (i27 == r.size(0))) {
            ...
            int d_vectorUB;
            int i28;
            d_scalarLB = (D3.size(0) / 2) << 1;
            d_vectorUB = d_scalarLB - 2;
            for (i28 = 0; i28 <= d_vectorUB; i28 += 2) {
                __m128d r7;
                __m128d r8;
                r7 = _mm_loadu_pd(&D3[i28]);
                r8 = _mm_loadu_pd(&r[i28]);
                _mm_storeu_pd(&D3[i28], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(u_vec), r7), r8));
            }
            for (i28 = d_scalarLB; i28 < k_loop_ub; i28++) {
