    ...
    for (int b_i{0}; b_i < 5; b_i++) {
        ...
        if ((d_b.size(0) == r_0D.size(0)) && (d_b.size(1) == r_0D.size(1)) &&
            ...
            for (int i15{0}; i15 < d_loop_ub; i15++) {
                ...
                vectorUB = scalarLB - 2;
                for (i16 = 0; i16 <= vectorUB; i16 += 2) {
                    __m128d r;
                    __m128d r1;
                    __m128d r2;
                    r = _mm_loadu_pd(&d_b[i16 + d_b.size(0) * i15]);
                    r1 = _mm_loadu_pd(&r_0D[i16 + r_0D.size(0) * i15]);
                    r2 = _mm_loadu_pd(&e_b[i16 + e_b.size(0) * i15]);
                    _mm_storeu_pd(&r_0D[i16 + r_0D.size(0) * i15],
                                  _mm_add_pd(_mm_mul_pd(r, r1), r2));
                }
                for (i16 = scalarLB; i16 < e_loop_ub; i16++) {
    ...
    for (int c_i{0}; c_i < 4; c_i++) {
        ...
        if ((d_b.size(0) == r_1D.size(0)) && (d_b.size(1) == r_1D.size(1)) &&
            ...
            for (int i22{0}; i22 < f_loop_ub; i22++) {
                ...
                for (i23 = 0; i23 <= b_vectorUB; i23 += 2) {
                    __m128d r3;
                    __m128d r4;
                    __m128d r5;
                    r3 = _mm_loadu_pd(&d_b[i23 + d_b.size(0) * i22]);
                    r4 = _mm_loadu_pd(&r_1D[i23 + r_1D.size(0) * i22]);
                    r5 = _mm_loadu_pd(&e_b[i23 + e_b.size(0) * i22]);
                    _mm_storeu_pd(&r_1D[i23 + r_1D.size(0) * i22],
                                  _mm_add_pd(_mm_mul_pd(r3, r4), r5));
                }
                for (i23 = b_scalarLB; i23 < g_loop_ub; i23++) {
        ...
        if ((d_b.size(0) == r_2D.size(0)) && (d_b.size(1) == r_2D.size(1)) &&
            ...
            for (int i29{0}; i29 < h_loop_ub; i29++) {
                ...
                for (i30 = 0; i30 <= c_vectorUB; i30 += 2) {
                    ...
                    __m128d r7;
                    __m128d r8;
                    r6 = _mm_loadu_pd(&d_b[i30 + d_b.size(0) * i29]);
                    r7 = _mm_loadu_pd(&r_2D[i30 + r_2D.size(0) * i29]);
                    r8 = _mm_loadu_pd(&e_b[i30 + e_b.size(0) * i29]);
                    _mm_storeu_pd(&r_2D[i30 + r_2D.size(0) * i29],
                                  _mm_add_pd(_mm_mul_pd(r6, r7), r8));
                }
                for (i30 = c_scalarLB; i30 < i_loop_ub; i30++) {
        ...
        if ((d_b.size(0) == r_3D.size(0)) && (d_b.size(1) == r_3D.size(1)) &&
            ...
            for (int i35{0}; i35 < j_loop_ub; i35++) {
                ...
                for (i36 = 0; i36 <= d_vectorUB; i36 += 2) {
                    ...
                    __m128d r11;
                    __m128d r9;
                    r9 = _mm_loadu_pd(&d_b[i36 + d_b.size(0) * i35]);
                    r10 = _mm_loadu_pd(&r_3D[i36 + r_3D.size(0) * i35]);
                    r11 = _mm_loadu_pd(&e_b[i36 + e_b.size(0) * i35]);
                    _mm_storeu_pd(&r_3D[i36 + r_3D.size(0) * i35],
                                  _mm_add_pd(_mm_mul_pd(r9, r10), r11));
                }
                for (i36 = d_scalarLB; i36 < k_loop_ub; i36++) {
                 ...
                 ::coder::array<double, 1U> &r_2D, ::coder::array<double, 1U> &r_3D)
{
    ...
    for (int b_i{0}; b_i < 5; b_i++) {
        ...
        if ((CurvStruct_CoeffP5.size(0) == r_0D.size(0)) && (i12 == r.size(0))) {
            ...
            int scalarLB;
            int vectorUB;
            scalarLB = (r_0D.size(0) / 2) << 1;
            vectorUB = scalarLB - 2;
            for (i14 = 0; i14 <= vectorUB; i14 += 2) {
                __m128d r1;
                __m128d r2;
                r1 = _mm_loadu_pd(&r_0D[i14]);
                r2 = _mm_loadu_pd(&r[i14]);
                _mm_storeu_pd(&r_0D[i14], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(u_vec), r1), r2));
            }
            for (i14 = scalarLB; i14 < e_loop_ub; i14++) {
                r_0D[i14] = u_vec * r_0D[i14] + r[i14];
            }
        } else {
    ...
    for (int c_i{0}; c_i < 4; c_i++) {
        ...
        if ((p5_1D.size(0) == r_1D.size(0)) && (i17 == r.size(0))) {
            ...
            int b_vectorUB;
            int i18;
            b_scalarLB = (r_1D.size(0) / 2) << 1;
            b_vectorUB = b_scalarLB - 2;
            for (i18 = 0; i18 <= b_vectorUB; i18 += 2) {
                __m128d r3;
                __m128d r4;
                r3 = _mm_loadu_pd(&r_1D[i18]);
                r4 = _mm_loadu_pd(&r[i18]);
                _mm_storeu_pd(&r_1D[i18], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(u_vec), r3), r4));
            }
            for (i18 = b_scalarLB; i18 < g_loop_ub; i18++) {
    ...
    for (int d_i{0}; d_i < 3; d_i++) {
        ...
        if ((p5_2D.size(0) == r_2D.size(0)) && (i22 == r.size(0))) {
            ...
            int c_vectorUB;
            int i23;
            c_scalarLB = (r_2D.size(0) / 2) << 1;
            c_vectorUB = c_scalarLB - 2;
            for (i23 = 0; i23 <= c_vectorUB; i23 += 2) {
                __m128d r5;
                __m128d r6;
                r5 = _mm_loadu_pd(&r_2D[i23]);
                r6 = _mm_loadu_pd(&r[i23]);
                _mm_storeu_pd(&r_2D[i23], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(u_vec), r5), r6));
            }
            for (i23 = c_scalarLB; i23 < i_loop_ub; i23++) {
    ...
    for (int e_i{0}; e_i < 2; e_i++) {
        ...
        if ((p5_3D.size(0) == r_3D.size(0)) && (i26 == r.size(0))) {
            ...
            int d_vectorUB;
            int i27;
            d_scalarLB = (r_3D.size(0) / 2) << 1;
            d_vectorUB = d_scalarLB - 2;
            for (i27 = 0; i27 <= d_vectorUB; i27 += 2) {
                __m128d r7;
                __m128d r8;
                r7 = _mm_loadu_pd(&r_3D[i27]);
                r8 = _mm_loadu_pd(&r[i27]);
                _mm_storeu_pd(&r_3D[i27], _mm_add_pd(_mm_mul_pd(_mm_set1_pd(u_vec), r7), r8));
            }
            for (i27 = d_scalarLB; i27 < j_loop_ub; i27++) {
