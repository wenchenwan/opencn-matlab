                ...
                if (batch_size > 1.0) {
                    ...
                    if (i12 != 0) {
                        ...
                        for (int c_k{0}; c_k <= i15; c_k++) {
                            ...
                            double d;
                            r5 = _mm_loadu_pd(&b[0]);
                            d = spline.sp.knots[(i9 + acoef * c_k) + 1];
                            r6 = _mm_set1_pd(d);
                    ...
                    if (i16 != 0) {
                        ...
                        for (int d_k{0}; d_k <= i17; d_k++) {
                            ...
                            double d1;
                            r7 = _mm_loadu_pd(&b[0]);
                            d1 = spline.sp.knots[((i9 + i13) + b_acoef * d_k) + 2];
                            r9 = _mm_set1_pd(d1);
                    ...
                    if (b_c.size(1) == c_c.size(1)) {
                        ...
                        for (int i18{0}; i18 < d_loop_ub; i18++) {
                            ...
                            __m128d r11;
                            __m128d r8;
                            r8 = _mm_loadu_pd(&b_c[5 * i18]);
                            r10 = _mm_loadu_pd(&c_c[5 * i18]);
                            r11 = _mm_set1_pd(2.0);
                            _mm_storeu_pd(&Umat[5 * i18], _mm_div_pd(_mm_add_pd(r8, r10), r11));
                            r8 = _mm_loadu_pd(&b_c[5 * i18 + 2]);
                            r10 = _mm_loadu_pd(&c_c[5 * i18 + 2]);
                            _mm_storeu_pd(&Umat[5 * i18 + 2], _mm_div_pd(_mm_add_pd(r8, r10), r11));
                            Umat[5 * i18 + 4] = (b_c[5 * i18 + 4] + c_c[5 * i18 + 4]) / 2.0;
                        }
                    } else {
                    ...
                    for (int j{0}; j < i21; j++) {
                        ...
                        for (int f_k{0}; f_k < i30; f_k++) {
                            ...
                            // coeffs, u(k));
                            r12[f_k] = Umat[f_k];
                            b_spline.set_size(1, i32);
                            for (int i36{0}; i36 < m_loop_ub; i36++) {
                                b_spline[i36] = spline.sp.coeff[j + spline.sp.coeff.size(0) * i36];
                            }
                            bspline_eval(spline.sp.Bl.handle, b_spline, &r12[f_k], &d2, &d3, &d4);
                            r17[f_k] = d4;
                            r15[f_k] = d3;
                            r14[f_k] = d2;
                        ...
                        n_loop_ub = r14.size(1);
                        for (int i35{0}; i35 < n_loop_ub; i35++) {
                            r1D[j + r1D.size(0) * i35] = r14[i35];
                        }
                    }
                    ...
                    for (int i23{0}; i23 < i_loop_ub; i23++) {
                        ...
                        for (int i26{0}; i26 < j_loop_ub; i26++) {
                            double c_varargin_1;
                            c_varargin_1 = r1D[i26 + r1D.size(0) * i23];
                            r13[i26 + r13.size(0) * i23] = std::pow(c_varargin_1, 2.0);
                        }
                    }
                    coder::sum(r13, x);
                    i25 = x.size(1);
                    ...
                    if ((i16 == i12) && (y.size(1) == i34)) {
                        ...
                        for (i37 = 0; i37 <= b_vectorUB; i37 += 2) {
                            __m128d r18;
                            __m128d r19;
                            __m128d r20;
                            r18 = _mm_loadu_pd(&spline.sp.knots[((i9 + i13) + i37) + 2]);
                            r19 = _mm_loadu_pd(&spline.sp.knots[(i9 + i37) + 1]);
                            r20 = _mm_loadu_pd(&y[i37]);
                            _mm_storeu_pd(&spline.sp.Lk[i37],
                                          _mm_div_pd(_mm_mul_pd(r20, _mm_sub_pd(r18, r19)),
                                                     _mm_set1_pd(2.0)));
                        }
                        for (i37 = b_scalarLB; i37 < o_loop_ub; i37++) {
                    ...
                    for (int i38{0}; i38 < 6; i38++) {
                        dv[i38] = (*(double(*)[6]) & batch_pvec[0])[i38];
                    }
                    for (int i39{0}; i39 < 6; i39++) {
                        dv1[i39] = (*(double(*)[6]) & batch_pvec[6 * (c_batch_pvec - 1)])[i39];
                    }
                    constrSplineStruct(
