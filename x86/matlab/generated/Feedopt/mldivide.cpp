            ...
            if (tau[b_j] != 0.0) {
                for (int c_k{0}; c_k < b_nb; c_k++) {
                    ...
                    if (wj != 0.0) {
                        ...
                        scalarLB = (((((m - b_j) - 1) / 2) << 1) + b_j) + 2;
                        vectorUB = scalarLB - 2;
                        for (g_i = i13; g_i <= vectorUB; g_i += 2) {
                            __m128d r;
                            __m128d r1;
                            r = _mm_loadu_pd(&b_A[(g_i + b_A.size(0) * b_j) - 1]);
                            r1 = _mm_loadu_pd(&b_B[(g_i + b_B.size(0) * c_k) - 1]);
                            _mm_storeu_pd(&b_B[(g_i + b_B.size(0) * c_k) - 1],
                                          _mm_sub_pd(r1, _mm_mul_pd(r, _mm_set1_pd(wj))));
                        }
                        for (g_i = scalarLB; g_i <= m; g_i++) {
