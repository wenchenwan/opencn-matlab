                        ...
                        *opt_struct = window[0];
                        ...
                        if ((r2D.size(0) == r1D.size(0)) && (i9 == r1D.size(0))) {
                            ...
                            for (i11 = 0; i11 <= vectorUB; i11 += 2) {
                                __m128d r1;
                                __m128d r2;
                                r1 = _mm_loadu_pd(&r2D[i11]);
                                r2 = _mm_loadu_pd(&r1D[i11]);
                                _mm_storeu_pd(
                                    &r2D[i11],
                                    _mm_mul_pd(_mm_add_pd(_mm_mul_pd(r1, _mm_set1_pd(b_y)),
                                                          _mm_mul_pd(r2, _mm_set1_pd(udd))),
                                               _mm_div_pd(r2, _mm_set1_pd(n))));
                            }
                            for (i11 = scalarLB; i11 < f_loop_ub; i11++) {
                        ...
                        if ((window[static_cast<int>(ind) - 1].Info.zspdmode == ZSpdMode_NZ) ||
                            ...
                            for (int i12{0}; i12 < g_loop_ub; i12++) {
                                ...
                                r[i12] = std::pow(d_varargin_1, 2.0);
                            }
                            ...
                            if ((r2D.size(0) == r1D.size(0)) && (i13 == r1D.size(0))) {
                                ...
                                for (i14 = 0; i14 <= b_vectorUB; i14 += 2) {
                                    __m128d r3;
                                    __m128d r4;
                                    r3 = _mm_loadu_pd(&r2D[i14]);
                                    r4 = _mm_loadu_pd(&r1D[i14]);
                                    _mm_storeu_pd(
                                        &r2D[i14],
                                        _mm_mul_pd(_mm_add_pd(_mm_mul_pd(r3, _mm_set1_pd(c_y)),
                                                              _mm_mul_pd(r4, _mm_set1_pd(b_udd))),
                                                   _mm_div_pd(r4, _mm_set1_pd(b_n))));
                                }
                                for (i14 = b_scalarLB; i14 < h_loop_ub; i14++) {
                                    r2D[i14] =
