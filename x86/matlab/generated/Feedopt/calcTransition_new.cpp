...
static bool check_continuity(const queue_coder *ctx_q_spline, const bool ctx_cfg_maskTot_data[],
                             ...
                             int ctx_cfg_NCart, int ctx_cfg_NRot, const CurvStruct *curv1,
                             ...
                             const CurvStruct *curv2)
{
    ...
    bool isValid;
    bool y;
    // 'calcTransition_new:77' tol         = 1E-6;
    // 'calcTransition_new:78' tol_cos     = deg2rad( 1 );
    // 'calcTransition_new:79' tol_kappa   = 1E-3;
    // 'calcTransition_new:81' [ r11, r1d1, r1dd1 ] = EvalCurvStruct( ctx, curv1, 1 );
    ...
    // 'calcTransition_new:82' [ r21, r2d1, r2dd1 ] = EvalCurvStruct( ctx, curv2, 0 );
    f_EvalCurvStruct(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
                     ctx_cfg_maskCart_data, ctx_cfg_maskCart_size, ctx_cfg_maskRot_data,
                     ctx_cfg_maskRot_size, ctx_cfg_indCart, ctx_cfg_indRot, ctx_cfg_NumberAxis,
                     ctx_cfg_NCart, ctx_cfg_NRot, curv2, r21, r2d1, r2dd1);
    // 'calcTransition_new:84' [t1, ~,  kappa1] = calc_t_nk_kappa( r1d1, r1dd1 );
    calc_t_nk_kappa(r1d1, r1dd1, t1, a__5, &kappa1);
    // 'calcTransition_new:85' [t2, ~,  kappa2] = calc_t_nk_kappa( r2d1, r2dd1 );
    calc_t_nk_kappa(r2d1, r2dd1, t2, a__6, &kappa2);
    // 'calcTransition_new:87' isC0    = all( abs( r11    -r21 ) < tol, 'all' );
    ...
    // 'calcTransition_new:88' isG1    = collinear(t1, t2, tol_cos);
    ...
    if ((coder::b_norm(t1) < 2.2204460492503131E-16) ||
        ...
        isG1 =
            (c / (std::sqrt(coder::sum(r2)) * std::sqrt(coder::sum(r3))) >= 0.017453292519943295);
    }
    // 'calcTransition_new:89' isG2    = abs( kappa1 -kappa2 )   < tol_kappa;
    // 'calcTransition_new:91' isValid = isC0 && isG1 && isG2;
                        ...
                        CurvStruct *curv2C, CurvStruct *curvT)
{
    ...
    // 'calcTransition_new:24' if( ( L1 < 3 * Lcut1 ) || ( L2 < 3 * Lcut2 ) )
    if ((L1 < 3.0 * ctx_cfg_CutOff) || (L2 < 3.0 * ctx_cfg_CutOff)) {
        // 'calcTransition_new:25' curv1C  = curv1;
        *curv1C = *curv1;
        // 'calcTransition_new:25' curv2C  = curv2;
        *curv2C = *curv2;
        // 'calcTransition_new:25' curvT   = curv1;
        *curvT = *curv1;
        // 'calcTransition_new:26' status  = TransitionResult.NoSolution;
        b_status = TransitionResult_NoSolution;
    } else {
        // 'calcTransition_new:30' [ ~, curv1C, ~ ] = cutCurvStruct( ctx, curv1, 0, L1-Lcut1, false
        // );
        ...
        // 'calcTransition_new:31' assert( check_curv_length( ctx, curv1C, L1-Lcut1 ), mfilename + "
        // Curve Length not valide"); 'calcTransition_new:95' tol = 1E-5; 'calcTransition_new:97'
        ...
        // 'calcTransition_new:33' assert( check_curv_length( ctx, curv2C, L2-Lcut2 ), mfilename + "
        // Curve Length not valide"); 'calcTransition_new:95' tol = 1E-5; 'calcTransition_new:97'
        // isValid = ( abs( LengthCurv( ctx, curv, 0, 1 ) - L ) <= tol );
        LengthCurv(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size, ctx_cfg_maskCart_data,
                   ctx_cfg_maskCart_size, ctx_cfg_maskRot_data, ctx_cfg_maskRot_size,
        ...
        // 'calcTransition_new:36' [r1D0, r1D1, r1D2] = EvalCurvStruct( ctx, curv2C, 0 );
        f_EvalCurvStruct(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
                         ctx_cfg_maskCart_data, ctx_cfg_maskCart_size, ctx_cfg_maskRot_data,
                         ctx_cfg_maskRot_size, ctx_cfg_indCart, ctx_cfg_indRot, ctx_cfg_NumberAxis,
                         ctx_cfg_NCart, ctx_cfg_NRot, curv2C, r1D0, r1D1, r1D2);
        // 'calcTransition_new:39' [p5, ret] = G2_Hermite_Interpolation_nAxis(ctx, r0D0, r0D1, r0D2,
        // ... 'calcTransition_new:40'                                                 r1D0, r1D1,
        // r1D2);
        G2_Hermite_Interpolation_nAxis(ctx_cfg_indRot, ctx_cfg_NumberAxis, ctx_cfg_coeffD, r0D0,
                                       r0D1, r0D2, r1D0, r1D1, r1D2, p5, &ret);
        // 'calcTransition_new:42' curvT = constrTransP5Struct( curv1.Info, curv1.R1, curv2.R0, p5
        // );
        ...
        // 'calcTransition_new:43' curvT.Info.SpindleSpeed = min( curv1.Info.SpindleSpeed, ...
        // 'calcTransition_new:44'                                curv2.Info.SpindleSpeed );
        curvT->Info.SpindleSpeed = std::fmin(curv1->Info.SpindleSpeed, curv2->Info.SpindleSpeed);
        // 'calcTransition_new:45' curvT.Info.FeedRate     = min( curv1.Info.FeedRate, ...
        // 'calcTransition_new:46'                                curv2.Info.FeedRate );
        curvT->Info.FeedRate = std::fmin(curv1->Info.FeedRate, curv2->Info.FeedRate);
        // 'calcTransition_new:48' if( ret== 1 )
        if (ret == 1) {
            //  standard case
            //  transition CurvStruct calculation
            // 'calcTransition_new:51' status = TransitionResult.Ok;
            b_status = TransitionResult_Ok;
        } else if (ret == 2) {
            // 'calcTransition_new:52' elseif( ret == 2 )
            //  badly conditioned matrix in G2_Hermite()
            // 'calcTransition_new:54' status = TransitionResult.NoSolution;
            b_status = TransitionResult_NoSolution;
        } else if (ret == 6) {
            // 'calcTransition_new:55' elseif( ret == 6 )
            //  TODO: decide in the future...
            //  Now we ignore and construct the transition curve anyway
            // 'calcTransition_new:58' status = TransitionResult.Ok;
            b_status = TransitionResult_Ok;
        } else {
            // 'calcTransition_new:59' else
            // 'calcTransition_new:60' status = TransitionResult.NoSolution;
            b_status = TransitionResult_NoSolution;
        }
        // 'calcTransition_new:63' if( ( status ~= TransitionResult.NoSolution ) && ...
        // 'calcTransition_new:64'     ( all( p5 <= 0, 'all' ) ) )
        ...
        // 'calcTransition_new:68' if( status == TransitionResult.Ok )
        if (b_status == TransitionResult_Ok) {
            bool b_isValid;
            bool isValid;
            // 'calcTransition_new:69' isValid = check_continuity( ctx, curv1C, curvT );
            isValid = check_continuity(
                ...
                ctx_cfg_indRot, ctx_cfg_NumberAxis, ctx_cfg_NCart, ctx_cfg_NRot, curv1C, curvT);
            // 'calcTransition_new:70' isValid = isValid && check_continuity( ctx, curvT, curv2C );
            if (isValid &&
                check_continuity(ctx_q_spline, ctx_cfg_maskTot_data, ctx_cfg_maskTot_size,
                                 ...
                                 ctx_cfg_NumberAxis, ctx_cfg_NCart, ctx_cfg_NRot, curvT, curv2C)) {
                b_isValid = true;
            } else {
                b_isValid = false;
            }
            // 'calcTransition_new:71' if( ~isValid )
            if (!b_isValid) {
                // 'calcTransition_new:71' status = TransitionResult.NoSolution;
                b_status = TransitionResult_NoSolution;
            }
        }
    }
